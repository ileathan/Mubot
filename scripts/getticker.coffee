...
module.exports = (robot) ->
  robot.hear /^stats (\w+)$/i, (msg) ->
    if tickers[msg.match[1].toUpperCase()]
      robot.http("https://api.coinmarketcap.com/v1/ticker/#{tickers[msg.match[1].toUpperCase()]}/")
        .get() (err, response, body) ->
          msg.send JSON.stringify(JSON.parse(body)[0], null, 2)
     else
       msg.send "#{msg.match[1]} is not listed."

  robot.hear /^price (\w+)$/i, (msg) ->
    if tickers[msg.match[1].toUpperCase()]
      robot.http("https://api.coinmarketcap.com/v1/ticker/#{tickers[msg.match[1].toUpperCase()]}/")
        .get() (err, response, body) ->
          msg.send "#{msg.match[1]} strike price is " + JSON.parse(body)[0].price_usd + "$."
     else
       msg.send "#{msg.match[1]} is not listed."

  robot.hear /^ticker (\w+) (\w+)$/i, (msg) ->
    robot.http("https://api.cryptonator.com/api/ticker/#{msg.match[1]}-#{msg.match[2]}")
      .get() (err, response, body) ->
        msg.send "One #{JSON.parse(body).ticker.base} gives you #{JSON.parse(body).ticker.price} #{JSON.parse(body).ticker.target}. [24h Change: #{JSON.parse(body).ticker.change}%]"
