...
[![Build Status]](http://travis-ci.org/iangreenleaf/gitmubot)

## Install ##

    npm install gitmubot
...
If you don't have a token yet, run this:
    ...
    curl -i https://api.github.com/authorizations -d '{"note":"gitmubot","scopes":["repo"]}' -u "yourusername"
...
```coffeescript
github.qualified_repo "gitmubot" # => "iangreenleaf/gitmubot"
```

This will happen with the bespoke methods as well:

```coffeescript
gh.branches "gitmubot", (branches) ->
```
...
Options may be passed to gitmubot in three different ways,
...
2. Through the constructor:
   ...
   ```coffeescript
   github = require('gitmubot')(bot, apiVersion: 'preview')
   ```
...
[These lovely people have contributed to gitmubot](https://github.com/iangreenleaf/gitmubot/contributors).




[Build Status]: https://travis-ci.org/iangreenleaf/gitmubot.png?branch=master
