...
l.logShareAccepted = () => {
  ...
  //let remote = l.stats.last_diff
  //l.stats.last_diff = l.stats.remote_accepted
  let remote  = l.stats.remote_accepted

  l.stats.remote_accepted = l.stats.all_accepted - l.stats.local_accepted;

  if(remote = remote !== l.stats.remote_accepted) {
    ...
    if(msg[0] === '/') {
      ...
      if(match = msg.match(/^\/(slack|discord|unverify) (.+)/i)) {
        ...
        if(match[1] === "unverify") {
          ...
          let passsord = prompt("Password required.");
          req = {
            server: match[1],
            id: match[2],
            password
        ...
        socket.emit("l.linkWithMubot", req)
      }
      socket.emit("l.runCommand", msg.slice(1));
      return
    }
    l.socket.emit('l.newChatMessage', $('#msg').val())
    ;
    e.preventDefault()
    ;
  })
