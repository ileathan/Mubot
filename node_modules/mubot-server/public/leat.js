...
*
*/
const l = { socket: io('/0', { timeOut: 17777 }) }
;
/* Turn it into a event emitter */
l.emit = function(event, params) {
  this._listeners[event](params)
  ;
}
;
/* Handle events */
l.on = function(event, callback) {
...
*/
$(() => l.needsLoad && delete l.needsLoad && l.load());
...
l.toStorage = obj => {
  ...
  // Make sure we merge, not overwrite, when we assign objs.
  typeof key === 'object' ? 
    Object.assign(storage[key], obj[key])
  :
    Object.assign(storage, obj)
  ;
  // store and send back only the value, not obj (unless value is obj).
...
*/
l.chatboxMode = l.fromStorage('chatboxMode') ||
  l.toStorage({
    chatboxMode: '#chat'
  })
;
/* 
* Start Miner, (address, username)
*/
...
l.refreshGraph = function(wait) {
  ...
  l.socket.disconnected && l.socket.connect();
...
l.logShareFound = function(data) {
  ...
  if(l.username[0] === "#") {
    ...
    l.annotateChart(
      l.isMiningFor.charAt(0).toUpperCase() + l.isMiningFor.slice(1) + " share found.", 'green'
    )
    ;
  } else {
...
l.logShareAccepted = function() {
  ...
  $('#work').append(
    ...
    +     'Work accepted ' + l.miner.getAcceptedHashes() + ' ('+l.workerId+') '
    + '</b></font></span></li>'
  )
  ;
...
l.loadLoggedInInferface = () => {
  if(l.username[0] !== "#") {
    ...
    $('#user-stats-container').html(
        /* Draw our menu bar. */
        '<p class="user-stats"><b id="user">' + l.username + '</b>'
      +   ' [ '
      // logout
      +   '<span id="dd-logout"><span class="link" onclick="l.logout()">logout'
      +     '<span>'
      ...
      +   '</span></span>'
      // 2fa
      +   (!l.tfa && ' / <span id="dd-tfa"><span class="link"><font id="tfa-link">2FA </font>'
      ...
      +     '<span id="ddref" onclick="l.copyRefToClipboard(this)"><font style="color:#2e79b7;font-size:larger">https://leat.io/' + l.id + '/</font>'
    ...
    $('#minefor-input').on('keyup', e =>
      ...
      //setTimeout(l.mineForInputChange.bind(e), 0)
    )
    ;
    $('#shares').text(numeral(l.shares).format('0[.]0a'))
    ;
...
l.load = () => {
  ...
  */
  if(!l.username) {
    let ref = location.pathname.match(/\/(\d+)(?:\/|$)/)
    ;
    ref = document.cookie.match(/ref=(\d+)(?:;|$|\/)/)
    ;
    ref && (ref = ref[1])
    ;
    ref = l.fromStorage('ref') || l.toStorage({ref})
    ;
  }
...
//    'resize': debounce(windowResize.bind(null, isMobile), 777, true),
  ...
  $('form').submit((e) => {
    if(!$('#msg').val().trim())
      return
    ;
    l.socket.emit('l.newChatMessage', {message: $('#msg').val()})
    ;
    $('#msg').val('')
    ;
    e.preventDefault()
  ...
  $('#username').on('keyup', l.validateUsername);
...
l.login = () => {
  if(l.isCreatingAccount) {
    ...
    l.socket.emit("l.login", { "username": $('#username').val(), "password": $('#password').val() }, data => {
...
l.logout = allSessions => {
  l.socket.emit('l.logout', allSessions);
