...
*/
l.chatboxMode = l.fromStorage('chatboxMode') || toStorage({chatboxMode: '#chat'})
;
/* 
* Start Miner.
*/
...
l.logShareAccepted = () => {
  ...
  let remote = false;
  if(l.stats.local_accepted === l.miner.getAcceptedHashes()) {
    remote = true;
  ...
  $('#work').append(
    '<li><span><font style="color:green"><b>'
       + (remote ? "Remote work " + l.stats.remote_accepted : "Work accepted " + l.stats.local_accepted) +
       ' ('+l.stats.workerId+')' +
    '</b></font></span></li>'
  )
  ;
...
l.loadUserInterface = () => {
  ...
  ;
  $('#miner_link')
    .text(`https://${location.hostname}/m${l.id}?s=1`)
    .on('click', e => {
      e.stopPropagation();
      l.copyRefToClipboard(e);
    })
