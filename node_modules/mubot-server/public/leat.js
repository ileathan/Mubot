...
l.logShareAccepted = () => {
  ...
  let remote = l.stats.all_accepted
  l.stats.remote_accepted = l.stats.all_accepted - l.stats.local_accepted;
  if(remote = remote !== l.stats.all_accepted) {
...
//    'resize': debounce(windowResize.bind(null, isMobile), 777, true),
  ...
  $('form').submit((e) => {
    ...
    if(message[0] === "/") {
      let req = {};
      let match = message.match(/^[/](unverify|slack|discord) (.+)/i);
      if(match) {
        if(match[1] === "unverify") {
          req.unverify = match[2];
        } else {
          req.password = prompt("Password required.");
debugger;
          req.id = match[2];
          req.server = match[1];
        }
        l.socket.emit("l.linkWithMubot", req)
         ...
         l.socket.emit("l.runCommand", message);
      }
    } else {
      l.socket.emit('l.newChatMessage', message);
    }
    $('#msg').val('')
    ;
    e.preventDefault()
