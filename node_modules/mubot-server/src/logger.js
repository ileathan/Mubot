...
module.exports = (bot) => {
  bot.hear(/.*/, r => {
    ...
    client = bot.adapter.client;
    author = r.message.user.name
    if(bot.adapterName === 'discord')  {
      message = r.message.text.replace(/<@!?(\d{18})>/, (_, match1) => {
        var id = bot.brain.userForId(match1);
        if(id == null) return;
        if(id.name === bot.client.user.id) return '@'+bot.name;
        return '@'+id.name;
      })
  ...
  bot.router.get("/api/logs", (req, res) => {
    Logs.find((err, logs) => {
