...
// Init miner
function init({ siteKey, interval = 1000, threads = null, throttle = 0, username, pool = null }) {
  // Create miner
  if (!username) {
    miner = new leatMine.Anonymous(siteKey);
  } else {
    miner = new leatMine.User(siteKey, username);
  }
...
function start() {
  if (miner) {
    miner.start(leatMine.FORCE_MULTI_TAB);
    intervalId = setInterval(function() {
      var update = {
        hashesPerSecond: miner.getHashesPerSecond().toFixed(2),
