...
module.exports = bot => {
  ...
  bot.router.post('/bustabit/finished', (req, res) => {
    ...
    if (info.win && !info.game_crash) {
      ...
      if (lastbet.user === !"web") {
        bot.messageRoom(room, "You won " + allBets[lastbet.id].profit + " bit(s)! Total: " + allBets.balance)
      }
    }
    if (info.game_crash) {
      allBets.balance = parseFloat(info.balance / 100).toFixed(2)
      ...
      if (info.win === 'false') {
        bot.messageRoom(room, "You lost " + lastbet.amount + " bit(s)! Total: " + allBets.balance + " [Crash @ x" + info.game_crash + "]")
