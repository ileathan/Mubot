...
Hubot out of the box doesn't do too much but it is an extensible, scriptable bot friend. There are [hundreds of scripts written and maintained by the community](/docs/#scripts.md) and it's easy to write your own.  You can create a custom script in imubot's `scripts` directory or [create a script package](#creating-a-script-package) for sharing with the community!

## Anatomy of a script

When you created your imubot, the generator also created a `scripts` directory. If you peek around there, you will see some examples of scripts. For a script to be a script, it needs to:

* live in a directory on the imubot script load path (`src/scripts` and `scripts` by default)
...
Hubot can make HTTP calls on your behalf to integrate & consume third party APIs. This can be through an instance of [node-scoped-http-client](https://github.com/technoweenie/node-scoped-http-client) available at `bot.http`. The simplest case looks like:
...
As mentioned, imubot uses [node-scoped-http-client](https://github.com/technoweenie/node-scoped-http-client) to provide a simple interface for making HTTP and HTTPS requests. Under its hood, it's using node's builtin [http](http://nodejs.org/api/http.html) and [https](http://nodejs.org/api/https.html) libraries, but providing an easy DSL for the most common kinds of interaction.
...
Hubot can access the environment he's running in, just like any other node program, using [`process.env`](http://nodejs.org/api/process.html#process_process_env). This can be used to configure how scripts are run, with the convention being to use the `MUBOT_` prefix.
...
Take care to make sure the script can load if it's not defined, give the Hubot developer notes on how to define it, or default to something. It's up to the script writer to decide if that should be a fatal error (e.g. imubot exits), or not (make any script that relies on it to say it needs to be configured. When possible and when it makes sense to, having a script work without any other configuration is preferred.
...
If you are using scripts from imubot-scripts, take note of the `Dependencies` documentation in the script to add. They are listed in a format that can be copy & pasted into `package.json`, just make sure to add commas as necessary to make it valid JSON.
...
module.exports = (bot) ->
  ...
  bot.router.post '/imubot/chatsecrets/:room', (req, res) ->
...
curl -X POST -H "Content-Type: application/json" -d '{"secret":"C-TECH Astronomy"}' http://127.0.0.1:8080/imubot/chatsecrets/general

// defaults Content-Type: application/x-www-form-urlencoded, must st payload=...
curl -d 'payload=%7B%22secret%22%3A%22C-TECH+Astronomy%22%7D' http://127.0.0.1:8080/imubot/chatsecrets/general
```

All endpoint URLs should start with the literal string `/imubot` (regardless of what your bot's name is). This consistency makes it easier to set up webhooks (copy-pasteable URL) and guarantees that URLs are valid (not all bot names are URL-safe).

## Events

Hubot can also respond to events which can be used to pass data between scripts. This is done by encapsulating node.js's [EventEmitter](http://nodejs.org/api/events.html#events_class_events_eventemitter) with `bot.emit` and `bot.on`.
...
module.exports = (bot) ->
  bot.router.post "/imubot/gh-commits", (req, res) ->
    bot.emit "commit", {
        user    : {}, #imubot user object
        repo    : 'https://github.com/github/imubot',
...
If you provide an event, it's highly recommended to include a imubot user or room object in its data. This would allow for imubot to notify a user or room in chat.

## Error Handling

No code is perfect, and errors and exceptions are to be expected. Previously, an uncaught exceptions would crash your imubot instance. Hubot now includes an `uncaughtException` handler, which provides hooks for scripts to do something about exceptions.
...
* Refer to the Hubot as imubot, even if your imubot is named something else. It will automatically be replaced with the correct name. This makes it easier to share scripts without having to update docs.
* For `bot.respond` documentation, always prefix with `imubot`. Hubot will automatically replace this with your bot's name, or the bot's alias if it has one
...
The other sections are more relevant to developers of the bot, particularly dependencies, configuration variables, and notes. All contributions to [imubot-scripts](https://github.com/github/imubot-scripts) should include all these sections that are related to getting up and running with the script.



## Persistence

Hubot has an in-memory key-value store exposed as `bot.brain` that can be
...
* all scripts __bundled__ with your imubot installation under `scripts/` directory
* __community scripts__ specified in `imubot-scripts.json` and shipped in the `imubot-scripts` npm package
...
Start by [checking if an NPM package](/docs/index.md#scripts) for a script like yours already exists.  If you don't see an existing package that you can contribute to, then you can easily get started using the `imubot` script [yeoman](http://yeoman.io/) generator.

## Creating A Script Package

Creating a script package for imubot is very simple.  Start by installing the `imubot` [yeoman](http://yeoman.io/) generator:
...
Once you've got the imubot generator installed, creating a imubot script is similar to creating a new imubot.  You create a directory for your imubot script and generate a new `imubot:script` in it.  For example, if we wanted to create a imubot script called "my-awesome-script":

```
% mkdir imubot-my-awesome-script
% cd imubot-my-awesome-script
% yo imubot:script
```

At this point, the you'll be asked a few questions about the author for the script, name of the script (which is guessed by the directory name), a short description, and keywords to find it (we suggest having at least `imubot, imubot-scripts` in this list).
...
You now have a imubot script repository that's ready to roll! Feel free to crack open the pre-created `src/awesome-script.coffee` file and start building up your script! When you've got it ready, you can publish it to [npmjs](http://npmjs.org) by [following their documentation](https://docs.npmjs.com/getting-started/publishing-npm-packages)!
...
see the [Testing Hubot Scripts](#testing-imubot-scripts) section.
...
For synchronous middleware (never yields to the event loop), imubot will automatically catch errors and emit an an `error` event, just like in standard listeners. Hubot will also automatically call the most recent `done` callback to unwind the middleware stack. Asynchronous middleware should catch its own exceptions, emit an `error` event, and call `done`. Any uncaught exceptions will interrupt all execution of middleware completion callbacks.

# Listener Middleware

Listener middleware inserts logic between the listener matching a message and the listener executing. This allows you to create extensions that run for every matching script. Examples include centralized authorization policies, rate limiting, logging, and metrics. Middleware is implemented like other imubot scripts: instead of using the `hear` and `respond` methods, middleware is registered using `listenerMiddleware`.

## Listener Middleware Examples

A fully functioning example can be found in [imubot-rate-limit](https://github.com/michaelansel/imubot-rate-limit/blob/master/src/rate-limit.coffee).
...
This simple middlware bans imubot use by a particular user, including `hear`
...
bot.receiveMiddleware (context, next, done) ->
  if context.response.message.user.id in BLACKLISTED_USERS
    ...
    # If the message starts with 'imubot' or the alias pattern, this user was
...
Response middleware runs against every message imubot sends to a chat room. It's
...
[imubot-test-helper](https://github.com/mtsmfm/imubot-test-helper) is a good
framework for unit testing Hubot scripts.  (Note that, in order to use
imubot-test-helper, you'll need a recent Node version with support for Promises.)

Install the package in your Hubot instance:

``` % npm install imubot-test-helper --save-dev ```
...
[Hubot sample script](https://github.com/github/generator-imubot/blob/master/generators/app/templates/scripts/example.coffee).  This script uses *Mocha*, *chai*, *coffee-script*, and of course *imubot-test-helper*:

**test/example-test.coffee**
```coffeescript
Helper = require('imubot-test-helper')
chai = require 'chai'
  ...
  it 'doesn\'t need badgers', ->
    @room.user.say('alice', 'did someone call for a badger?').then =>
      expect(@room.messages).to.eql [
        ...
        ['imubot', 'Badgers? BADGERS? WE DON\'T NEED NO STINKIN BADGERS']
      ]

  it 'won\'t open the pod bay doors', ->
    @room.user.say('bob', '@imubot open the pod bay doors').then =>
      expect(@room.messages).to.eql [
        ['bob', '@imubot open the pod bay doors']
        ['imubot', '@bob I\'m afraid I can\'t let you do that.']
      ]

  it 'will open the dutch doors', ->
    @room.user.say('bob', '@imubot open the dutch doors').then =>
      expect(@room.messages).to.eql [
        ['bob', '@imubot open the dutch doors']
        ['imubot', '@bob Opening dutch doors']
      ]
```
