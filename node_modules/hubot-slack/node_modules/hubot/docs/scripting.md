...
module.exports = (bot) ->
  ...
  bot.router.post '/hubot/chatsecrets/:room', (req, res) ->
...
All endpoint URLs should start with the literal string `/hubot` (regardless of what your bot's name is). This consistency makes it easier to set up webhooks (copy-pasteable URL) and guarantees that URLs are valid (not all bot names are URL-safe).
...
module.exports = (bot) ->
  bot.router.post "/hubot/gh-commits", (req, res) ->
    bot.emit "commit", {
...
* For `bot.respond` documentation, always prefix with `hubot`. Hubot will automatically replace this with your bot's name, or the bot's alias if it has one
