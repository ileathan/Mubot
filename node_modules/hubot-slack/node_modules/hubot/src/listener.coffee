...
class Listener
  ...
  constructor: (@bot, @matcher, @options, @callback) ->
    if not @matcher?
  ...
  call: (message, middleware, cb) ->
    ...
    if match = @matcher message
      ...
      executeListener = (context, done) =>
        @bot.logger.debug "Executing listener callback for Message '#{message}'"
        try
      ...
      response = new @bot.Response(@bot, message, match)
      middleware.execute(
...
class TextListener extends Listener
  ...
  #            (optional).
  # callback - A Function that is triggered if the incoming message matches.
  constructor: (@bot, @regex, @options, @callback) ->
    @matcher = (message) =>
      if message instanceof TextMessage
        message.match @regex
    super @bot, @matcher, @options, @callback
