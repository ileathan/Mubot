...
historySize = if process.env.MUBOT_SHELL_HISTSIZE?
                parseInt(process.env.MUBOT_SHELL_HISTSIZE)
              else
                1024

historyPath = ".imubot_history"
...
class Shell extends Adapter
  ...
  buildCli: () ->
    ...
    @cli.command '*', (input) =>
      userId = process.env.MUBOT_SHELL_USER_ID or '1'
      if userId.match (/\A\d+\z/)
        userId = parseInt(userId)

      userName = process.env.MUBOT_SHELL_USER_NAME or 'Shell'
      user = @bot.brain.userForId userId, name: userName, room: 'Shell'
  ...
  # Private: load history from .imubot_history.
  #
