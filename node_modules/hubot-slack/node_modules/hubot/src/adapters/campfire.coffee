...
class Campfire extends Adapter
    ...
    if strings.length > 0
      ...
      if typeof(string) == 'function'
        ...
        @bot.Room(envelope.room).speak string, (err, data) =>
          @bot.logger.error "Campfire send error: #{err}" if err?
          @send envelope, strings...
  ...
  topic: (envelope, strings...) ->
    @bot.Room(envelope.room).topic strings.join(" / "), (err, data) =>
      @bot.logger.error "Campfire topic error: #{err}" if err?

  play: (envelope, strings...) ->
    @bot.Room(envelope.room).sound strings.shift(), (err, data) =>
      @bot.logger.error "Campfire sound error: #{err}" if err?
      @play envelope, strings...
  ...
  run: ->
    ...
    bot = new CampfireStreaming(options, @bot)
  ...
  Room: (id) ->
    ...
    listen: ->
      headers =
        ...
        "User-Agent"    : "Hubot/#{@bot?.version} (#{@bot?.name})"
