...
MUBOT_DEFAULT_ADAPTERS = [
  'campfire'
  'shell'
]

MUBOT_DOCUMENTATION_SECTIONS = [
  'description'
  'dependencies'
  'configuration'
  'commands'
...
class Robot
  ...
  constructor: (adapterPath, adapter, httpd, name = 'Hubot', alias = false) ->
    ...
    @logger     = new Log process.env.MUBOT_LOG_LEVEL or 'info'
    @pingIntervalId = null
    @globalHttpOptions = {}
  ...
  # directly to the bot
  #
  # regex - A RegExp for the message part that follows the bot's name/alias
  #
  # Returns RegExp.
  ...
  # Public: Load scripts specified in the `imubot-scripts.json` file.
  #
  # path    - A String path to the imubot-scripts files.
  # scripts - An Array of scripts to load.
  #
  # Returns nothing.
  loadHubotScripts: (path, scripts) ->
    @logger.debug "Loading imubot-scripts from #{path}"
  ...
  #
  # packages - An Array of packages containing imubot scripts to load.
  #
  # Returns nothing.
  ...
  setupExpress: ->
    ...
    app.use (req, res, next) =>
      res.setHeader "X-Powered-By", "imubot/#{@name}"
      next()
    ...
    if herokuUrl
      ...
      @pingIntervalId = setInterval =>
        HttpClient.create("#{herokuUrl}imubot/ping").post() (err, res, body) =>
  ...
  loadAdapter: (adapter) ->
    ...
    try
      path = if adapter in MUBOT_DEFAULT_ADAPTERS
        "#{@adapterPath}/#{adapter}"
      else
        "imubot-#{adapter}"
  ...
  parseHelp: (path) ->
    ...
    for line in body.split "\n"
      ...
      if nextSection in MUBOT_DOCUMENTATION_SECTIONS
    ...
    if currentSection is null
      ...
      for line in body.split("\n")
        ...
        cleanedLine = line[2..line.length].replace(/^imubot/i, @name).trim()
  ...
  #    bot.http("https://example.com", {rejectUnauthorized: false})
  #
