...
(function() {
  var Adapter, CatchAllMessage, EnterMessage, LeaveMessage, Message, ReactionMessage, Bot, SlackBot, SlackClient, TextMessage, TopicMessage, ref,
    bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    hasProp = {}.hasOwnProperty,
    slice = [].slice;

  ref = require('mubot'), Adapter = ref.Adapter, TextMessage = ref.TextMessage, EnterMessage = ref.EnterMessage, LeaveMessage = ref.LeaveMessage, TopicMessage = ref.TopicMessage, Message = ref.Message, CatchAllMessage = ref.CatchAllMessage, Bot = ref.Bot;

  SlackClient = require('./client');

  ReactionMessage = require('./reaction-message');

  Bot.prototype.react = function(matcher, options, callback) {
    var matchReaction;
  ...
  SlackBot = (function(superClass) {
    ...
    function SlackBot(bot, options1) {
      this.bot = bot;
      ...
      this.client = new SlackClient(this.options, this.bot);
    }
    ...
    SlackBot.prototype.run = function() {
      ...
      if (!this.options.token) {
        return this.bot.logger.error("No service token provided to Hubot");
      }
      if (!((ref1 = this.options.token.substring(0, 5)) === 'xoxb-' || ref1 === 'xoxp-')) {
        return this.bot.logger.error("Invalid service token provided, please follow the upgrade instructions");
      }
      ...
      this.bot.brain.on('loaded', (function(_this) {
        return function() {
    ...
    SlackBot.prototype.open = function() {
      this.bot.logger.info('Slack client now connected');
    ...
    SlackBot.prototype.authenticated = function(identity) {
      ...
      this.bot.name = this.self.name;
      return this.bot.logger.info("Logged in as " + this.bot.name + " of " + team.name);
    };
    ...
    SlackBot.prototype.close = function() {
      if (this.options.autoReconnect) {
        return this.bot.logger.info('Slack client closed, waiting for reconnect');
      } else {
        this.bot.logger.info('Slack client connection was closed, exiting mubot process');
    ...
    SlackBot.prototype.error = function(error) {
      if (error.code === -1) {
        return this.bot.logger.warning("Received rate limiting error " + (JSON.stringify(error)));
      }
    ...
    SlackBot.prototype.reply = function() {
      ...
      for (i = 0, len = messages.length; i < len; i++) {
        ...
        if (message !== '') {
          ...
          this.bot.logger.debug("Sending to " + envelope.room + ": " + message);
          sent_messages.push(this.client.send(envelope, message));
        }
      }
      return sent_messages;
    ...
    SlackBot.prototype.message = function(message) {
      ...
      switch (subtype) {
        case 'message':
        case 'bot_message':
          this.bot.logger.debug("Received message: '" + text + "' in channel: " + channel.name + ", from: " + user.name);
          textMessage = new TextMessage(user, text, message.ts);
          textMessage.thread_ts = message.thread_ts;
          return this.receive(textMessage);
        case 'channel_join':
        case 'group_join':
          this.bot.logger.debug(user.name + " has joined " + channel.name);
          return this.receive(new EnterMessage(user));
        case 'channel_leave':
        case 'group_leave':
          this.bot.logger.debug(user.name + " has left " + channel.name);
          return this.receive(new LeaveMessage(user));
        case 'channel_topic':
        case 'group_topic':
          this.bot.logger.debug(user.name + " set the topic in " + channel.name + " to " + topic);
          return this.receive(new TopicMessage(user, message.topic, message.ts));
        default:
          this.bot.logger.debug("Received message: '" + text + "' in channel: " + channel.name + ", subtype: " + subtype);
          message.user = user;
    ...
    SlackBot.prototype.loadUsers = function(err, res) {
      ...
      if (err || !res.ok) {
        this.bot.logger.error("Can't fetch users");
        return;
      }
      ref1 = res.members;
      results = [];
    ...
    SlackBot.prototype.userChange = function(event_or_user) {
      ...
      if (user.id in this.bot.brain.data.users) {
        ref1 = this.bot.brain.data.users[user.id];
        for (key in ref1) {
          value = ref1[key];
          if (!(key in newUser)) {
            newUser[key] = value;
      ...
      delete this.bot.brain.data.users[user.id];
      return this.bot.brain.userForId(user.id, newUser);
    };
