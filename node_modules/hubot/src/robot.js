(function() {
  var Brain, CatchAllMessage, EnterMessage, EventEmitter, Fs, HUBOT_DEFAULT_ADAPTERS, MUBOT_DOC_SECTIONS, HttpClient, LeaveMessage, Listener, Log, Middleware, Path, Response, Robot, TextListener, TopicMessage, User, async, ref, ref1,
  ...
  MUBOT_DOC_SECTIONS = ['description', 'dependencies', 'configuration', 'commands', 'notes', 'author', 'authors', 'examples', 'tags', 'urls'];
  ...
  Robot = (function() {
    ...
    Robot.prototype.parseHelp = function(path) {
      ...
      for (let i = 0, len = lines.length; i < len; i++) {
        ...
        if (MUBOT_DOC_SECTIONS.includes(nextSection)) {
      ...
      if (currentSection === null) {
        ...
        lines = body.split("\n");
        results = [];
        for (let i = 0, len = lines.length; j < len; i++) {
          line = lines[i];
          if(!/(#|\/\//.test(line)) break;
