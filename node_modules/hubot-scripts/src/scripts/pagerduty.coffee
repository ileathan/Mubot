...
# Commands:
#
#   imubot who's on call - return the username of who's on call
#   imubot pager me trigger <msg> - create a new incident with <msg>
#   imubot pager me 60 - take the pager for 60 minutes
#   imubot pager me as <email> - remember your pager email is <email>
#   imubot pager me incidents - return the current incidents
#   imubot pager me incident NNN - return the incident NNN
#   imubot pager me note <incident> <content> - add note to incident #<incident> with <content>
#   imubot pager me notes <incident> - show notes for incident #<incident>
#   imubot pager me problems - return all open incidents
#   imubot pager me ack <incident> - ack incident #<incident>
#   imubot pager me ack <incident1> <incident2> ... <incidentN> - ack all specified incidents
#   imubot pager me ack - ack all triggered incidents 
#   imubot pager me resolve <incident> - resolve incident #<incident>
#   imubot pager me resolve <incident1> <incident2> ... <incidentN>- resolve all specified incidents
#   imubot pager me resolve - resolve all acknowledged incidents
#
# Dependencies:
#  "moment": "1.6.2"
#
...
# Configuration:
#
#   MUBOT_PAGERDUTY_API_KEY - API Access Key
#   MUBOT_PAGERDUTY_SUBDOMAIN
#   MUBOT_PAGERDUTY_SERVICE_API_KEY - Service API Key from a 'General API Service'
#   MUBOT_PAGERDUTY_SCHEDULE_ID
#   MUBOT_PAGERDUTY_ROOM - Room in which you want the PagerDuty webhook notifications to appear
#   MUBOT_PAGERDUTY_ENDPOINT - PagerDuty webhook listener e.g /hook
#
# Authors: 
...
pagerDutyApiKey        = process.env.MUBOT_PAGERDUTY_API_KEY
pagerDutySubdomain     = process.env.MUBOT_PAGERDUTY_SUBDOMAIN
pagerDutyBaseUrl       = "https://#{pagerDutySubdomain}.pagerduty.com/api/v1"
pagerDutyServiceApiKey = process.env.MUBOT_PAGERDUTY_SERVICE_API_KEY
pagerDutyScheduleId    = process.env.MUBOT_PAGERDUTY_SCHEDULE_ID
pagerRoom              = process.env.MUBOT_PAGERDUTY_ROOM
# Webhook listener endpoint. Set it to whatever URL you want, and make sure it matches your pagerduty service settings 
pagerEndpoint          = process.env.MUBOT_PAGERDUTY_ENDPOINT || "/hook"

module.exports = (bot) ->
  bot.respond /pager( me)?$/i, (msg) ->
    ...
    withPagerDutyUser msg, (user) ->
      emailNote = if msg.message.user.pagerdutyEmail
                    ...
                    "I'm assuming your PagerDuty email is #{msg.message.user.email_address}. Change it with `#{bot.name} pager me as you@yourdomain.com`"
      if user
  ...
  bot.respond /(pager|major)( me)? incident (.*)$/, (msg) ->
    pagerDutyIncident msg, msg.match[3], (incident) ->
      msg.send formatIncident(incident)

  bot.respond /(pager|major)( me)? (inc|incidents|sup|problems)$/i, (msg) ->
  ...
  bot.respond /(pager|major)( me)? (?:trigger|page) (.+)$/i, (msg) ->
  ...
  bot.respond /(?:pager|major)(?: me)? ack(?:nowledge)? (.+)$/i, (msg) ->
  ...
  bot.respond /(?:pager|major)(?: me)? res(?:olve)?(?:d)? (.+)$/i, (msg) ->
    incidentNumbers = parseIncidentNumbers(msg.match[1])
  ...
  bot.respond /(pager|major)( me)? notes (.+)$/i, (msg) ->
  ...
  bot.respond /(pager|major)( me)? note ([\d\w]+) (.+)$/i, (msg) ->
    incidentId = msg.match[3]
    content = msg.match[4]
  ...
  # who is on call?
  bot.respond /who('s|s| is)? (on call|oncall)/i, (msg) ->
  ...
  missingEnvironmentForApi = (msg) ->
    ...
    unless pagerDutySubdomain?
      msg.send "PagerDuty Subdomain is missing:  Ensure that MUBOT_PAGERDUTY_SUBDOMAIN is set."
      missingAnything |= true
    unless pagerDutyApiKey?
      msg.send "PagerDuty API Key is missing:  Ensure that MUBOT_PAGERDUTY_API_KEY is set."
      missingAnything |= true
    unless pagerDutyScheduleId?
      msg.send "PagerDuty Schedule ID is missing:  Ensure that MUBOT_PAGERDUTY_SCHEDULE_ID is set."
      missingAnything |= true
    missingAnything
  ...
  withPagerDutyUser = (msg, cb) ->
    ...
    unless email
      msg.send "Sorry, I can't figure out your email address :( Can you tell me with `#{bot.name} pager me as you@yourdomain.com`?"
      return
  ...
  pagerDutyIntegrationAPI = (msg, cmd, description, cb) ->
    unless pagerDutyServiceApiKey?
      ...
      msg.send "Ensure that MUBOT_PAGERDUTY_SERVICE_API_KEY is set."
      return
  ...
  generateIncidentString = (incident, hookType) ->
    ...
    if hookType == "incident.trigger"
      ...
      #{incident.status} and assigned to #{getUserForIncident(incident)}
       #{incident.html_url}
      To acknowledge: @#{bot.name} pager me ack #{incident.incident_number}
       To resolve: @#{bot.name} pager me resolve #{incident.incident_number}
      """
  ...
  if pagerEndpoint && pagerRoom
    bot.router.post pagerEndpoint, (req, res) ->
      bot.messageRoom(pagerRoom, parseWebhook(req,res))
      res.end()
