...
# Configuration:
#   MUBOT_REDMINE_SSL
#   MUBOT_REDMINE_BASE_URL
#   MUBOT_REDMINE_TOKEN
#   MUBOT_REDMINE_IGNORED_USERS
#
# Commands:
#   imubot (redmine|show) me <issue-id> - Show the issue status
#   imubot show (my|user's) issues - Show your issues or another user's issues
#   imubot assign <issue-id> to <user-first-name> ["notes"] - Assign the issue to the user (searches login or firstname)
#   imubot update <issue-id> with "<note>" - Adds a note to the issue
#   imubot add <hours> hours to <issue-id> ["comments"] - Adds hours to the issue with the optional comments
#   imubot link me <issue-id> - Returns a link to the redmine issue
#   imubot set <issue-id> to <int>% ["comments"] - Updates an issue and sets the percent done
#
# Notes:
...
if process.env.MUBOT_REDMINE_SSL?
  HTTP = require('https')
else
  HTTP = require('http')
...
module.exports = (bot) ->
  redmine = new Redmine process.env.MUBOT_REDMINE_BASE_URL, process.env.MUBOT_REDMINE_TOKEN

  # Robot link me <issue>
  bot.respond /link me (?:issue )?(?:#)?(\d+)/i, (msg) ->
    id = msg.match[1]
    msg.reply "#{redmine.url}/issues/#{id}"

  # Robot set <issue> to <percent>% ["comments"]
  bot.respond /set (?:issue )?(?:#)?(\d+) to (\d{1,3})%?(?: "?([^"]+)"?)?/i, (msg) ->
  ...
  bot.respond /add (\d{1,2}) hours? to (?:issue )?(?:#)?(\d+)(?: "?([^"]+)"?)?/i, (msg) ->
  ...
  bot.respond /show (?:my|(\w+\'s)) (?:redmine )?issues/i, (msg) ->
    userMode = true
    firstName =
      if msg.match[1]?
        userMode = false
  ...
  bot.respond /update (?:issue )?(?:#)?(\d+)(?:\s*with\s*)?(?:[-:,])? (?:"?([^"]+)"?)/i, (msg) ->
  ...
  bot.respond /add (?:issue )?(?:\s*to\s*)?(?:"?([^" ]+)"? )(?:tracker\s)?(\d+)?(?:\s*with\s*)("?([^"]+)"?)/i, (msg) ->
  ...
  bot.respond /assign (?:issue )?(?:#)?(\d+) to (\w+)(?: "?([^"]+)"?)?/i, (msg) ->
  ...
  bot.respond /(?:redmine|show)(?: me)? (?:issue )?(?:#)?(\d+)/i, (msg) ->
    id = msg.match[1]
  ...
  bot.hear /.*(#(\d+)).*/, (msg) ->
    ...
    ignoredUsers = process.env.MUBOT_REDMINE_IGNORED_USERS or ""
