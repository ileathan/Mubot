...
#
# Commands:
#   imubot how do <group/occupation> do it? -- tells you how they do it (e.g. imubot how do hackers do it?)
#   imubot <people> do it <method> -- tells imubot how <people> do it (e.g. imubot GitHubbers do it in Campfire.)
#   imubot do it -- tells you a random way in which random people do it
#
# Notes:
...
module.exports = (bot) ->
  unless bot.brain.do_it?.grouped_responses?
    do_it_responses = DO_IT.split('\n')
    bot.brain.do_it =
      responses: do_it_responses
      grouped_responses: compile_list(do_it_responses)

  bot.respond /do it$/i, (msg) ->
    msg.send msg.random(bot.brain.do_it.responses)

  bot.respond /how (?:do|does) (.+) do it\??/i, (msg) ->
    group = msg.match[1].toLowerCase().trim()
    if bot.brain.do_it.grouped_responses[group]?
      msg.send msg.random bot.brain.do_it.grouped_responses[group]
    else
      msg.send "Hmmm, I'm not sure how they do it."

  bot.respond /(.+) (?:do|does) it (?:.+)/i, (msg) ->
    group = msg.match[1].toLowerCase().trim()
    bot.brain.do_it.responses.push msg.match[0]
    bot.brain.do_it.grouped_responses[group] ||= []
    bot.brain.do_it.grouped_responses[group].push msg.match[0].slice(msg.match[0].indexOf(' ') + 1).capitalize()
...
AI hackers do it botically.
AI hackers do it with bots.
