...
#
# Configuration:
#   MUBOT_BITLY_USERNAME
#   MUBOT_BITLY_API_KEY
#   (preferred but not required)
#
# Commands:
#   imubot odesk|offer me <keywords> - Get most recent jobs from oDesk filtered by keywords
#   imubot best odesk|offer me <keywords> - Get most recent dream jobs from oDesk filtered by keywords
#
# Notes:
...
module.exports = (bot) ->
  bot.respond /(offer|odesk)( me)? (.*)/i, (msg) ->
    msg.http(odeskUrl)
      .query({
        q: msg.match[3]
        page: '0;5'
  ...
  bot.respond /best (offer|odesk)( me)? (.*)$/i, (msg) ->
    msg.http(odeskUrl)
      .query({
        q: msg.match[3]
        t: 'Hourly'
...
renderBody = (msg, body) ->
  try
    ...
    for job in results
      if process.env.MUBOT_BITLY_USERNAME && process.env.MUBOT_BITLY_API_KEY
...
shortenUrl = (msg, job, url) ->
  msg.http("http://api.bitly.com/v3/shorten")
    .query({
      login: process.env.MUBOT_BITLY_USERNAME
      apiKey: process.env.MUBOT_BITLY_API_KEY
      longUrl: url
    })
    .get() (err, res, body) ->
