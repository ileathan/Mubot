...
module.exports = (bot) ->
  bot.brain.on 'loaded', =>
    bot.brain.data.stagehand ||= {}
    for env in ['staging', 'development', 'production']
      do (env) ->
        bot.brain.data.stagehand[env] ||= { user: "initial", expires: new Date(0) }

  bot.respond /stagehand book\s?([A-Za-z]+)*\s?(\d+)*/i, (msg) ->
    message = new Message(msg.match[1], msg.match[2])
    env = message.getEnv()
    minutes = message.getMinutes()

    bookEnv(bot.brain.data.stagehand[env], msg.message.user.name, minutes)
    msg.send status(env, bot.brain.data.stagehand[env])

  bot.respond /stagehand who\s?([A-Za-z]+)*/i, (msg) ->
    message = new Message(msg.match[1])
    env = message.getEnv()

    msg.send status(env, bot.brain.data.stagehand[env])

  bot.respond /stagehand cancel\s?([A-Za-z]+)*/i, (msg) ->
    message = new Message(msg.match[1])
    env = message.getEnv()

    cancelBooking(bot.brain.data.stagehand[env])
    msg.send status(env, bot.brain.data.stagehand[env])
