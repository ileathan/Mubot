...
module.exports = (bot) ->
  bot.brain.data.achievements ||= {}

  bot.hear /(.*): *(\+1|thx) for (.*)$/i, (msg) ->
    ...
    if receiver != thanking and reason?
      bot.brain.data.achievements[receiver] ||= []
      event = {reason: reason, given_by: thanking}
      bot.brain.data.achievements[receiver].push event
      msg.send "#{event.given_by} say thanks to #{receiver} for #{event.reason}"

  bot.respond /who thanks me??/i, (msg) ->
    user = msg.message.user.name
    response = "#{user}, #{bot.brain.data.achievements[user].length} time(s) someone thanked you:\n"
    for achievement in bot.brain.data.achievements[user]
      response += "#{achievement.given_by} for #{achievement.reason}\n"
    msg.send response

  bot.respond /(|show )ranking/i, (msg) ->
    ranking = []

    for person, achievements of bot.brain.data.achievements
