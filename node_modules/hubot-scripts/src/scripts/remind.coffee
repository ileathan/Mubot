...
#
# Commands:
#   imubot remind me (on <date>|in <time>) to <action> - Set a reminder in <time> to do an <action> <time> is in the format 1 day, 2 hours, 5 minutes etc. Time segments are optional, as are commas
#   imubot delete reminder <action> - Delete reminder matching <action> (exact match required)
#   imubot show reminders
#
# Author:
#   whitman
#   jtwalters
...
class Reminders
  constructor: (@bot) ->
    ...
    @bot.brain.on('loaded', =>
      if @bot.brain.data.reminders
        @cache = _.map(@bot.brain.data.reminders, (item) ->
          new Reminder(item)
        )
    ...
    @bot.brain.on('save', =>
      @bot.brain.data.reminders = @cache
    )
  ...
  queue: ->
    ...
    trigger = =>
      ...
      @bot.reply(reminder.msg_envelope, 'you asked me to remind you to ' + reminder.action)
      @queue()
...
module.exports = (bot) ->
  reminders = new Reminders(bot)

  bot.respond(/show reminders$/i, (msg) ->
    text = ''
  ...
  bot.respond(/delete reminder (.+)$/i, (msg) ->
    query = msg.match[1]
  ...
  bot.respond(/remind me (in|on) (.+?) to (.*)/i, (msg) ->
    type = msg.match[1]
    time = msg.match[2]
    action = msg.match[3]
    options =
