...
#
# Configuration:
#   MUBOT_WUNDERGROUND_API_KEY Sign up at http://www.wunderground.com/weather/api/.
#   MUBOT_WUNDERGROUND_USE_METRIC Set to arbitrary value to use forecasts with metric system units
#
# Commands:
#   imubot weather me <location> - short-term forecast
#   imubot radar me <location> - recent radar image
#   imubot satellite me <location> - get a recent satellite image
#   imubot weathercam me <location> - get a weather webcam image near location
#
# Notes:
...
module.exports = (bot) ->
  bot.respond /weather (me|at|for|in)? ?(.*)$/i, (msg) ->
    location = msg.match[2]
    get_data bot, msg, location, 'forecast', location.replace(/\s/g, '_'), send_forecast, 60*60*2

  bot.respond /radar (me|at|for|in)? ?(.*)$/i, (msg) ->
    location = msg.match[2]
    get_data bot, msg, location, 'radar', location.replace(/\s/g, '_'), send_radar, 60*10

  bot.respond /satellite (me|at|for|in)? ?(.*)$/i, (msg) ->
    location = msg.match[2]
    get_data bot, msg, location, 'satellite', location.replace(/\s/g, '_'), send_satellite, 60*10

  bot.respond /weathercam (me|at|for|in)? ?(.*)$/i, (msg) ->
    location = msg.match[2]
    get_data bot, msg, location, 'webcams', location.replace(/\s/g, '_'), send_webcam, 60*30

# check cache, get data, store data, invoke callback.
get_data = (bot, msg, location, service, query, cb, lifetime, stack=0) ->
  # what redis key to use
  cache_key = key_for service, location
  bot.brain.data.wunderground or= {}

  data = bot.brain.data.wunderground[cache_key]
  if data? and ttl(data) <= 0
    #console.log 'needs refresh'
    bot.brain.data.wunderground[cache_key] = data = null
  if data?
    #console.log 'cache is valid'
    cb msg, location, data
  else
    if not process.env.MUBOT_WUNDERGROUND_API_KEY?
      msg.send "MUBOT_WUNDERGROUND_API_KEY is not set. Sign up at http://www.wunderground.com/weather/api/."
      return
    # get new data
    msg
      .http("http://api.wunderground.com/api/#{process.env.MUBOT_WUNDERGROUND_API_KEY}/#{service}/q/#{encodeURIComponent query}.json")
      .get() (err, res, body) ->
        ...
        # ambiguous place, multiple matches
          ...
          if alts.length == 1 && stack == 0
            get_data bot, msg, location, service, alts[0], cb, lifetime, 1
          else
            msg.send "Possible matches for '#{location}'.\n - #{alts.join('\n - ')}"

        # looks good
        else
          bot.brain.data.wunderground[cache_key] = data
          bot.brain.data.wunderground[cache_key].retrieved = new Date
          bot.brain.data.wunderground[cache_key].lifetime = lifetime
          cb msg, location, bot.brain.data.wunderground[cache_key]

send_forecast = (msg, location, data) ->
  report = data.forecast.txt_forecast.forecastday[0]
  useMetric = process.env.MUBOT_WUNDERGROUND_USE_METRIC?
