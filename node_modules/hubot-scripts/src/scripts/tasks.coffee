...
#
# Commands:
#   imubot task add <task> - Add a task
#   imubot task list tasks - List the tasks
#   imubot task delete <task number> - Delete a task
#
# Author:
#   Crofty
...
class Tasks
  constructor: (@bot) ->
    @cache = []
    @bot.brain.on 'loaded', =>
      if @bot.brain.data.tasks
        @cache = @bot.brain.data.tasks
  nextTaskNum: ->
...
module.exports = (bot) ->
  ...
  bot.respond /(task add|add task) (.+?)$/i, (msg) ->
    task = tasks.add msg.match[2]
    msg.send "Task added: ##{task.num} - #{task.task}"

  bot.respond /(task list|list tasks)/i, (msg) ->
  ...
  bot.respond /(task delete|delete task) #?(\d+)/i, (msg) ->
    taskNum = msg.match[2]
