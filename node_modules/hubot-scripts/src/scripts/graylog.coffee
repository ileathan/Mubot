...
#
# Commands:
#   imubot graylog - output last 5 graylog messages
#   imubot graylog <count> - output last <count> graylog messages
#   imubot graylog streams - list graylog streams
#   imubot graylog hosts - list graylog hosts
#   imubot graylog <stream> <count> - output some messages from given stream
#   imubot graylog host <host> <count> - output some messages from given host
#
# Notes
...
module.exports = (bot) ->
  ...
  bot.respond /graylog streams$/i, (msg) ->
    graylogStreams msg, (what) ->
      msg.send what

  bot.respond /graylog hosts$/i, (msg) ->
    graylogHosts msg, (what) ->
      msg.send what

  bot.respond /graylog$/i, (msg) ->
    graylogStreamMessages msg, 'all', 5, (what) ->
      msg.send what

  bot.respond /graylog (\d+)$/i, (msg) ->
  ...
  bot.respond /graylog host (.+) (\d+)/i, (msg) ->
    host = msg.match[1]
    count = parseInt(msg.match[2] || '5')
...
graylogList = (msg, url, cb) ->
  url = graylogUrl url
  msg.bot.http(url).get() (err, res, body) ->
