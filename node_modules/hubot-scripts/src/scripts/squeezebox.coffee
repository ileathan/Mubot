...
#
# Commands:
#   imubot pandorame <artist, song, etc> - plays on pandora
#   imubot pause|play
#   imubot vol <positive or negative #> - changes volume
#   imubot crankit|indoor voices - volume presets
#   imubot who's playing? - lists station, artist, song
#   imubot thumbsup|thumbsdown - relay preferences to pandora
#
# Author:
#   kylefritz
...
module.exports = (bot) ->
  ...
  bot.respond /(queue ?up|pandora ?me) (.+)/i, (msg) ->
    queueup(msg, msg.match[2])

  bot.respond /pause/i, (msg) ->
    _cmd msg, ["pause"]
    msg.send "lips are sealed"

  bot.respond /play/i, (msg) ->
    _cmd msg, ["play"]
    msg.send "resuming jam"

  bot.respond /vol \+?(\-?\d+)/i, (msg) ->
    vol(msg,parseInt(msg.match[1] || "10"))
    msg.send "commencing volume adjustment"

  bot.respond /crank ?it/i, (msg) ->
    vol(msg,100)
    msg.send "oh hell yeah"

  bot.respond /indoor voices/i, (msg) ->
    vol msg,-100, () ->
      vol(msg,40)
    msg.send "what do you work at a library?"

  bot.respond /wh(o|at)'?s ?(playing|this)/i, (msg) ->
    artist(msg)

  bot.respond /\(?thumbsup\)?/i,(msg) ->
    _cmd msg, ["pandora","rate",1]
    msg.send "glad you like it"

  bot.respond /yuck|\(?thumbsdown\)?/i,(msg) ->
