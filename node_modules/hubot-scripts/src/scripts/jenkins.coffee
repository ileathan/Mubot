...
#
# Commands:
#   imubot jenkins b <jobNumber> - builds the job specified by jobNumber. List jobs to get number.
#   imubot jenkins build <job> - builds the specified Jenkins job
#   imubot jenkins build <job>, <params> - builds the specified Jenkins job with parameters as key=value&key2=value2
#   imubot jenkins list <filter> - lists Jenkins jobs
#   imubot jenkins describe <job> - Describes the specified Jenkins job
#   imubot jenkins last <job> - Details about the last build for the specified Jenkins job
...
jenkinsBuild = (msg, buildWithEmptyParameters) ->
    url = process.env.MUBOT_JENKINS_URL
    ...
    if process.env.MUBOT_JENKINS_AUTH
      auth = new Buffer(process.env.MUBOT_JENKINS_AUTH).toString('base64')
...
module.exports = (bot) ->
  bot.respond /j(?:enkins)? build ([\w\.\-_ ]+)(, (.+))?/i, (msg) ->
    jenkinsBuild(msg, false)

  bot.respond /j(?:enkins)? b (\d+)/i, (msg) ->
    jenkinsBuildById(msg)

  bot.respond /j(?:enkins)? list( (.+))?/i, (msg) ->
    jenkinsList(msg)

  bot.respond /j(?:enkins)? describe (.*)/i, (msg) ->
    jenkinsDescribe(msg)

  bot.respond /j(?:enkins)? last (.*)/i, (msg) ->
    jenkinsLast(msg)
