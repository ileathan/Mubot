...
plugin = (bot)->
  bot.brain.on 'loaded', =>
    bot.brain.data.outList = []  unless _(bot.brain.data.outList).isArray()

  bot.respond /whoisout/i, (msg)->
    msg.send (plugin.getAbsentees bot, msg.match[1])

  bot.respond /(?:I am|I'm|I will be) out +(.*)/i, (msg)->
    thisDate = plugin.parseDate msg.match[1]?.trim()
    if thisDate
      plugin.save bot, thisDate, msg.message
      msg.send "ok, #{msg.message.user.name} is out on #{thisDate}"
    else
      msg.send 'unable to save date'

  bot.respond /when is (.*)/i, (msg)->
...
plugin.save = (bot, date, msg)->
  userOutList = bot.brain.data.outList
...
plugin.getAbsentees = (bot, targetDate)->
  targetDate = new Date() unless targetDate?
  if _(bot.brain.data.outList).isArray() and (bot.brain.data.outList.length > 0)
    names = []
    _(bot.brain.data.outList).each (item)->
