...
#   imubot list emoji - Print a list of available emojis
#   imubot more emoji - Try to substitute all words - no :colons: required.
#   imubot less emoji - Require :colons: to substitute emojis (default).
#
# Notes:
...
module.exports = (bot) ->
  ...
  bot.brain.on "loaded", ->
    bot.brain.data.emojis ?= {}

  bot.hear /.+/, (msg) ->
    if textWithEmoji = substituteEmoji(msg.message.text, bot)
      msg.send(textWithEmoji)

  bot.respond /more emoji/, (msg) ->
    bot.brain.data.emojis.more = true

  bot.respond /less emoji/, (msg) ->
    bot.brain.data.emojis.more = false

  bot.respond /list emoji/, (msg) ->
    buf = ""
    for symbol, hexCode of emojis
      buf += "#{toEmoji(hexCode)}   #{symbol}\n"
    msg.send buf

substituteEmoji = (text, bot) ->
  ct = 0
  re = if bot.brain.data.emojis.more then /([a-zA-Z0-9_:]{3,})/g else /:([a-zA-Z0-9_]+):/g
