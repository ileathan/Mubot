...
#   MUBOT_JIRA_USER
#   MUBOT_JIRA_PASSWORD
#   MUBOT_JIRA_ISSUE_TYPES
#   MUBOT_JIRA_ISSUE_PRIORITIES
#
# Commands:
#   imubot list my bugs - Retrieve the list of all a user's bugs from JIRA ('my' is optional)
#   imubot list my bugs about <searchterm> - Retrieve list of all a user's bugs from JIRA where the summary or description field contains <phrase> ('my' is optional)
#   imubot list my <priority> priority bugs  - Retrieve the list of a user's <priority> priority bugs from JIRA ('my' is optional)
#   imubot list my <priority> priority bugs about <phrase> - Retrieve list of all a user's <priority> priority bugs from JIRA where the summary or description field contains <phrase> ('my' is optional)
#
# Author:
#   crcastle

# e.g. "bug|task|sub task|support ticket|new feature|epic"
issueTypes = process.env.MUBOT_JIRA_ISSUE_TYPES
issueTypes or= "bug|task|sub task|support ticket|new feature|epic" #some defaults

# e.g. "blocker|high|medium|minor|trivial"
issuePriorities = process.env.MUBOT_JIRA_ISSUE_PRIORITIES
...
getIssues = (msg, issueType, assignee, priority, phrase, callback) ->
    username = process.env.MUBOT_JIRA_USER
    password = process.env.MUBOT_JIRA_PASSWORD
    domain = process.env.MUBOT_JIRA_DOMAIN

    # do some error handling
    unless username
        msg.send "MUBOT_JIRA_USER environment variable must be set to a valid JIRA user's username."
        return
    unless password
        msg.send "MUBOT_JIRA_PASSWORD environment variable must be set to a valid JIRA user's password."
        return
    unless domain
        msg.send "MUBOT_JIRA_DOMAIN environment variables must be set to a valid <ORG>.jira.com domain."
        return

    jiraTypeList = toJiraTypeList(process.env.MUBOT_JIRA_ISSUE_TYPES.split('|'))
