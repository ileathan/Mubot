...
# Configuration:
#   MUBOT_GITHUB_REPO
#     The `user/repository` that you want to connect to. example: github/mubot-scripts
#   MUBOT_GITHUB_USER
...
#       https://github.com/iangreenleaf/gitmubot
#   MUBOT_GITHUB_API
...
#     <SHA>            links to that commit in MUBOT_GITHUB_REPO
#     repo@<SHA>       links to that commit in MUBOT_GITHUB_USER's repo
...
module.exports = (bot) ->
  ...
  bot.hear ///
      ...
      ///i, (msg) ->
    if process.env.MUBOT_GITHUB_REPO && process.env.MUBOT_GITHUB_TOKEN
      if !(msg.message.text.match(/commit\//))
        commit_sha = msg.match[2]
        if msg.match[1]?
          bot_github_repo = github.qualified_repo msg.match[1]
        else
          bot_github_repo = github.qualified_repo process.env.MUBOT_GITHUB_REPO
        issue_title = ""
        ...
        github.get "#{base_url}/repos/#{bot_github_repo}/commits/" + commit_sha, (commit_obj) ->
          if process.env.MUBOT_GITHUB_API
      ...
      msg.send "Hey! You need to set MUBOT_GITHUB_REPO and MUBOT_GITHUB_TOKEN before I can link to that commit."
