...
#
# Commands:
#   imubot travis me <user>/<repo> - Returns the build status of https://github.com/<user>/<repo>
# 
# URLS:
#   POST /imubot/travis?room=<room>[&type=<type]
...
module.exports = (bot) ->
  ...
  bot.router.post "/imubot/travis", (req, res) ->
    ...
    try
      ...
      bot.send user, "#{payload.status_message.toUpperCase()} build (#{payload.build_url}) on #{payload.repository.name}:#{payload.branch} by #{payload.author_name} with commit (#{payload.compare_url})"
