...
#
# Configuration:
#   MUBOT_S3_BRAIN_ACCESS_KEY_ID      - AWS Access Key ID with S3 permissions
#   MUBOT_S3_BRAIN_SECRET_ACCESS_KEY  - AWS Secret Access Key for ID
#   MUBOT_S3_BRAIN_BUCKET             - Bucket to store brain in
#   MUBOT_S3_BRAIN_SAVE_INTERVAL      - [Optional] auto-save interval in seconds
...
module.exports = (bot) ->
  ...
  loaded            = false
  key               = process.env.MUBOT_S3_BRAIN_ACCESS_KEY_ID
  secret            = process.env.MUBOT_S3_BRAIN_SECRET_ACCESS_KEY
  bucket            = process.env.MUBOT_S3_BRAIN_BUCKET
  # default to 30 minutes (in seconds)
  save_interval     = process.env.MUBOT_S3_BRAIN_SAVE_INTERVAL || 30 * 60
  brain_dump_path   = "#{bucket}/brain-dump.json"

  if !key && !secret && !bucket
    throw new Error('S3 brain requires MUBOT_S3_BRAIN_ACCESS_KEY_ID, ' +
      'MUBOT_S3_BRAIN_SECRET_ACCESS_KEY and MUBOT_S3_BRAIN_BUCKET configured')

  save_interval = parseInt(save_interval)
  if isNaN(save_interval)
    throw new Error('MUBOT_S3_BRAIN_SAVE_INTERVAL must be an integer')

  s3 = aws.load('s3', key, secret)

  store_brain = (brain_data, callback) ->
    if !loaded
      bot.logger.debug 'Not saving to S3, because not loaded yet'
      return
    ...
    s3.putBuffer brain_dump_path, buffer, 'private', headers, (err, response) ->
      if err
        bot.logger.error util.inspect(err)
      else if response
        bot.logger.debug "Saved brain to S3 path #{brain_dump_path}"

      if callback then callback(err, response)

  store_current_brain = () ->
    store_brain bot.brain.data
  ...
  s3.get brain_dump_path, 'buffer', (err, response) ->
    ...
    if response && response.buffer
      bot.brain.mergeData JSON.parse(response.buffer.toString())
    else
      bot.brain.mergeData {}

  bot.brain.on 'loaded', () ->
    loaded = true
    bot.brain.resetSaveInterval(save_interval)
    store_current_brain()
