...
#
# Configuration:
#   MUBOT_SENDGRID_USER to your SendGrid username (the same as you use to log in to sendgrid.com)
#   MUBOT_SENDGRID_KEY to your SendGrid password (the same as you use to log in to sendgrid.com)
#
# Commands:
#   imubot sendgrid total - total sendgrid usage for the account
#   imubot sendgrid today - Total usage for today
#   imubot sendgrid last X [days] - Total usage for the last X days
#   imubot sendgrid c[ategory] [total] <category> - Today or all time usage for the given category
#   imubot sendgrid c[ategory] last X [days] <category> - Total usage for the last X days for the given category
#   imubot sendgrid categories - list all categories for account
#
# Author:
#   sixfeetover
#   drdamour
...
module.exports = (bot) ->
  if env.MUBOT_SENDGRID_USER and env.MUBOT_SENDGRID_KEY
    bot.respond /(sendgrid)( me)? today/i, (msg) ->
      opts =
        start_date: getTodayStr
      query msg, opts, (json) ->
        msg.send formatResponse(json[0], json[0].date)

    bot.respond /(sendgrid)( me)? total/i, (msg) ->
      opts =
        aggregate: 1
      query msg, opts, (json) ->
        msg.send formatResponse(json, "All Time")

    bot.respond /(sendgrid)( me)? last (\d+)( days)?/i, (msg) ->
      opts =
        days: msg.match[3]
        aggregate: 1
      query msg, opts, (json) ->
    ...
    bot.respond /(sendgrid)( me)? c(ategory)?( total)? (.*)/i, (msg) ->
    ...
    bot.respond /(sendgrid)( me)? c(ategory)? last (\d+)( days)? (.*)/i, (msg) ->
    ...
    bot.respond /(sendgrid)( me)? categories/i, (msg) ->
      opts =
        list: "true"
      query msg, opts, (json) ->
...
query = (msg, opts, callback) ->
  opts.api_user = env.MUBOT_SENDGRID_USER
  opts.api_key = env.MUBOT_SENDGRID_KEY
