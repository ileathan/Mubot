...
#
# Configuration:
#   Just put this url <MUBOT_URL>:<PORT>/imubot/jenkins-notify?room=<room> to your Jenkins
...
#
# URLS:
#   POST /imubot/jenkins-notify?room=<room>[&type=<type>][&notstrat=<notificationSTrategy>]
#
...
module.exports = (bot) ->
  ...
  bot.router.post "/imubot/jenkins-notify", (req, res) ->
      ...
      if data.build.phase == 'FINISHED' or data.build.phase == 'FINALIZED'
        if data.build.status == 'FAILURE'
          if data.name in @failing
            ...
            build = "started"
          bot.send envelope, "#{data.name} build ##{data.build.number} #{build} failing (#{encodeURI(data.build.full_url)})" if shouldNotify(envelope.notstrat, data, @failing)
        ...
        if data.build.status == 'SUCCESS'
          if data.name in @failing
            ...
            build = "succeeded"
          bot.send envelope, "#{data.name} build ##{data.build.number} #{build} (#{encodeURI(data.build.full_url)})"  if shouldNotify(envelope.notstrat, data, @failing)
