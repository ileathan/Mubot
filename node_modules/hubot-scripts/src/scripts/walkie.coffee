...
#
# Configuration:
#   MUBOT_WALKIE_USERNAME - Basecamp account username
#   MUBOT_WALKIE_PASSWORD - Basecamp account password
#   MUBOT_WALKIE_ROOMS - comma-separated list of rooms
#
# Commands:
#   imubot walkie on <projectURL> - Start watching events for the project
#   imubot walkie off <projectURL> - Stop watching events for the project
#
# Author:
#   tybenz
...
module.exports = (bot) ->
  ...
  if process.env.MUBOT_WALKIE_ROOMS
    allRooms = process.env.MUBOT_WALKIE_ROOMS.split(',')
  else
    allRooms = []

  user = process.env.MUBOT_WALKIE_USERNAME
  pass = process.env.MUBOT_WALKIE_PASSWORD
  if user and pass
  ...
  startListening = ->
    ...
    if (auth)
      interval 10000, ->
        listeners = bot.brain.data.walkie
        for i, project of listeners
          project = JSON.parse project
          bot.http("https://basecamp.com/#{project.accountID}/api/v1/projects/#{project.projectID}/events.json?since=#{project.timestamp}")
              ...
              switch res.statusCode
                when 200
                  ...
                  for event, i in events
                    ...
                    bot.messageRoom allRooms, message
                else
  ...
  bot.brain.on 'loaded', =>
    bot.brain.data.walkie ||= {}
    startListening()

  # Start listening for events on project
  bot.respond /walkie on ([\S]*)/i, (msg) ->
              ...
              when 200
                ...
                for p, i in projects
                  if p.id is projectID
                    target = p
                if target
                  bot.brain.data.walkie["#{accountID}/#{projectID}"] = JSON.stringify { projectName: target.name, accountID: accountID, projectID: projectID, timestamp: strftime(format) }
  ...
  bot.respond /walkie off ([\S]*)/i, (msg) ->
    if not(user and pass and allRooms.length > 0)
      msg.send("Walkie's config variables are not set")
    else
      url = msg.match[1]
      if /http(s)?\:\/\//.test(url)
        ...
        if bot.brain.data.walkie["#{accountID}/#{projectID}"]?
          msg.send "Walkie has stopped scanning on #{JSON.parse(bot.brain.data.walkie["#{accountID}/#{projectID}"]).projectName}"
          delete bot.brain.data.walkie["#{accountID}/#{projectID}"]
        else
  ...
  bot.respond /walkie fetch ([\S]*)/i, (msg) ->
    fetch = msg.match[1]
    data = bot.brain.data.walkie[fetch]
    if data
      msg.send data
    else
  ...
  bot.respond /walkie clear/i, (msg) ->
    for i, item of bot.brain.data.walkie
      delete bot.brain.data.walkie[i]
