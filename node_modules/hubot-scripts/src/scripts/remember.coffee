...
#
# Commands:
#   mubot what is|remember <key> - Returns a string
#   mubot remember <key> is <value>. - Returns nothing. Remembers the text for next time!
#   mubot what do you remember - Returns everything mubot remembers.
#   mubot forget <key> - Removes key from mubots brain.
#   mubot what are your favorite memories? - Returns a list of the most remembered memories.  
#   mubot random memory - Returns a random string
#
# Dependencies:
#   "underscore": "*"
...
module.exports = (bot) ->
  memoriesByRecollection = () -> bot.brain.data.memoriesByRecollection ?= {}
  memories = () -> bot.brain.data.remember ?= {}

  findSimilarMemories = (key) ->
    searchRegex = new RegExp(key, 'i')
    Object.keys(memories()).filter (key) -> searchRegex.test(key)

  bot.respond /(?:what is|rem(?:ember)?)\s+(.*)/i, (msg) ->
    words = msg.match[1]
  ...
  bot.respond /what do you remember/i, (msg) ->
    msg.finish()
    keys = []
    keys.push key for key of memories()
    msg.send "I remember:\n#{keys.join('\n')}"

  bot.respond /what are your favorite memories/i, (msg) ->
    msg.finish()
  ...
  bot.respond /(me|random memory|memories)$/i, (msg) ->
    msg.finish()
    randomKey = msg.random(Object.keys(memories()))
    msg.send randomKey
    msg.send memories()[randomKey]

  bot.respond /mem(ory)? bomb x?(\d+)/i, (msg) ->
    keys = []
