...
#
# Notes:
#   MUBOT_DRUPALORG_LINKDELAY: number of seconds to not respond to a link again after it's been
...
module.exports = (bot) ->
  ...
  linkDelay = process.env.MUBOT_DRUPALORG_LINKDELAY || 30
  ...
  fetchPage = (msg) ->
    ...
    recentLinks.add url
    request {url: url, headers: {'User-Agent': 'imubot'}}, (err, res, body) ->
      if err
  ...
  bot.hear /https?:\/\/(www.)?drupal.org\/node\/(\d+)/, fetchPage
