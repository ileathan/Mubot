...
#
# Configuration:
#   MUBOT_TRELLO_KEY - Trello application key
#   MUBOT_TRELLO_TOKEN - Trello API token
#   MUBOT_TRELLO_LIST - The list ID that you'd like to create cards for
#
# Commands:
#   imubot trello card <name> - Create a new Trello card
#   imubot trello show - Show cards on list
#
# Notes:
...
module.exports = (bot) ->
  bot.respond /trello card (.*)/i, (msg) ->
    cardName = msg.match[1]
    if not cardName.length
      msg.send "You must give the card a name"
      return
    if not process.env.MUBOT_TRELLO_KEY
      msg.send "Error: Trello app key is not specified"
    if not process.env.MUBOT_TRELLO_TOKEN
      msg.send "Error: Trello token is not specified"
    if not process.env.MUBOT_TRELLO_LIST
      msg.send "Error: Trello list ID is not specified"
    if not (process.env.MUBOT_TRELLO_KEY and process.env.MUBOT_TRELLO_TOKEN and process.env.MUBOT_TRELLO_LIST)
      return
    createCard msg, cardName
    
  bot.respond /trello show/i, (msg) ->
    showCards msg

createCard = (msg, cardName) ->
  Trello = require("node-trello")
  t = new Trello(process.env.MUBOT_TRELLO_KEY, process.env.MUBOT_TRELLO_TOKEN)
  t.post "/1/cards", {name: cardName, idList: process.env.MUBOT_TRELLO_LIST}, (err, data) ->
    if err
...
showCards = (msg) ->
  Trello = require("node-trello")
  t = new Trello(process.env.MUBOT_TRELLO_KEY, process.env.MUBOT_TRELLO_TOKEN)
  t.get "/1/lists/"+process.env.MUBOT_TRELLO_LIST, {cards: "open"}, (err, data) ->
    if err
