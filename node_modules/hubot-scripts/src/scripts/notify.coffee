...
#
# Commands:
#   imubot notify me by prowl with YOUR_PROWL_API_KEY
#   imubot notify me by nma with YOUR_NMA_API_KEY
#   imubot notify me by sms with 15556667890
#   imubot list notifiers
#
# Author:
#   marten
#
...
module.exports = (bot) ->
  sendSms = (to, msg) ->
    sid   = process.env.MUBOT_SMS_SID
    tkn   = process.env.MUBOT_SMS_TOKEN
    from  = process.env.MUBOT_SMS_FROM
    ...
    unless sid
      ...
      msg.send "Please set the MUBOT_SMS_SID environment variable."
      return

    unless tkn
      msg.send "Twilio token isn't set."
      msg.send "Please set the MUBOT_SMS_TOKEN environment variable."
      return

    unless from
      msg.send "Twilio from number isn't set."
      msg.send "Please set the MUBOT_SMS_FROM environment variable."
      return
  ...
  notify = (username, msg) ->
    ...
    if username == "all" or username == "everyone"
      for username, apikey of bot.brain.data.notifiers
        unless username.toLowerCase() == msg.message.user.name.toLowerCase()
          notifies.push apikey
    else if apikey = bot.brain.data.notifiers[username.toLowerCase()]
      notifies.push apikey
  ...
  checkIfOffline = (user, callback) ->
    if process.env.MUBOT_HIPCHAT_TOKEN
      options =
        ...
        path: "/v1/users/show?"+QS.stringify({
          format:"json"
          user_id:user.id
          auth_token:process.env.MUBOT_HIPCHAT_TOKEN
        })
        method: "GET"
  ...
  bot.hear /@(\w+)/i, (msg) ->
    ...
    for userId, user of bot.brain.users
  ...
  bot.respond /do not notify me/i, (msg) ->
    delete bot.brain.data.notifiers[msg.message.user.name.toLowerCase()]
    msg.send "OK"


  bot.respond /notify me by prowl with (\w+)/i, (msg) ->
    apikey = msg.match[1].toLowerCase()
    bot.brain.data.notifiers ?= {}
    bot.brain.data.notifiers[msg.message.user.name.toLowerCase()] = "prowl:#{apikey}"
    msg.send "OK"

  bot.respond /notify me by nma with (\w+)/i, (msg) ->
    apikey = msg.match[1].toLowerCase()
    bot.brain.data.notifiers ?= {}
    bot.brain.data.notifiers[msg.message.user.name.toLowerCase()] = "nma:#{apikey}"
    msg.send "OK"

  bot.respond /notify me by sms with (\d+)/i, (msg) ->
    number = msg.match[1].toLowerCase()
    bot.brain.data.notifiers ?= {}
    bot.brain.data.notifiers[msg.message.user.name.toLowerCase()] = "sms:#{number}"
    msg.send "OK"


  bot.respond /list notifiers/i, (msg) ->
    for username, apikey of bot.brain.data.notifiers
