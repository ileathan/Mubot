...
class ScoreKeeper
  constructor: (@bot) ->
    ...
    @bot.brain.on 'loaded', =>
      @bot.brain.data.scores ||= {}
      @bot.brain.data.scoreLog ||= {}

      @cache.scores = @bot.brain.data.scores
      @cache.scoreLog = @bot.brain.data.scoreLog
  ...
  saveUser: (user, from) ->
    ...
    @bot.brain.data.scores[user] = @cache.scores[user]
    @bot.brain.data.scoreLog[from] = @cache.scoreLog[from]
    @bot.brain.emit('save', @bot.brain.data)
...
module.exports = (bot) ->
  scoreKeeper = new ScoreKeeper(bot)

  bot.hear /([\w\S]+)([\W\s]*)?(\+\+)$/i, (msg) ->
