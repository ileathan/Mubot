...
#
# Configuration:
#   MUBOT_HELPSCOUT_API_KEY - Go to Your Profile -> API Keys
#
# Commands:
#   imubot hs mailboxes - list Helpscout mailboxes
#   imubot hs count MAILBOX_ID - Return the number of active tickets in a mailbox
#   imubot hs users MAILBOX_ID - List the number of active tickets each user has in a mailbox
#
# Author:
...
api_key = process.env.MUBOT_HELPSCOUT_API_KEY

getRequest = (msg, path, callback) ->
  auth = 'Basic ' + new Buffer("#{api_key}:x").toString('base64')
  msg.bot.http("#{url}#{path}")
    .headers("Authorization": auth, "Accept": "application/json")
    .get() (err, res, body) ->
      callback(err, res, body)

module.exports = (bot) ->
  # imubot helpscout users FOLDER_ID
  bot.respond /hs users\s?(@\w+)?(.*)/i, (msg) ->
    if api_key
      ...
      msg.send "Don't have the MUBOT_HELPSCOUT_API_KEY."

  # imubot helpscout count FOLDER_ID
  bot.respond /hs count\s?(@\w+)?(.*)/i, (msg) ->
    if api_key
      mailboxId = msg.match[2] # Second Term
  ...
  # imubot helpscout mailboxes  
  bot.respond /hs mailboxes\s?(.*)?/i, (msg) ->
    if api_key
