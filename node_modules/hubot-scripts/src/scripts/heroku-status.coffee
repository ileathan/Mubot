...
#
# Commands:
#   imubot heroku status - Returns the current Heroku status for app operations and tools
#   imubot heroku status issues <limit> - Returns a list of recent <limit> issues (default limit is 5)
#   imubot heroku status issue <id> - Returns a single issue by ID number
#
# Author:
#   juno

module.exports = (bot) ->
  bot.respond /heroku status$/i, (msg) ->
    status msg

  bot.respond /heroku status issues\s?(\d*)/i, (msg) ->
    limit = msg.match[1] or 5
    statusIssues msg, limit

  bot.respond /heroku status issue (\d+)/i, (msg) ->
    id = msg.match[1]
    statusIssue msg, id
