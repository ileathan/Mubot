...
#   MUBOT_JIRA_USE_V2 (defaults to "true", set to "false" for JIRA earlier than 5.0)
#   MUBOT_JIRA_MAXLIST
#   MUBOT_JIRA_ISSUEDELAY
#   MUBOT_JIRA_IGNOREUSERS
#
# Commands:
#   <Project Key>-<Issue ID> - Displays information about the JIRA ticket (if it exists)
#   imubot show watchers for <Issue Key> - Shows watchers for the given JIRA issue
#   imubot search for <JQL> - Search JIRA with JQL
#   imubot save filter <JQL> as <name> - Save JIRA JQL query as filter in the brain
#   imubot use filter <name> - Use a JIRA filter from the brain
#   imubot show filter(s) - Show all JIRA filters
#   imubot show filter <name> - Show a specific JIRA filter
#
# Author:
#   codec
...
class IssueFilters
  constructor: (@bot) ->
    ...
    @bot.brain.on 'loaded', =>
      jqls_from_brain = @bot.brain.data.jqls
...
module.exports = (bot) ->
  filters = new IssueFilters bot

  useV2 = process.env.MUBOT_JIRA_USE_V2 != "false"
  # max number of issues to list during a search
  maxlist = process.env.MUBOT_JIRA_MAXLIST || 10
  # how long (seconds) to wait between repeating the same JIRA issue link
  issuedelay = process.env.MUBOT_JIRA_ISSUEDELAY || 30
  # array of users that are ignored
  ignoredusers = (process.env.MUBOT_JIRA_IGNOREUSERS.split(',') if process.env.MUBOT_JIRA_IGNOREUSERS?) || []

  recentissues = new RecentIssues issuedelay

  get = (msg, where, cb) ->
    console.log(process.env.MUBOT_JIRA_URL + "/rest/api/latest/" + where)

    httprequest = msg.http(process.env.MUBOT_JIRA_URL + "/rest/api/latest/" + where)
    if (process.env.MUBOT_JIRA_USER)
      authdata = new Buffer(process.env.MUBOT_JIRA_USER+':'+process.env.MUBOT_JIRA_PASSWORD).toString('base64')
        ...
        issue =
          ...
          fixVersion: ->
            if issues.fields.fixVersions? and issues.fields.fixVersions.length > 0
              ...
              "no fix version"
          url: process.env.MUBOT_JIRA_URL + '/browse/' + issues.key
      else
        issue =
          key: issues.key
  ...
  search = (msg, jql, cb) ->
    get msg, "search/?jql=#{escape(jql)}", (result) ->
      ...
      resultText = "I found #{result.total} issues for your search. #{process.env.MUBOT_JIRA_URL}/secure/IssueNavigator.jspa?reset=true&jqlQuery=#{escape(jql)}"
  ...
  bot.respond /(show )?watchers (for )?(\w+-[0-9]+)/i, (msg) ->
    if msg.message.user.id is bot.name
      return

    watchers msg, msg.match[3], (text) ->
      msg.send text

  bot.respond /search (for )?(.*)/i, (msg) ->
    if msg.message.user.id is bot.name
      return
  ...
  bot.respond /save filter (.*) as (.*)/i, (msg) ->
  ...
  bot.respond /delete filter (.*)/i, (msg) ->
    filters.delete msg.match[1]

  bot.respond /(use )?filter (.*)/i, (msg) ->
    name    = msg.match[2]
  ...
  bot.respond /(show )?filter(s)? ?(.*)?/i, (msg) ->
