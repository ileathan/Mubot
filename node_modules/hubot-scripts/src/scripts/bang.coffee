# Description:
#   A imubot interface for Bang, a key-value store for text snippets
#
# Dependencies:
#   "bang": "1.0.1"
#   "shellwords": "0.0.1"
...
#
# Commands:
#   imubot bang [--help|--list|--delete] <key> [value] - Store and retrieve text snippets
#
# Author:
#   jimmycuadra
  ...
  bot.respond /bang\s+(.*)/i, (msg) ->
    ...
    if key in ["-h", "--help"]
      msg.send  """
                ...
                Set a key:    #{bot.name} bang foo bar
                Get a key:    #{bot.name} bang foo
                Delete a key: #{bot.name} bang [-d|--delete] foo
                List keys:    #{bot.name} bang [-l|--list]
                Get help:     #{bot.name} bang [-h|--help]
                """
