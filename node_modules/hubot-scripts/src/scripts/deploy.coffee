...
#
# Configuration:
#   MUBOT_DEPLOY_ROOM - room to send announcements to
#
# Commands:
#   None
#
# URLs:
#   GET /mubot/deploy?environment=<environment>&project=<project>[&version=<version>]
#   GET /mubot/deploy?hosts=<hosts>&wars=<wars>[&version=<version>]
#
...
module.exports = (bot) ->
  ...
  bot.router.get "/mubot/deploy", (req, res) ->
    ...
    user = {}
    user.room = process.env.MUBOT_DEPLOY_ROOM

    message = "DEPLOY: " + project + " was deployed to " + environment
    message += " version " + version if version

    bot.send(user, message)
    if bot.adapter.bot?.Room?
      bot.adapter.bot.Room(user.room).sound "trombone", (err, data) =>
        console.log "campfire error: #{err}" if err

    res.end "Deploy sent"


  bot.router.get "/mubot/deployhost", (req, res) ->
    query = qs.parse(req._parsedUrl.query)
    hosts = query.hosts
    wars = query.wars
    version = query.version if query.version
    ...
    if bot.adapter.bot?
      bot.adapter.bot.Room(user.room).sound "ohyeah", (err, data) =>
        console.log "campfire error: #{err}" if err
