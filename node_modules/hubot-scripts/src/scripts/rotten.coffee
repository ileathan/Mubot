...
#
# Configuration:
#   MUBOT_ROTTEN_TOMATOES_API_KEY
#
# Commands:
#   imubot rotten [me] <movie>
#   imubot what's in theaters?
#   imubot what's coming out in theaters?
#   imubot what's coming out on (dvd|bluray)? - there is not a distinction between dvd and bluray
#
# Author:
#   mportiz08
...
class Rotten
  constructor: (@bot) ->
    ...
    @api_key = process.env.MUBOT_ROTTEN_TOMATOES_API_KEY
  ...
  send: (url, options, callback) =>
    ...
    @bot.http(url).query(options).get()(callback)
...
module.exports = (bot) ->
  rotten = new Rotten bot

  bot.respond /rotten (me )?(.*)$/i, (message) ->
  ...
  bot.respond /what(\')?s in theaters(\?)?$/i, (message) ->
    message.send "Well, let's see..."
  ...
  bot.respond /what(\')?s coming out ((on (dvd|blu(-)?ray))|(in theaters))(\?)?$/i, (message) ->
    message.send "Well, let's see..."
