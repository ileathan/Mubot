...
#
# Commands:
#   imubot show [me] [<limit> [of]] [<assignee>'s|my] [<label>] issues [for <user/repo>] [about <query>] -- Shows open GitHub issues for repo.
#   imubot show [me] issues for <repo> -- List all issues for given repo IFF MUBOT_GITHUB_USER configured
#   imubot show [me] issues for <user/repo> -- List all issues for given repo
#   imubot show [me] issues -- Lists all issues IFF MUBOT_GITHUB_REPO configured
#   imubot show <chat user's> issues -- Lists all issues for chat user IFF MUBOT_GITHUB_USER_(.*) configured
#
# Notes:
#   If, for example, MUBOT_GITHUB_USER_JOHN is set to GitHub user login
...
# Given the text sent to bot.respond (e.g. 'imubot show me...'), parse the
...
complete_assignee = (msg, name) ->
  ...
  resolve = (n) -> process.env["MUBOT_GITHUB_USER_#{n.replace(/\s/g, '_').toUpperCase()}"]
  resolve(name) or resolve(name.split(' ')[0]) or name

module.exports = (bot) ->
  github = require("gitimubot")(bot)
  bot.respond ASK_REGEX, (msg) ->
    ...
    base_url = process.env.MUBOT_GITHUB_API || 'https://api.github.com'
