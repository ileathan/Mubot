...
#
# Commands:
#   imubot sms <user> <message> - Sends <message> to the number <to>
#   imubot <user> has phone number <phone> - Sets the phone number of <user> to <phone>
#   imubot give me the phone number to <user> - Gets the phone number of <user>
#
# Author:
#   kimf
...
module.exports = (bot) ->
  ...
  bot.respond /sms (\w+) (.*)/i, (msg) ->
    ...
    if user = bot.brain.userForName(to)
      if user.phone == ""
      ...
      users = bot.brain.usersForFuzzyName(to)
  ...
  bot.respond /@?([\w .-_]+) has phone number (\d*)*$/i, (msg) ->
    name  = msg.match[1]
  ...
  bot.respond /@?give me the phone number to ([\w .-_]+)*/i, (msg) ->
    name  = msg.match[1]
    users = bot.brain.usersForFuzzyName(name)
    if users.length is 1
      user = users[0]
