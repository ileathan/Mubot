...
#
# Configuration:
#   MUBOT_GERRIT_SSH_URL
#   MUBOT_GERRIT_EVENTSTREAM_ROOMS
#
# Commands:
#   imubot gerrit search <query> - Search Gerrit for changes - the query should follow the normal Gerrit query rules
#   imubot gerrit (ignore|report) events for (project|user|event) <thing> - Tell Hubot how to report Gerrit events
#
# Notes:
...
sshUrl = process.env.MUBOT_GERRIT_SSH_URL || ""
...
eventStreamRooms = process.env.MUBOT_GERRIT_EVENTSTREAM_ROOMS
...
module.exports = (bot) ->
  ...
  if gerrit.protocol != "ssh:" || gerrit.hostname == ""
    bot.logger.error "Gerrit commands inactive because MUBOT_GERRIT_SSH_URL=#{gerrit.href} is not a valid SSH URL"
  else
    eventStreamMe bot, gerrit unless eventStreamRooms == "disabled"
    bot.respond /gerrit (?:search|query)(?: me)? (.+)/i, searchMe bot, gerrit
    bot.respond /gerrit (ignore|report)(?: me)? events for (project|user|event) (.+)/i, ignoreOrReportEventsMe bot, gerrit

searchMe = (bot, gerrit) -> (msg) ->
...
ignoreOrReportEventsMe = (bot, gerrit) -> (msg) ->
  type = msg.match[2].toLowerCase()
  thing = msg.match[3]
  ignores = (t for t in ignoresOfType bot, type when t isnt thing)
  ignores.push thing if msg.match[1] == "ignore"

  bot.brain.data.gerrit ?= { }
  bot.brain.data.gerrit.eventStream ?= { }
  bot.brain.data.gerrit.eventStream.ignores ?= { }
  bot.brain.data.gerrit.eventStream.ignores[type] = ignores
  msg.send "Got it, the updated list of Gerrit #{type}s to ignore is #{ignores.join(', ') || 'empty'}"

eventStreamMe = (bot, gerrit) ->
  bot.logger.info "Gerrit stream-events: Starting connection"
  ...
  streamEvents.on "exit", (code) ->
    bot.logger.info "Gerrit stream-events: Connection lost (rc=#{code})"
    reconnect = setTimeout (-> eventStreamMe bot, gerrit), 10 * 1000 unless done

  isIgnored = (type, thing) -> (t for t in ignoresOfType bot, type when t is thing).length != 0
  isWanted = (event) -> !(
  ...
  streamEvents.stderr.on "data", (data) ->
    bot.logger.info "Gerrit stream-events: #{data}"
  streamEvents.stdout.on "data", (data) ->
    bot.logger.debug "Gerrit stream-events: #{data}"
    json = try
      JSON.parse data
    catch error
      bot.logger.error "Gerrit stream-events: Error parsing Gerrit JSON. Error=#{error}, Event=#{data}"
      null
    return unless json
    ...
    msg = try
      ...
      bot.logger.error "Gerrit stream-events: Error formatting event. Error=#{error}, Event=#{data}"
      null
    if formatter == null
      bot.logger.info "Gerrit stream-events: Unrecognized event #{data}"
    else if msg && isWanted json
      # Bug in messageRoom? Doesn't work with multiple rooms
      #bot.messageRoom room, "Gerrit: #{msg}" for room in botRooms bot
      bot.send room: room, "Gerrit: #{msg}" for room in botRooms bot

ignoresOfType = (bot, type) -> bot.brain.data.gerrit?.eventStream?.ignores?[type] || []
...
botRooms = (bot) ->
  roomlists =
    if eventStreamRooms
      ...
      v for k,v of process.env when /^MUBOT_.+_ROOMS/i.exec(k) isnt null
  bot.logger.error "Gerrit stream-events: Unable to determine the list of rooms" if roomlists.length == 0
