...
#
# Commands:
#   imubot kickstarter start - Start the kickstarter update feed
#   imubot kickstarter change <mins> - Change the interval of kickstarter updates
#   imubot kickstarter stop - Stop the kickstarter update feed
#
# Author:
#   pksunkara
...
module.exports = (bot) ->
  ...
  bot.respond /kickstarter start/i, (msg) ->
    if not init
      init = true
  ...
  bot.respond /kickstarter stop/i, (msg) ->
    if init
      init = false
      clearTimeout timer
      msg.send "Stopped the kickstarter update feed"

  bot.respond /kickstarter change ([1-9][0-9]*)/i, (msg) ->
    clearTimeout timer
  ...
  setTimer = (interval, msg) ->
    timer = setTimeout scrape, interval*60*1000, bot, (err, data) ->
      if not err and data
        setTimer interval, msg
...
scrape = (bot, cb) ->
  bot.http("http://www.kickstarter.com/projects/#{process.env.KICKSTARTER_PROJECT}")
    .get() (err, res, body) ->
