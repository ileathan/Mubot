...
#
# Commands:
#   mubot i am working on <anything> - Set what you're working on
#   mubot what is everyone working on? - Find out what everyone is working on
#
# Author:
#   beezee
...
module.exports = (bot) ->
  ...
  bot.respond /(?:what\'s|what is|whats) @?([\w .\-]+) working on(?:\?)?$/i, (msg) ->
    name = msg.match[1].trim()

    if name is "you"
      msg.send "I dunno, bot things I guess."
    else if name.toLowerCase() is bot.name.toLowerCase()
      msg.send "World domination!"
    else if name.match(/(everybody|everyone)/i)
      messageText = '';
      users = bot.brain.users()
      for k, u of users
          if u.workingon
  ...
  bot.respond /(?:i\'m|i am|im) working on (.*)/i, (msg) ->
