...
#
# Configuration:
#   MUBOT_KLOUT_API_KEY must be set in the environment. 
#   The competitors list is automatically created and includes the last 9 valid twitter handles. 
#
# Commands:
#   imubot klout <twitter name with or without @> - report klout score for twitter handle, and compare to competitors
#
# Author:
#   l_kang
...
klout_api_key = () ->
    return process.env.MUBOT_KLOUT_API_KEY
    
module.exports = (bot) -> 
    bot.respond /.*(klout) [@]?(.+)$/i, (msg) ->
        unless klout_api_key()
            msg.send "I wont report any scores until your environment MUBOT_KLOUT_API_KEY is set"
            return
        name = msg.match[2]
