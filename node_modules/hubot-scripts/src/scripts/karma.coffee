...
#   imubot karma <thing> - check thing's karma (if <thing> is omitted, show the top 5)
#   imubot karma empty <thing> - empty a thing's karma
#   imubot karma best - show the top 5
#   imubot karma worst - show the bottom 5
#
# Author:
#   stuartf
...
class Karma
  ...
  constructor: (@bot) ->
    ...
    @bot.brain.on 'loaded', =>
      if @bot.brain.data.karma
        @cache = @bot.brain.data.karma

  kill: (thing) ->
    delete @cache[thing]
    @bot.brain.data.karma = @cache
...
module.exports = (bot) ->
  ...
  bot.respond /karma empty ?(\S+[^-\s])$/i, (msg) ->
    subject = msg.match[1].toLowerCase()
    if allow_self is true or msg.message.user.name.toLowerCase() != subject
      karma.kill subject
  ...
  bot.respond /karma( best)?$/i, (msg) ->
    verbiage = ["The Best"]
    for item, rank in karma.top()
      verbiage.push "#{rank + 1}. #{item.name} - #{item.karma}"
    msg.send verbiage.join("\n")

  bot.respond /karma worst$/i, (msg) ->
    verbiage = ["The Worst"]
    for item, rank in karma.bottom()
      verbiage.push "#{rank + 1}. #{item.name} - #{item.karma}"
    msg.send verbiage.join("\n")

  bot.respond /karma (\S+[^-\s])$/i, (msg) ->
