...
#
# Commands:
#   mubot reload all scripts - Reloads scripts without restart. Loads new scripts too.
...
module.exports = (bot) ->
  ...
  bot.hear /command count/i, (msg) ->
    msg.send "I am aware of #{msg.bot.commands.length} commands"

  bot.respond /reload all scripts/i, (msg) ->
    try
...
reloadAllScripts = (msg, success, error) ->
  ...
  Fs.exists mubotScripts, (exists) ->
    if exists
      Fs.readFile mubotScripts, (err, data) ->
        if data.length > 0
          try
            ...
            error "Error parsing JSON data from mubot-scripts.json: #{err}"
            return
