...
#
# Commands:
#   imubot responders - List all responders
#   imubot responder /pattern/ - Show a responder
#   imubot forget /pattern/ - Remove a responder
#   imubot respond /pattern/ msg.send(...) - Create a new responder
#
# Notes:
...
class Responders
  constructor: (@bot) ->
    @bot.brain.data.responders = {}
    @bot.brain.on 'loaded', (data) =>
  ...
  remove: (pattern) ->
    ...
    if responder
      if responder.index
        @bot.listeners.splice(responder.index, 1, (->))
  ...
  add: (pattern, callback) ->
    ...
    if eval_pattern instanceof RegExp and eval_callback instanceof Function
      @remove(pattern)
      @bot.respond(eval_pattern, eval_callback)
      @responders()[pattern] = {
        callback: callback,
        index: @bot.listeners.length - 1,
      }
      @responder(pattern)

module.exports = (bot) ->
  responders = new Responders(bot)

  bot.respond /responders/i, (msg) ->
