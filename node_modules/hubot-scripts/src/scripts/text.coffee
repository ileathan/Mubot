...
#
# Configuration:
#   MUBOT_SMSIFIED_USERNAME
#   MUBOT_SMSIFIED_PASSWORD
#   MUBOT_SMSIFIED_SENDERADDRESS
#
# Commands:
#   imubot text <phonenumber> <message> - Sends <message> to <phonenumber>.
#
# Notes: 
...
module.exports = (bot) ->
  bot.respond /text (\d+) (.*)/i, (msg) ->
    address = msg.match[1]
    message = msg.match[2] 
    username = process.env.MUBOT_SMSIFIED_USERNAME
    password = process.env.MUBOT_SMSIFIED_PASSWORD
    senderAddress  = process.env.MUBOT_SMSIFIED_SENDERADDRESS
    auth  = 'Basic ' + new Buffer(username + ':' + password).toString("base64")
    data  = QS.stringify address: address, message: message

    unless username
      msg.send "SMSified username isn't set."
      msg.send "Please set the MUBOT_SMSIFIED_USERNAME environment variable."
      return

    unless password
      msg.send "SMSified password isn't set."
      msg.send "Please set the MUBOT_SMSIFIED_PASSWORD environment variable."
      return

    unless senderAddress
      msg.send "SMSified senderAddress isn't set."
      msg.send "Please set the MUBOT_SMSIFIED_SENDERADDRESS environment variable."
      return
