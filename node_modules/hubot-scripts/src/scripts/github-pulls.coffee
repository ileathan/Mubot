...
#
# Commands:
#   imubot show [me] <user/repo> pulls [with <regular expression>] -- Shows open pull requests for that project by filtering pull request's title.
#   imubot show [me] <repo> pulls -- Show open pulls for MUBOT_GITHUB_USER/<repo>, if MUBOT_GITHUB_USER is configured
#   imubot show [me] org-pulls [for <organization>] -- Show open pulls for all repositories of an organization, default is MUBOT_GITHUB_ORG
#
# Notes:
...
#   For example, `show me imubot pulls with awesome fix`.
#
# Author:
#   jingweno
...
module.exports = (bot) ->
  ...
  unless (url_api_base = process.env.MUBOT_GITHUB_API)?
    url_api_base = "https://api.github.com"

  bot.respond /show\s+(me\s+)?(.*)\s+pulls(\s+with\s+)?(.*)?/i, (msg)->
  ...
  bot.respond /show\s+(me\s+)?org\-pulls(\s+for\s+)?(.*)?/i, (msg) ->

    org_name = msg.match[3] || process.env.MUBOT_GITHUB_ORG

    unless (org_name)
      msg.send "No organization specified, please provide one or set MUBOT_GITHUB_ORG accordingly."
      return
