...
#
# Configuration:
#   MUBOT_YELP_CONSUMER_KEY
#   MUBOT_YELP_CONSUMER_SECRET
#   MUBOT_YELP_TOKEN
#   MUBOT_YELP_TOKEN_SECRET
#   MUBOT_YELP_SEARCH_ADDRESS
#   MUBOT_YELP_SEARCH_RADIUS
#   MUBOT_YELP_SORT
#   MUBOT_YELP_DEFAULT_SUGGESTION
#
...
#
# Commands:
#   imubot what's (to eat)? for <query> near <place>?
#   imubot what's (to eat)? for <query>?
#   imubot where should (I|we|<person>) (eat|go for) <query> near <place>?
#   imubot where should (I|we|<person>) (eat|go for) <query>?
#   imubot yelp me <query> (near <place>)?
#
# Examples:
#   imubot what's for lunch near Palo Alto
#   imubot what's to eat for lunch
#   imubot where should chris eat steak?
#   imubot where should I go for thai near Palo Alto?
#   imubot where should we eat
#   imubot yelp me thai near Palo Alto
#
#
# Author:
...
consumer_key = process.env.MUBOT_YELP_CONSUMER_KEY
consumer_secret = process.env.MUBOT_YELP_CONSUMER_SECRET
token = process.env.MUBOT_YELP_TOKEN
token_secret = process.env.MUBOT_YELP_TOKEN_SECRET

# Default search parameters
start_address = process.env.MUBOT_YELP_SEARCH_ADDRESS or "Palo Alto"
radius = process.env.MUBOT_YELP_SEARCH_RADIUS or 600
sort = process.env.MUBOT_YELP_SORT or 0
default_suggestion = process.env.MUBOT_YELP_DEFAULT_SUGGESTION or "Chipotle"
...
module.exports = (bot) ->
  bot.respond /where should \w+ (eat|go for)(.*)/i, (msg) ->
    query = msg.match[2]
    lunchMe msg, query

  bot.respond /what\'?s( to eat)? for(.*)/i, (msg) ->
    query = msg.match[2]
    lunchMe msg, query

  bot.respond /yelp me(.*)/i, (msg) ->
    query = msg.match[1]
