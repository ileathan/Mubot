...
#   None
# Configuration:
#   MUBOT_HARVEST_SUBDOMAIN
#
# Commands:
#
#   imubot remember my harvest account <email> with password <password> - Make imubot remember your Harvest credentials
#   imubot forget my harvest account - Make imubot forget your Harvest credentials again
#   imubot start harvest - Restart the last timer.
#   imubot start harvest at <project>/<task>: <notes> - Start a Harvest timer at a given project-task combination
#   imubot stop harvest [at project/task] - Stop the most recent Harvest timer or the one for the given project-task combination.
#   imubot daily harvest [of <user>] [at yyyy-mm-dd] - Show a user's Harvest timers for today (or yours, if noone is specified) or a specific day
#   imubot list harvest tasks [of <user>] - Show the Harvest project-task combinations available to a user (or you, if noone is specified)
#   imubot is harvest down/up - Check if the Harvest API is reachable.
#
# Notes:
#   All commands and command arguments are case-insenitive. If you work
#   on a project "FooBar", imubot will unterstand "foobar" as well. This
#   is also true for abbreviations, so if you don't have similary named
#   projects, "foob" will do as expected.
#
#   Some examples:
#   > imubot remember my harvest account joe@example.org with password doe
#   > imubot list harvest tasks
#   > imubot start harvest at myproject/important-task: Some notes go here.
#   > imubot start harvest at myp/imp: Some notes go here.
#   > imubot daily harvest of nickofotheruser
#
#   Full command descriptions:
#
#   imubot remember my harvest account <email> with password <password>
#     Saves your Harvest credentials to allow Hubot to track
#     time for you.
#
#   imubot forget my harvest account
#     Deletes your account credentials from Hubt's memory.
#
#  imubot start harvest
#    Examines the list of timers for today and creates a new timer with
#    the same properties as the most recent one.
#
#   imubot start harvest at <project>/<task>: <notes>
...
#
#   imubot stop harvest [at <project>/<task>]
...
#
#   imubot daily harvest [of <user>] [on yyyy-mm-dd]
...
#
#   imubot list harvest tasks [of <user>]
...
#
#   Note on MUBOT_HARVEST_SUBDOMAIN:
...
unless process.env.MUBOT_HARVEST_SUBDOMAIN
  console.log "Please set MUBOT_HARVEST_SUBDOMAIN in the environment to use the harvest plugin script."
...
check_user = (bot, msg, test_user = null) ->
  # Detect the user; if none is passed, assume the sender.
  user = null
  if test_user
    user = bot.brain.userForName(test_user)
    unless user
...
check_harvest_down = (callback) ->
  opts =
    ...
    method: "GET"
    host: "#{process.env.MUBOT_HARVEST_SUBDOMAIN}.harvestapp.com"
    port: 80
...
module.exports = (bot) ->
  ...
  cb = ->
    check_harvest_down (error) ->
      if (error)
        bot.send "broadcast", "Harvest appears to be down; exact error is: #{error}"
  setInterval(cb, 600000) # 10 Minutes in milliseconds

  # Check if Harvest is available.
  bot.respond /is harvest (down|up)/i, (msg) ->
    check_harvest_down (error) ->
      if error
  ...
  bot.respond /remember my harvest account (.+) with password (.+)/i, (msg) ->
  ...
  bot.respond /forget my harvest account/i, (msg) ->
  ...
  bot.respond /daily harvest( of (\w+))?( on (\d{4})-(\d{2})-(\d{2}))?/i, (msg) ->
    unless user = check_user(bot, msg, msg.match[2])
      return
  ...
  bot.respond /list harvest tasks( of (.+))?/i, (msg) ->
    unless user = check_user(bot, msg, msg.match[2])
      return
  ...
  bot.respond /start harvest at (.+)\/(.+): (.*)/i, (msg) ->
    unless user = check_user(bot, msg)
      return
  ...
  bot.respond /start harvest$/i, (msg) ->
    unless user = check_user(bot, msg)
      return
  ...
  bot.respond /stop harvest( at (.+)\/(.+))?/i, (msg) ->
    unless user = check_user(bot, msg)
      return
...
class HarvestService
  ...
  # account.
  constructor: (account) ->
    @base_url = "https://#{process.env.MUBOT_HARVEST_SUBDOMAIN}.harvestapp.com"
    @account = account
