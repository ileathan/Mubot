# Description:
#   allows imubot to track the cash and burn rate and displays a summary
#   of the current cash state. Also stores historical values in the
#   imubot brain so that they can be referred to later.
#
#   The s3-brain is HIGHLY recommended for keeping track of historical
#   cash values and not losing everything when imubot restarts.
#
# Dependencies:
#   None
#
...
#
# Commands:
#   imubot cash <left|on hand>: <amount> - set the cash on hand
#   imubot cash <burn rate|burn>: <amount> - set the burn rate
#   imubot cash <update|state|stats> - show the cash situation
#
# Notes:
#
# Author:
...
class Cash
  constructor: (@bot) ->
    ...
    @bot.brain.on 'loaded', =>
      if @bot.brain.data.cash
        @cache = @bot.brain.data.cash
