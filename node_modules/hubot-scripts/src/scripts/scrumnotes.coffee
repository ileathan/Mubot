...
#
# Configuration:
#   MUBOT_SCRUMNOTES_PATH - if set, folder where daily notes should be saved as json files (otherwise they just stay on bot brain)
#
# Commands:
#   imubot take scrum notes - Starts taking notes from all users in the room (records all messages starting with yesterday, today, tomorrow, sun, mon, tue, wed, thu, fri, sat, blocking)
#   imubot stop taking notes - Stops taking scrum notes (if a path is configured saves day notes to a json file)
#   imubot scrum notes - shows scrum notes taken so far
#   imubot are you taking notes? - imubot indicates if he's currently taking notes
#
# Author:
#   benjamin eidelman
...
module.exports = (bot) ->
  ...
  # rooms where imubot is hearing for notes
  hearingRooms = {}
  ...
  startHearing = ->  
    ...
    listenersCount = bot.catchAll (msg) ->
      ...
      bot.brain.data.scrumNotes ?= {};
      notes = bot.brain.data.scrumNotes[today] ?= {}
    ...
    listener = bot.listeners[listenersCount - 1]
  ...
  stopHearing = ->
    ...
    listenerIndex = -1
    for list, i in bot.listeners
      if list is listener
        listenerIndex = i
        break
    if (listenerIndex >= 0)
        setTimeout ->
          bot.listeners.splice(i, 1)
        , 0
    listener = null
  ...
  bot.respond /(?:show )?scrum notes/i, (msg) ->
  ...
  bot.respond /take scrum notes/i, (msg) ->
  ...
  bot.respond /are you taking (scrum )?notes\?/i, (msg) ->
  ...
  bot.respond /stop taking (?:scrum )?notes/i, (msg) ->
    ...
    saveTo = process.env.MUBOT_SCRUMNOTES_PATH 
