...
#
# Configuration:
#   MUBOT_UPTIMEROBOT_APIKEY
#
# Commands:
#   mubot uptimebot - Show the status of the monitored sites
#   mubot what is the uptime of the monitored sites? - Show the status of the monitored sites
#   mubot uptimebot add <url>[ as <friendlyname>] - Instructs uptime bot to monitor the <url>, optionally with a name
#   mubot start monitoring the http uptime of <url> - Instructs uptime bot to monitor the <url>
#
# Author:
#   lukesmith

module.exports = (bot) ->
  apikey = process.env.MUBOT_UPTIMEROBOT_APIKEY

  getMonitors = (msg) ->
    msg.http("http://api.uptimebot.com/getMonitors")
      .query({ apiKey: apikey, logs: 0, format: "json", noJsonCallback: 1 })
      .get() (err, res, body) ->
        if err
          msg.send "Uptime bot says: #{err}"
          return
  ...
  bot.respond /what is the uptime of the monitored sites?/i, getMonitors
  bot.respond /uptimebot/i, getMonitors

  newMonitor = (monitorUrl, monitorFriendlyName) ->
    monitorFriendlyName ?= monitorUrl
    msg.http("http://api.uptimebot.com/newMonitor")
  ...
  bot.respond /start monitoring the http uptime of (.*)$/i, (msg) ->
    newMonitor(msg.match[1], null)
  bot.respond /uptimebot add (http(s?)\:\/\/\S+)( as (.+))?/i, (msg) ->
