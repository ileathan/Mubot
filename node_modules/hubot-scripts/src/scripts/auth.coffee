...
#
# Commands:
#   imubot <user> has <role> role - Assigns a role to a user
#   imubot <user> doesn't have <role> role - Removes a role from a user
#   imubot what role does <user> have - Find out what roles are assigned to a specific user
#   imubot who has admin role - Find out who's an admin and can assign roles
#
# Notes:
#   * Call the method: bot.Auth.hasRole('<user>','<role>')
...
module.exports = (bot) ->
  ...
  class Auth
    hasRole: (name, role) ->
      user = bot.brain.userForName(name)
  ...
  bot.respond /@?(.+) (has) (["'\w: -_]+) (role)/i, (msg) ->
  ...
  bot.respond /@?(.+) (doesn't have|does not have) (["'\w: -_]+) (role)/i, (msg) ->
    name    = msg.match[1].trim()
    newRole = msg.match[3].trim().toLowerCase()
  ...
  bot.respond /(what role does|what roles does) @?(.+) (have)\?*$/i, (msg) ->
  ...
  bot.respond /who has admin role\?*$/i, (msg) ->
