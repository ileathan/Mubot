...
#
# Configuration:
#   The imubot user will need permissions to run drush on the server that it is installed on.
#   If the site aliases are to remote servers (likely the case) then the imubot user will also need
...
#
# Commands:
#   imubot drush sa - show the list of available sites ( --update-aliases will refresh this list )
#   imubot drush rq - show pending core requirements at a warning level or above
#   imubot drush <site alias> cc - Clears "all" cache for a given site alias.
#   imubot drush <site alias> pml - Lists the site modules ( "enabled" and "non-core" by default this can be changed with --disbaled or --core )
#   imubot drush <site alias> pmi <module/theme> - Show detailed info about a module or theme
#   imubot drush <site alias> uinf <user> - Display information about the user specified by uid, email, or username
#   imubot drush <site alias> ws - Show the 10 most recent watchdog messages
#   imubot drush <site alias> vget <variable name> - Show the value of a given variable
#
# Author:
#   rh0
