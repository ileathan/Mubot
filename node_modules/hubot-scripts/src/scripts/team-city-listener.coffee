...
#
#   Install web hooks, set this sucker up with Hubot, make sure you have a port configured for him, and set the MUBOT_ROOM_TO_RECEIVE_TEAM_CITY_BUILD_RESULTS
...
#
# Configuration:
#   MUBOT_ROOM_TO_RECEIVE_TEAM_CITY_BUILD_RESULTS
#
# Commands:
#   None
#
...
TextMessage = require('imubot').TextMessage

room = process.env.MUBOT_ROOM_TO_RECEIVE_TEAM_CITY_BUILD_RESULTS
unless room
  throw "Need a room to send build status messages to once we receive the web hook call"
module.exports = (bot)->
  bot.router.post "/imubot/build/", (req, res)->
    user = bot.brain.userForId 'broadcast'
    user.room = room
    user.type = 'groupchat'
    build = req.body.build

    bot.send user, "#{build.message} and ran on agent:#{build.agentName}"
    ...
    if build.buildResult == 'failure'
      ...
      bot.receive new TextMessage user, message

    bot.receive new TextMessage user, "imubot sound #{soundToPlay}"
