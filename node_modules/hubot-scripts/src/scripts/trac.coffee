...
#
# Configuration:
#   MUBOT_TRAC_URL: Base URL to Trac instance, without trailing slash eg: https://myserver.com/trac
#   MUBOT_TRAC_USER: Trac username (uses HTTP basic authentication)
#   MUBOT_TRAC_PASSWORD: Trac password
#
# Optional Configuration:
#   MUBOT_TRAC_JSONRPC: "true" to use the Trac http://trac-hacks.org/wiki/XmlRpcPlugin.
#                       Requires jsonrpc to be enabled in the plugin. Default to "true".
#   MUBOT_TRAC_SCRAPE: "true" to use HTTP scraping to pull information from Trac. 
#                      Defaults to "true".
#   MUBOT_TRAC_LINKDELAY: number of seconds to not show a link for again after it's been
#                         mentioned once. This helps to cut down on noise from the bot.
#                         Defaults to 30.
#   MUBOT_TRAC_IGNOREUSERS: Comma-seperated list of users to ignore "hearing" issues from.
...
module.exports = (bot) ->
  ...
  useJsonrpc = process.env.MUBOT_TRAC_JSONRPC || false

  # if screen scraping can be used for tickets/changesets. If both jsonrpc and scrape are off, only a link gets posted
  useScrape = process.env.MUBOT_TRAC_SCRAPE || true

  # how long (seconds) to wait between repeating the same link
  linkdelay = process.env.MUBOT_TRAC_LINKDELAY || 30

  # array of users that are ignored
  ignoredusers = (process.env.MUBOT_TRAC_IGNOREUSERS.split(',') if process.env.MUBOT_TRAC_IGNOREUSERS?) || []
  ...
  ticketRpc = (msg, ticket) ->
    jsonRpc msg, process.env.MUBOT_TRAC_URL+'/login/jsonrpc', process.env.MUBOT_TRAC_USER, process.env.MUBOT_TRAC_PASSWORD, 
      'ticket.get', [ticket],
      (err,response) ->
        if err
        ...
        url = process.env.MUBOT_TRAC_URL+"/ticket/"+ticketid
  ...
  ticketScrape = (msg, ticket) ->
    scrapeHttp  msg, process.env.MUBOT_TRAC_URL+'/ticket/'+ticket, process.env.MUBOT_TRAC_USER, process.env.MUBOT_TRAC_PASSWORD,
      ['#ticket h2.summary', 'td[headers=h_owner]', '#trac-ticket-title .status', 'td[headers=h_milestone]']
      (err, response) ->
        console.log 'scrape response', response
        url = process.env.MUBOT_TRAC_URL+"/ticket/"+ticket
  ...
  changesetScrape = (msg, revision) ->
    scrapeHttp  msg, process.env.MUBOT_TRAC_URL+'/changeset/'+revision, process.env.MUBOT_TRAC_USER, process.env.MUBOT_TRAC_PASSWORD,
      ['#content.changeset dd.message', '#content.changeset dd.author', '#content.changeset dd.time']
      (err, response) ->
        console.log 'scrape response', response
        url = process.env.MUBOT_TRAC_URL+"/changeset/"+revision
        author = response[1]
    ...
    for matched in msg.match
      ...
      if !recentlinks.contains linkid
        ...
        if useJsonrpc
          ...
          msg.send "Trac \##{ticket}: #{process.env.MUBOT_TRAC_URL}/ticket/#{ticket}"

  # listen for ticket numbers
  bot.hear /([^\w]|^)\#(\d+)(?=[^\w]|$)/ig, fetchTicket

  # listen for ticket links
  ticketUrl = new RegExp("#{process.env.MUBOT_TRAC_URL}/ticket/([0-9]+)", 'ig')
  bot.hear ticketUrl, fetchTicket
    ...
    for matched in msg.match
      ...
      if !recentlinks.contains linkid
        ...
        if useScrape
          ...
          msg.send "Trac r#{revision}: #{process.env.MUBOT_TRAC_URL}/changeset/#{revision}"

  # trigger on "r123"
  bot.hear /([^\w]|^)r(\d+)(?=[^\w]|$)/ig, handleChangeset
  # trigger on [123]
  bot.hear /([^\w]|^)\[(\d+)\](?=[^\w]|$)/ig, handleChangeset
