...
#
# Configuration:
#   MUBOT_TEAMCITY_USERNAME = <user name>
#   MUBOT_TEAMCITY_PASSWORD = <password>
#   MUBOT_TEAMCITY_HOSTNAME = <host : port>
#   MUBOT_TEAMCITY_SCHEME = <http || https> defaults to http if not set.
#
# Commands:
#   imubot show me builds - Show status of currently running builds
#   imubot tc list projects - Show all available projects
#   imubot tc list buildTypes - Show all available build types
#   imubot tc list buildTypes of <project> - Show all available build types for the specified project
#   imubot tc list builds <buildType> <number> - Show the status of the last <number> builds.  Number defaults to five.
#   imubot tc list builds of <buildType> of <project> <number>- Show the status of the last <number> builds of the specified build type of the specified project. Number can only follow the last variable, so if project is not passed, number must follow buildType directly. <number> Defaults to 5
#   imubot tc build start <buildType> - Adds a build to the queue for the specified build type
#   imubot tc build start <buildType> of <project> - Adds a build to the queue for the specified build type of the specified project
#   imubot tc build stop all <buildType> id <buildId> of <project> - Stops all currently running builds of a given buildType. Project parameter is optional. Please note that the special 'all' keyword will kill all currently running builds ignoring all further parameters. imubot tc build stop all all
#
# Author:
...
module.exports = (bot) ->
  username = process.env.MUBOT_TEAMCITY_USERNAME
  password = process.env.MUBOT_TEAMCITY_PASSWORD
  hostname = process.env.MUBOT_TEAMCITY_HOSTNAME
  scheme = process.env.MUBOT_TEAMCITY_SCHEME || "http"
  ...
  bot.respond /show me builds/i, (msg) ->
    getCurrentBuilds msg, (err, builds, msg) ->
      if typeof(builds)=='string'
        builds=JSON.parse(builds)
      if builds['count']==0
  ...
  bot.respond /tc build start (.*)/i, (msg) ->
  ...
  bot.respond /tc list (projects|buildTypes|builds) ?(.*)?/i, (msg) ->
    type = msg.match[1]
  ...
  bot.respond /tc build stop all (.*)/i, (msg) ->
    getCurrentBuilds msg, (err, builds, msg) ->
      if typeof(builds)=='string'
        builds=JSON.parse(builds)
      if builds['count']==0
