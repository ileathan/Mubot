...
#   MUBOT_URL
#   MUBOT_GITHUB_USER
#   MUBOT_GITHUB_PASSWORD
#   or
#   MUBOT_GITHUB_TOKEN
#
# Commands:
#   imubot octospy <repo> [event_type] - Start watching events for the repo, default push
#   imubot octospy stop <repo> [event_type] - Stop watching events for the repo
#   imubot octospying - Show what you're spying on
#   imubot octospy events - List the events you can watch
#
# Author:
#   rcs
...
#
# msg    - The imubot msg object, used for its http client
...
# Example:
#
# pubsub_modify(msg, 'subscribe', { github_url: 'github.com', repo: 'github/imubot', event: 'push' }, (err,resp,body) -> msg.send "aaaaaallllright.")
#
pubsub_modify = (msg, action, target, cb) ->
  ...
  data = QS.stringify {
    ...
    "hub.callback": "#{process.env.MUBOT_URL}/imubot/octospy/#{github_url}/#{event}"
  }

  # Check authentication, return error if it isn't specified
  if process.env.MUBOT_GITHUB_TOKEN
    auth = "token #{process.env.MUBOT_GITHUB_TOKEN}"
  else if (process.env.MUBOT_GITHUB_USER and process.env.MUBOT_GITHUB_PASSWORD)
    auth = 'Basic ' + new Buffer("#{process.env.MUBOT_GITHUB_USER}:#{process.env.MUBOT_GITHUB_PASSWORD}").toString('base64')
  else
...
module.exports = (bot) ->
      ...
      (err,res,body) ->
        switch res.statusCode
          when 204
            ...
            # bot.brain.data.octospy = data

            bot.logger.info "The last user unsubscribed. Removed my subscription to #{repo} #{event} events"
          else
            bot.logger.warning "Failed to unsubscribe to #{repo} #{event} events on #{github_url}: #{body} (Status Code: #{res.statusCode})"
  ...
  bot.respond /octospy ([^ ]+\/[^ ]+) ?([^ ]*)? ?([^ ]*)?/i, (msg) ->
    ...
    addListener = ->
      # Vivify!
      repos = bot.brain.data.octospy[github_url] ||= {}
      events = repos[repo] ||= {}
          ...
          switch res.statusCode
            ...
            when 401
              msg.reply """
                ...
                Specify credentials in the environment. (MUBOT_GITHUB_USERNAME,MUBOT_GITHUB_PASSWORD) or MUBOT_GITHUB_TOKEN"
            ...
            when 422
              ...
              bot.logger.warning "#{JSON.stringify body}"
    else
      addListener()
  ...
  bot.router.post '/imubot/octospy/:github/:event', (req, res) ->
    req.body = req.body || {}
