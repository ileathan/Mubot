...
#
# Configuration:
#   MUBOT_YOUTRACK_HOSTNAME = <host:port>
#   MUBOT_YOUTRACK_USERNAME = <user name>
#   MUBOT_YOUTRACK_PASSWORD = <password>
#   MUBOT_YOUTRACK_URL      = <scheme>://<username>:<password>@<host:port>/<basepath>
#
# Commands:
...
yt_url   = process.env.MUBOT_YOUTRACK_URL
host     = process.env.MUBOT_YOUTRACK_HOSTNAME
username = process.env.MUBOT_YOUTRACK_USERNAME
password = process.env.MUBOT_YOUTRACK_PASSWORD
...
module.exports = (bot) ->
  ...
  bot.hear /what (are )?my issues/i, (msg) ->
    msg.send "@#{msg.message.user.name}, you have many issues.  Shall I enumerate them?  I think not."   if Math.random() < .2

  bot.hear /what ((are )?my issues|am I (doing|working on|assigned))/i, (msg) ->
    return unless youTalkinToMe msg, bot
  ...
  bot.hear /what (can|might|should)\s+(I|we)\s+(do|work on)/i, (msg) ->
    return unless youTalkinToMe msg, bot
  ...
  bot.hear hashTagYoutrackIssueNumber, (msg) ->
    issueId = msg.match[1]
