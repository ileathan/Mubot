...
#
# Configuration:
#   MUBOT_MEMEGENERATOR_USERNAME -- Username on MemeGenerator.net
#   MUBOT_MEMEGENERATOR_PASSWORD -- Password for account on MemeGenerator.net
#
# Commands:
...
generateMeme = (msg, generatorID, imageID, msg1, msg2) ->
  if not process.env.MUBOT_MEMEGENERATOR_USERNAME
    msg.send "Error: You must specify your MemeGenerator.net username"
  if not process.env.MUBOT_MEMEGENERATOR_PASSWORD
    msg.send "Error: You must specify your MemeGenerator.net pasword"
  if not (process.env.MUBOT_MEMEGENERATOR_USERNAME and process.env.MUBOT_MEMEGENERATOR_PASSWORD)
    return
  url = "http://version1.api.memegenerator.net/Instance_Create?username=#{ process.env.MUBOT_MEMEGENERATOR_USERNAME }&password=#{ process.env.MUBOT_MEMEGENERATOR_PASSWORD }&languageCode=en&generatorID=#{ generatorID }&imageID=#{ imageID }&text0=#{ encodeURIComponent(msg1) }&text1=#{ encodeURIComponent(msg2) }"
  msg.http(url).get() (err, res, body) ->
    resp = JSON.parse(body)
