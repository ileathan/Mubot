...
module.exports = (bot) ->
  ...
  getCountdownMsg = (countdownKey) ->
    now = new Date()
    eventTime = new Date(bot.brain.data.countdown[countdownKey].date)
    gap = eventTime.getTime() - now.getTime()
    gap =  Math.floor(gap / (1000 * 60 * 60 * 24));
    "Only #{gap} days remaining till #{countdownKey}!"

  bot.hear /countdown set (\w+) (.*)/i, (msg) ->
    bot.brain.data.countdown or= {}
    ...
    try 
      ...
      bot.brain.data.countdown[countdownKey] = {"date" : date.toDateString()} 
      msg.send "Countdown set for #{countdownKey} at #{date.toDateString()}"
      msg.send getCountdownMsg(countdownKey)  if bot.brain.data.countdown.hasOwnProperty(countdownKey)
    catch error
        console.log(error.message)
        msg.send "Invalid date passed!"

  bot.hear /countdown list/i, (msg) ->
    countdowns = bot.brain.data.countdown;
  ...
  bot.hear /(countdown)( for)? (.*)/, (msg) ->
    countdownKey = msg.match[3]
    countdowns = bot.brain.data.countdown;
    msg.send getCountdownMsg(countdownKey)  if countdowns.hasOwnProperty(countdownKey)

  bot.hear /countdown clear/i, (msg) ->
    bot.brain.data.countdown = {}
    msg.send "Countdowns cleared"

  bot.hear /countdown delete (.*)/i, (msg) ->
    countdownKey = msg.match[1]
    if bot.brain.data.countdown.hasOwnProperty(countdownKey)
      delete bot.brain.data.countdown[countdownKey]
      msg.send "Countdown for #{countdownKey} deleted."
    else
      msg.send "Countdown for #{countdownKey} does not exist!"

  bot.hear /countdown set$|countdown help/i, (msg) ->
