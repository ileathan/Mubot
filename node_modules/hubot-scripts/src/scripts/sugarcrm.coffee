...
# Configuration:
#   MUBOT_SUGARCRM_URL
#   MUBOT_SUGARCRM_USERNAME
#   MUBOT_SUGARCRM_PASSWORD
#   MUBOT_SUGARCRM_REPORT_ID
#   MUBOT_SUGARCRM_REPORT_FIELD
#
# Commands:
#   imubot pipeline me - Gives the total amount in sales pipeline
#
# Author:
#   skalnik

module.exports = (bot) ->
  bot.respond /pipeline( me)?/i, (msg) ->
    url          = process.env.MUBOT_SUGARCRM_URL
    username     = process.env.MUBOT_SUGARCRM_USERNAME
    password     = process.env.MUBOT_SUGARCRM_PASSWORD
    reportID     = process.env.MUBOT_SUGARCRM_REPORT_ID
    reportField  = process.env.MUBOT_SUGARCRM_REPORT_FIELD

    unless url
      msg.send "SugarCRM URL isn't set."
      msg.send "Please set the MUBOT_SUGARCRM_URL environment variable without prefixed HTTP or trailing slash"
      return

    unless username
      msg.send "SugarCRM username isn't set."
      msg.send "Please set the MUBOT_SUGARCRM_USERNAME environment variable"
      return

    unless password
      msg.send "SugarCRM password isn't set."
      msg.send "Please set the MUBOT_SUGARCRM_PASSWORD environment variable"
      return

    unless reportID
      msg.send "SugarCRM report ID is not set."
      msg.send "Please set the MUBOT_SUGARCRM_REPORT_ID to the report ID of your pipeline report"
      return

    unless reportField
      msg.send "SugarCRM report field is not set."
      msg.send "Please set the MUBOT_SUGARCRM_REPORT_FIELD to the field of the report that should be totaled"
      return
    ...
    sugarCRMLogin msg, url, username, password, (session) ->
      ...
      sugarCRMCall msg, url, 'get_report_entries', data, (err, res, body) ->
        ...
        if fieldID == -1
          ...
          msg.send "Please double check MUBOT_SUGARCRM_REPORT_FIELD."
          return
