...
module.exports = (bot) ->
  bot.respond /show\s+(me\s+)?stories(\s+for\s+)?(.*)/i, (msg)->
    token = process.env.MUBOT_PIVOTAL_TOKEN
    project_name = msg.match[3]
    if project_name == ""
      project_name = RegExp(process.env.MUBOT_PIVOTAL_PROJECT, "i")
    else
  ...
  bot.respond /(pivotal story)? (.*)/i, (msg)->
    token = process.env.MUBOT_PIVOTAL_TOKEN
    project_id = process.env.MUBOT_PIVOTAL_PROJECT
