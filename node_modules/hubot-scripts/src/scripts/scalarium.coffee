...
#
# Configuration:
#   MUBOT_SCALARIUM_TOKEN: for authenticating the requests (see https://manage.scalarium.com/users/<user-id>/api)
#
# Commands:
#   imubot scalarium list (apps|clouds) - Lists all applications/clouds on scalarium
#   imubot scalarium deploy <app id> <message> - Triggers an deployment of *app id* with *message*
#
# Author:
#   nesQuick

https = require 'https'
token = process.env.MUBOT_SCALARIUM_TOKEN
...
module.exports = (bot)->
  ...
  bot.respond /scalarium list apps/i, (message)->
    client.getApplications (apps) ->
      for app in apps
        message.send "#{app.name} - #{app.id}"

  bot.respond /scalarium list clouds/i, (message)->
    client.getClouds (clouds) ->
      for cloud in clouds
        message.send "#{cloud.name} - #{cloud.id}"

  bot.respond /scalarium deploy ([0-9a-f]+) (.+)$/i, (message)->
