...
//
// Configuration:
//   MUBOT_EL_IDEAS_URL  - The Etherpad Lite main URL, e.g. "http://my-etherpad.net/"
//   MUBOT_EL_IDEAS_KEY  - The Etherpad Lite API key (see https://github.com/ether/etherpad-lite/wiki/HTTP-API)
//   MUBOT_EL_IDEAS_PAD  - The Etherpad Lite pad ID, e.g. "my-pad"
//   MUBOT_EL_IDEAS_SEPARATOR  - (optional) A regex to find lines separating ideas, default: "^\s?(?:- )+\-?$"
//
// Commands:
//   imubot list ideas  - List 10 latest ideas
//   imubot list <count> ideas  - List <count> latest ideas
//   imubot list ideas with <filter>  - List all ideas matching <filter>
//   imubot show idea <idea_number>  - Show an idea
//   imubot show idea  - Show the last idea
//   imubot count ideas  - Show the ideas count
//
// Notes:
...
var ERROR_ENV = 'MUBOT_EL_IDEAS_URL, MUBOT_EL_IDEAS_KEY and/or MUBOT_EL_IDEAS_PAD are not set.';
...
function parseIdeas(rawText, cb) {
  ...
  separator = process.env.MUBOT_EL_IDEAS_SEPARATOR;
  if (!separator) {
    separator = '^\s?(?:- )+\-?$';
  }
...
function getIdeasList(ideas, cb) {
  var result = 'Select an idea with “imubot show idea <number>”:\n';
  var ideaNum;
...
function getPadUrl() {
  var url = process.env.MUBOT_EL_IDEAS_URL;
  var key = process.env.MUBOT_EL_IDEAS_KEY;
  var pad = process.env.MUBOT_EL_IDEAS_PAD;
  if (!pad || !key || !pad) {
    return null;
  }
...
module.exports = function(bot) {
  ...
  bot.respond(/list\s+(all\s+|[0-9]+\s+)?ideas$/i, function(msg){
  ...
  bot.respond(/list\s+ideas\s+with\s+(.+$)/i, function(msg){
  ...
  bot.respond(/show\s+idea(\s+[0-9]+)?/i, function(msg){
  ...
  bot.respond(/count\s+ideas/i, function(msg){
    getIdeas(msg, function(err, ideas){
      if (err) return msg.send(err);
