...
#   can be configured from the enviornment with `$MUBOT_MAKE_ME_URL`,
#   `$MUBOT_MAKE_ME_USER` and `$MUBOT_MAKE_ME_PASS`
#
# Commands:
#   imubot 3d me <url..url_n> [options] - 3D Print the URLs
#   imubot 3d? - Show some help
#
# Notes:
#   None
#
...
makeServer = process.env.MUBOT_MAKE_ME_URL or 'http://localhost:9292'
[authUser, authPass] = [process.env.MUBOT_MAKE_ME_USER or 'imubot', process.env.MUBOT_MAKE_ME_PASS or 'isalive']
auth64 = (new Buffer("#{authUser}:#{authPass}")).toString("base64")

module.exports = (bot) ->
  bot.respond /3d\??$/i, (msg) ->
    response = """#{bot.name} 3d me [STL URLs] [[options]] - prints an STL file
...
#{bot.name} 3d snapshot - takes a picture and tells you the locked status
#{bot.name} 3d unlock - unlocks the printer after you clean up

Only 1 print at a time is allowed, and you are required to tell
#{bot.name} after you've cleaned your print off.

The web frontend is at #{makeServer}, and
the most current log is always available at #{makeServer}/log"""
    msg.send response

  bot.respond /3d (snapshot|status)/i, (msg) ->
    locked_msg = "unlocked"
  ...
  bot.respond /3d unlock( me)?/i, (msg) ->
    msg.http(makeServer + "/lock")
      .header("Authorization", "Basic #{auth64}")
  ...
  bot.respond /(3d|make)( me)?( a)? (http[^\s]+)\s*(.*)/i, (msg) ->
    things = [msg.match[4]]
    count = 1
    scale = 1.0
    supports = false
