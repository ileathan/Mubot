...
#   2. Add <MUBOT_URL>:<PORT>/imubot/gh-pull-requests?room=<room>[&type=<type>] url hook via API:
...
#
# URLS:
#   POST /imubot/gh-pull-requests?room=<room>[&type=<type]
#
# Authors:
#   spajus
#
# Notes:
#   Room information can be obtained by imubot-script: room-info.coffee
...
module.exports = (bot) ->
  bot.router.post "/imubot/gh-pull-requests", (req, res) ->
    ...
    try
      announcePullRequest data, (what) ->
        bot.messageRoom room, what
    catch error
      bot.messageRoom room, "Whoa, I got an error: #{error}"
