...
#
#   Will ignore users set in MUBOT_JIRA_ISSUES_IGNORE_USERS (by default, JIRA and GitHub).
#
# Dependencies:
#   None
#
# Configuration:
#   MUBOT_JIRA_URL (format: "https://jira-domain.com:9090")
#   MUBOT_JIRA_IGNORECASE (optional; default is "true")
#   MUBOT_JIRA_USERNAME (optional)
#   MUBOT_JIRA_PASSWORD (optional)
#   MUBOT_JIRA_ISSUES_IGNORE_USERS (optional, format: "user1|user2", default is "jira|github")
#
# Commands:
# 
# Author:
...
module.exports = (bot) ->
  ...
  # previous behavior.
  jiraUrl = process.env.MUBOT_JIRA_URL || "https://#{process.env.MUBOT_JIRA_DOMAIN}"
  jiraUsername = process.env.MUBOT_JIRA_USERNAME
  jiraPassword = process.env.MUBOT_JIRA_PASSWORD
  
  if jiraUsername != undefined && jiraUsername.length > 0
    auth = "#{jiraUsername}:#{jiraPassword}"

  jiraIgnoreUsers = process.env.MUBOT_JIRA_ISSUES_IGNORE_USERS
  if jiraIgnoreUsers == undefined
    jiraIgnoreUsers = "jira|github"

  bot.http(jiraUrl + "/rest/api/2/project")
    .auth(auth)
    .get() (err, res, body) ->
      ...
      ic = process.env.MUBOT_JIRA_IGNORECASE
      if ic == undefined || ic == "true"
        jiraPattern += "i"

      bot.hear eval(jiraPattern), (msg) ->
        ...
        for i in msg.match
          ...
          if cache.length == 0 or (item for item in cache when item.issue is issue).length == 0
            bot.http(jiraUrl + "/rest/api/2/issue/" + issue)
              .auth(auth)
              .get() (err, res, body) ->
                try
                  json = JSON.parse(body)
