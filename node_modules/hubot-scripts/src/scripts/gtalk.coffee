# Description:
#   Send gtalk messages to channels via imubot
#
# Dependencies:
#   "node-xmpp": "0.3.2"
#
  ...
  message_handler = (stanza) ->
    if stanza.is('message') and stanza.attrs.type != 'error'
      ...
      if message_body_element
        bot.send { room: room }, "From: " + stanza.attrs.from + " " + message_body_element.getText()

  online_handler = ->
    xmpp_client.send(new xmpp.Element('presence', {}).c('show').t('chat').up().c('status').t(presence));
    if (online_announce)
      bot.send { room: room }, online_announce

  error_handler = (error) ->
    bot.send { room: room }, "Caught an error " + error
