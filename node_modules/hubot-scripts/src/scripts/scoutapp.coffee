...
# Notes:
#   To use:
#     setup http://hostname/imubot/scoutapp/ROOMNUMBER as
#     your notification webook. If on Heroku lookup the hostname where
#     the imubot server is running. (e.g. my-imubot.herokuapp.com)
#
# Author:
#   gstark

module.exports = (bot) ->
  bot.router.post '/imubot/scoutapp/:room', (req, res) ->
    ...
    bot.messageRoom room, "Scout #{data.severity} - #{data.server_name} on host #{data.server_hostname} #{data.lifecycle}ed - #{data.plugin_name} - #{data.title} - Current value #{data.metric_name}=#{data.metric_value} - Details: #{data.url}"
    bot.messageRoom room, data.sparkline_url if data.sparkline_url
