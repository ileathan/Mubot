...
#
#   MUBOT_TWEETER_ACCOUNTS should be a string that parses to a JSON
#   object that contains access_token and access_token_secret for each
#   twitter screen name you want to allow people to use.
#
#   For example:
#   {
#     "imubot" : { "access_token" : "", "access_token_secret" : ""},
#     "github" : { "access_token" : "", "access_token_secret" : ""}
#   }
#
#   This also can be installed as an npm package: imubot-tweeter
#
# Commands:
#   imubot tweet@<screen_name> <update> - posts the update to twitter as <screen_name>
#
# Dependencies:
#   "twit": "1.1.8"
#
...
#   MUBOT_TWEETER_ACCOUNTS
#
# Author:
#   jhubert
#
# Repository:
#   https://github.com/jhubert/imubot-tweeter

Twit = require "twit"
config =
  consumer_key: process.env.MUBOT_TWITTER_CONSUMER_KEY
  consumer_secret: process.env.MUBOT_TWITTER_CONSUMER_SECRET
  accounts_json: process.env.MUBOT_TWEETER_ACCOUNTS

unless config.consumer_key
  console.log "Please set the MUBOT_TWITTER_CONSUMER_KEY environment variable."
unless config.consumer_secret
  console.log "Please set the MUBOT_TWITTER_CONSUMER_SECRET environment variable."
unless config.accounts_json
  console.log "Please set the MUBOT_TWEETER_ACCOUNTS environment variable."

config.accounts = JSON.parse(config.accounts_json || "{}")

module.exports = (bot) ->
  bot.respond /tweet\@([^\s]+)$/i, (msg) ->
    msg.reply "You can't very well tweet an empty status, can ya?"
    return

  bot.respond /tweet\@([^\s]+)\s(.+)$/i, (msg) ->
