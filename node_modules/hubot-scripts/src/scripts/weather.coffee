...
#
# Configuration:
#   MUBOT_WEATHER_CELSIUS - Display in celsius
#   MUBOT_FORECAST_API_KEY - Forecast.io API Key
#
# Commands:
#   imubot weather <city> - Get the weather for a location.
#   imubot forecast <city> - Get the 3 day forecast for a location.
#
# Author:
#   markstory
#   mbmccormick
...
forecastIoUrl = 'https://api.forecast.io/forecast/' + process.env.MUBOT_FORECAST_API_KEY + '/'
...
lookupWeather = (msg, coords, err) ->
  ...
  return msg.send "You need to set env.MUBOT_FORECAST_API_KEY to get weather data" if not env.MUBOT_FORECAST_API_KEY
...
module.exports = (bot) ->
  ...
  bot.respond /weather(?: me|for|in)?\s(.*)/i, (msg) ->
    location = msg.match[1]
    lookupAddress(msg, location, lookupWeather)

  bot.respond /forecast(?: me|for|in)?\s(.*)/i, (msg) ->
    location = msg.match[1]
