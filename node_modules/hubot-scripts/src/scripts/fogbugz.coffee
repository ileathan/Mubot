...
# Notes:
#   
#   curl 'https://MUBOT_FOGBUGZ_HOST/api.asp' -F'cmd=logon' # -F'email=EMAIL' -F'password=PASSWORD'
...
module.exports = (bot) ->
  if env.MUBOT_FOGBUGZ_HOST and env.MUBOT_FOGBUGZ_TOKEN
    bot.hear /(?:bugz?|case) (\d+)/i, (msg) ->
      msg.http("https://#{env.MUBOT_FOGBUGZ_HOST}/api.asp")
        .query
          cmd: "search"
          token: env.MUBOT_FOGBUGZ_TOKEN
          q: msg.match[1]
        ...
        .get() (err, res, body) ->
          ...
          (new Parser()).parseString body, (err,json) ->
            if json.response.error
              ...
              if bug
                msg.send "https://#{env.MUBOT_FOGBUGZ_HOST}/?#{bug.ixBug[0]}"
                details = [
