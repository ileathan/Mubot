...
#
# Configuration:
#   MUBOT_SPINNER_SECRET_KEY
#   MUBOT_SPINNER_ACCESS_KEY
#
# Commands:
...
secret_key = env.MUBOT_SPINNER_SECRET_KEY
access_key = env.MUBOT_SPINNER_ACCESS_KEY
...
spun = (msg, spinner = null, bot_brain = null) ->
	timestamp = gmdatenow()
			...
			if err
				...
				if body.response is 'error'
					...
					if spinner
						...
						if bot_brain
							if not bot_brain.data.spin
								bot_brain.data.spin = []
							bot_brain.data.spin[msg.message.user.reply_to] = body.message
						msg.send body.message
...
module.exports = (bot) ->
	hear_and_respond bot, 'spin me ([\\s\\S]*)', (msg) ->
		msg.send spin msg.match[1]

	hear_and_respond bot, 'spun me ([\\s\\S]*)', (msg) ->
		spun(msg, null, bot.brain)

	hear_and_respond bot, 'spun and spin me ([\\s\\S]*)', (msg) ->
		spun(msg, spin)

	hear_and_respond bot, 'spin the last spun$', (msg) ->
		# get user centric input from memory
		if @bot.brain.data.spin[msg.message.user.reply_to]
			msg.send spin bot.brain.data.spin[msg.message.user.reply_to]
		else
