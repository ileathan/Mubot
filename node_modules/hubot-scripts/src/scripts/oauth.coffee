...
handle_authorization = (bot, msg) ->
  callback = (url) ->
    message = if url then url else "Error on retrieving url. See logs for more details."
    msg.send message
  new scribe.OAuth(bot.brain.data, msg.match[1].toLowerCase(), services).get_authorization_url(callback)

handle_verification = (bot, msg) ->
  api = msg.match[1].toLowerCase()
  callback = (response) ->
    if response
      if not bot.brain.data.oauth_user
        bot.brain.data.oauth_user = []
      ...
      bot.brain.data.oauth_user[api] = msg.message.user.reply_to
      message = "Verification done"
    else 
      message = "Error on verification process. See logs for more details."
    msg.send message
  new scribe.OAuth(bot.brain.data, api, services).set_verification_code(msg.match[2], callback)

handle_refresh = (bot, msg) ->
  service = new scribe.OAuth(bot.brain.data, msg.match[1].toLowerCase(), services)
...
hear_and_respond = (bot, regex, callback) ->
  bot.hear eval('/^'+regex+'/i'), callback
  bot.respond eval('/'+regex+'/i'), callback

module.exports = (bot) ->
  hear_and_respond bot, 'get ([0-9a-zA-Z].*) authorization url$', (msg) ->
    handle_authorization bot, msg

  hear_and_respond bot, 'set ([0-9a-zA-Z].*) verifier (.*)', (msg) ->
    handle_verification bot, msg

  hear_and_respond bot, 'refresh ([0-9a-zA-Z].*) token$', (msg) ->
    handle_refresh bot, msg

  hear_and_respond bot, 'get ([0-9a-zA-Z].*) request token$', (msg) ->
    if token = new scribe.OAuth(bot.brain.data, msg.match[1].toLowerCase(), services).get_request_token()
      message = "Request token: " + token.getToken()
    else
      message = "Request token not found"
    msg.send message

  hear_and_respond bot, 'get ([0-9a-zA-Z].*) access token$', (msg) ->
    if token = new scribe.OAuth(bot.brain.data, msg.match[1].toLowerCase(), services).get_access_token()
      message = "Access token: " + token.getToken()
    else
      message = "Access token not found"
    msg.send message

  hear_and_respond bot, 'get ([0-9a-zA-Z].*) verifier$', (msg) ->
    if token = new scribe.OAuth(bot.brain.data, msg.match[1].toLowerCase(), services).get_verifier()
      message = "Verifier: " + token.getValue()
    else
      message = "Verifier not found"
    msg.send message

  hear_and_respond bot, 'remove ([0-9a-zA-Z].*) authorization$', (msg) ->
    api = msg.match[1].toLowerCase()
    if bot.brain.data.oauth_user and bot.brain.data.oauth_user[api] == msg.message.user.reply_to
      message = "Authorization removed: " + new scribe.OAuth(bot.brain.data, api, services).remove_authorization()
    else
      message = "Authorization can be removed by original verifier only: " + bot.brain.data.oauth_user[api]
    msg.send message

  hear_and_respond bot, 'set ([0-9a-zA-Z].*) access token (.*)', (msg) ->
    api = msg.match[1].toLowerCase()
    if new scribe.OAuth(bot.brain.data, api, services).set_access_token_code(msg.match[2])
      bot.brain.data.oauth_user[api] = msg.message.user.reply_to
