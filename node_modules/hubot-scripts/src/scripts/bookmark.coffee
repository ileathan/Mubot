# Description:
#   Manage your links and bookmarks. Links get stored in the bot brain while
#   bookmarks get stored at delicious.com. Also keeps a history of all URLs in
#   the "urls" section of the bot brain. 
#
# Dependencies:
#   "xml2js": "0.1.14"
#
...
#
# Commands:
#   imubot bookmark <url> as <description> - add a url to your delicious feed
#   imubot link <url> as <description> - add a url to the bot brain
#   imubot link me for <description> - find a link by description
#   imubot list bookmarks - get a list of the 15 most recent bookmarks
#   imubot list links - List all of the links that are being tracked
#   imubot feed me - get the URL to subscribe to your bookmark rss
#
# Author
#   mm53bar

module.exports = (bot) ->
  bot.respond /feed me/i, (msg) ->
    msg.reply "You can subscribe to the delicious feed at https://delicious.com/v2/rss/#{process.env.DELICIOUS_USER}"

  bot.respond /list bookmarks/i, (msg) ->
  ...
  bot.respond /bookmark (http(s?)\:\/\/\S+) as (.+)/i, (msg) ->
    delicious = new Delicious msg, process.env.DELICIOUS_USER, process.env.DELICIOUS_PASSWORD
    url = msg.match[1]
  ...
  bot.respond /link (http(s?)\:\/\/\S+) as (.+)/i, (msg) ->
    url = msg.match[1]
    description = msg.match[3]    
    bookmark = new Bookmark url, description
    link = new Link bot
    ...
    link.add bookmark, (err, message) ->
      if err?
        ...
        msg.reply "I've stuck that link into my bot brain." 
        
  bot.respond /link me for (.+)/i, (msg) ->
...
class Link
  constructor: (bot) ->
    bot.brain.data.links ?= []
    @links_ = bot.brain.data.links
