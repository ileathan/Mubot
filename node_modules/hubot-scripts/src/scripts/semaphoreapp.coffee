...
#           one of the Campfire room ids configured in MUBOT_CAMPFIRE_ROOMS.
#
#     Examples:
#
#       "The Internal Room"
...
#
# Commands:
#   imubot semaphoreapp status [<project> [<branch>]] - Reports build status for projects' branches
#
# URLs:
#   POST /imubot/semaphoreapp
#     First, read http://docs.semaphoreapp.com/webhooks, then your URL to
#     receive the payload would be "<MUBOT_URL>:<PORT>/imubot/semaphoreapp"
#     or if you deployed Hubot onto Heroku: "<HEROKU_URL>/imubot/semaphoreapp".
#
# Author:
#   exalted

module.exports = (bot) ->
  if process.env.MUBOT_SEMAPHOREAPP_TRIGGER
    trigger = process.env.MUBOT_SEMAPHOREAPP_TRIGGER.split(',').join('|')

  bot.respond new RegExp("(?:semaphoreapp|#{trigger})\\s+status(?:\\s+(\\S+)(?:\\s+(\\S+))?)?\\s*$", "i"), (msg) ->
  ...
  bot.router.post "/imubot/semaphoreapp", (req, res) ->
    unless process.env.MUBOT_SEMAPHOREAPP_NOTIFY_RULES
      message = "semaphoreapp hook warning: MUBOT_SEMAPHOREAPP_NOTIFY_RULES is empty."
    ...
    rules = process.env.MUBOT_SEMAPHOREAPP_NOTIFY_RULES.split(',')
    for rule in (x.split(':') for x in rules)
      ...
      if branchRegExp.test(payload.branch_name)
        bot.messageRoom room, statusMessage payload
...
class SemaphoreApp
  ...
  getListOfAllProjects: (callback) ->
    unless process.env.MUBOT_SEMAPHOREAPP_AUTH_TOKEN
      @msg.reply "I am not allowed to access Semaphore APIs, sorry. :cry:"
      return

    @msg.bot.http("https://semaphoreapp.com/api/v1/projects")
      .query(auth_token: "#{process.env.MUBOT_SEMAPHOREAPP_AUTH_TOKEN}")
      .get() (err, res, body) ->
        try
