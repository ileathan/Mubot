...
# Configuration:
#   MUBOT_ZENDESK_USER
#   MUBOT_ZENDESK_PASSWORD
#   MUBOT_ZENDESK_SUBDOMAIN
#
# Commands:
#   imubot (all) tickets - returns the total count of all unsolved tickets. The 'all' keyword is optional.
#   imubot new tickets - returns the count of all new (unassigned) tickets
#   imubot open tickets - returns the count of all open tickets
#   imubot on hold tickets - returns the count of all on hold tickets
#   imubot escalated tickets - returns a count of tickets with escalated tag that are open or pending
#   imubot pending tickets - returns a count of tickets that are pending
#   imubot list (all) tickets - returns a list of all unsolved tickets. The 'all' keyword is optional.
#   imubot list new tickets - returns a list of all new tickets
#   imubot list open tickets - returns a list of all open tickets
#   imubot list pending tickets - returns a list of pending tickets
#   imubot list escalated tickets - returns a list of escalated tickets
#   imubot ticket <ID> - returns information about the specified ticket

sys = require 'sys' # Used for debugging
tickets_url = "https://#{process.env.MUBOT_ZENDESK_SUBDOMAIN}.zendesk.com/tickets"
queries =
...
zendesk_request = (msg, url, handler) ->
  zendesk_user = "#{process.env.MUBOT_ZENDESK_USER}"
  zendesk_password = "#{process.env.MUBOT_ZENDESK_PASSWORD}"
  auth = new Buffer("#{zendesk_user}:#{zendesk_password}").toString('base64')
  zendesk_url = "https://#{process.env.MUBOT_ZENDESK_SUBDOMAIN}.zendesk.com/api/v2"
