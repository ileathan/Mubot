# Description:
#   Allows imubot to update itself using git pull and npm update.
#   If updates are downloaded you'll need to restart imubot, for example using "imubot die" (restart using a watcher like forever.js).
#
# Dependencies:
#   None
#
...
#
# Commands:
#   imubot update - Performs a git pull and npm udate.
#   imubot pending update - Informs if there are pending updates (imubot needs a restart)
#
# Author:
#   benjamine
...
module.exports = (bot) ->
    ...
    bot.respond /pending updates?\??$/i, (msg) ->
        if downloaded_updates
            msg.send "I have some pending updates, KILL ME PLEASE! (hint: imubot die)"
        else
            msg.send "I'm up-to-date!"

    bot.respond /update( yourself)?$/i, (msg) ->
        changes = false
        try
            msg.send "git pull..."
                ...
                try
                    ...
                    child_process.exec 'npm update', (error, stdout, stderr) ->
                        ...
                        if changes
                            ...
                            msg.send "I downloaded some updates, KILL ME PLEASE! (hint: imubot die)"
                        else
                            if downloaded_updates
                                msg.send "I have some pending updates, KILL ME PLEASE! (hint: imubot die)"
                            else
