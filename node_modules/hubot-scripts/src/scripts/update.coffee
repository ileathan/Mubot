# Description:
#   Allows mubot to update itself using git pull and npm update.
#   If updates are downloaded you'll need to restart mubot, for example using "mubot die" (restart using a watcher like forever.js).
#
# Dependencies:
#   None
#
...
#
# Commands:
#   mubot update - Performs a git pull and npm udate.
#   mubot pending update - Informs if there are pending updates (mubot needs a restart)
#
# Author:
#   benjamine
...
module.exports = (bot) ->
    ...
    bot.respond /pending updates?\??$/i, (msg) ->
        if downloaded_updates
            msg.send "I have some pending updates, KILL ME PLEASE! (hint: mubot die)"
        else
                ...
                try
                    ...
                    child_process.exec 'npm update', (error, stdout, stderr) ->
                        ...
                        if changes
                            ...
                            msg.send "I downloaded some updates, KILL ME PLEASE! (hint: mubot die)"
                        else
                            if downloaded_updates
                                msg.send "I have some pending updates, KILL ME PLEASE! (hint: mubot die)"
                            else
