...
#
# Commands:
#   imubot chess me - Creates a new game between yourself and another person in the room
#   imubot chess status - Gets the current state of the board
#   imubot chess move <to> - Moves a piece to the coordinate position using standard chess notation
#
# Author:
#   thallium205
#
...
module.exports = (bot) ->
  bot.respond /chess me$/i, (msg) ->
    bot.brain.data.chess[msg.message.room] = Chess.create()
    boardToFen bot.brain.data.chess[msg.message.room].getStatus(), (status, fen) ->
  ...
  bot.respond /chess status/i, (msg) ->
    try
      boardToFen bot.brain.data.chess[msg.message.room].getStatus(), (status, fen) ->
        if status
          msg.send status
  ...
  bot.respond /chess move (.*)/i, (msg) ->
    try
      bot.brain.data.chess[msg.message.room].move msg.match[1]
      boardToFen bot.brain.data.chess[msg.message.room].getStatus(), (status, fen) ->
       if status
          msg.send status
        msg.send 'http://webchess.freehostia.com/diag/chessdiag.php?fen=' + encodeURIComponent(fen) + '&size=large&coord=yes&cap=yes&stm=yes&fb=no&theme=classic&format=auto&color1=E3CEAA&color2=635147&color3=000000&.png'
