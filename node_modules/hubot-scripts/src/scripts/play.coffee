...
#
# Commands:
#   imubot play - Plays music.
#   imubot play next - Plays the next song.
#   imubot play previous - Plays the previous song.
#   imubot what's playing - Returns the currently-played song.
#   imubot what's next - Returns next song in the queue.
#   imubot I want this song - Returns a download link for the current song.
#   imubot I want this album - Returns a download link for the current album.
#   imubot play <artist> - Queue up ten songs from a given artist.
#   imubot play <album> - Queue up an entire album.
#   imubot play <song> - Queue up a particular song. This grabs the first song by playcount.
#   imubot play <something> right [fucking] now - Play this shit right now.
#   imubot where's play - Gives you the URL to the web app.
#   imubot volume? - Returns the current volume level.
#   imubot volume [0-100] - Sets the volume.
#   imubot be quiet - Mute play.
#   imubot say <message> - `say` your message over your speakers.
#   imubot clear play - Clears the Play queue.
#
# Author:
#   holman

URL = "#{process.env.MUBOT_PLAY_URL}"

authedRequest = (message, path, action, options, callback) ->
  message.http("#{URL}#{path}")
    .query(login: message.message.user.githubLogin, token: "#{process.env.MUBOT_PLAY_TOKEN}")
    .header('Content-Length', 0)
    .query(options)[action]() (err, res, body) ->
      callback(err,res,body)

module.exports = (bot) ->
  bot.respond /where'?s play/i, (message) ->
    message.finish()
    authedRequest message, '/stream_url', 'get', {}, (err, res, body) ->
      message.send("play's at #{URL} and you can stream from #{body}")

  bot.respond /what'?s playing/i, (message) ->
  ...
  bot.respond /what'?s next/i, (message) ->
  ...
  bot.respond /play next/i, (message) ->
    message.finish()
  ...
  bot.respond /app volume\?/i, (message) ->
    message.finish()
  ...
  bot.respond /pause|(pause play)|(play pause)/i, (message) ->
    message.finish()
    params = {volume: 0}
    authedRequest message, '/system-volume', 'put', params, (err, res, body) ->
      message.send("The office is now quiet. (But the stream lives on!)")

  bot.respond /(unpause play)|(play unpause)/i, (message) ->
    message.finish()
    params = {volume: 50}
    authedRequest message, '/system-volume', 'put', params, (err, res, body) ->
      message.send("The office is now rockin' at half-volume.")

  bot.respond /start play/i, (message) ->
    message.finish()
    authedRequest message, '/play', 'put', {}, (err, res, body) ->
      json = JSON.parse(body)
      message.send("Okay! :)")

  bot.respond /stop play/i, (message) ->
    message.finish()
  ...
  bot.respond /I (like|star|love|dig) this( song)?/i, (message) ->
