...
#
# Commands:
#   imubot sprintly [product_id] [status] [limit] - list items in status (default status is in-progress, other values: backlog, completed, accepted; default limit is 20)
#   imubot sprintly [product_id] mine - list items assigned to me
#   imubot sprintly [product_id] #42 - show item #42
#   imubot sprintly [product_id] #42 tasks - list unfinished subtasks of story #42
#   imubot sprintly [product_id] <action> #42 - carry out action on item #42 (available actions: start, stop, finish, accept, reject, delete)
#   imubot sprintly [product_id] [environment] deploy 4,8,15,16,23,42 - mark items as deployed to an environment
#   imubot sprintly token <email:apitoken> - set/update credentials for user (required for other commands to work)
#   imubot sprintly default 1234 - set default product_id
#   imubot sprintly default_env production - set default environment (used for deploy)
#
# Author:
#   lackac
...
module.exports = (bot) ->
  ...
  bot.respond /sprintly +token +(.*)/i, (msg) ->
  ...
  bot.respond /sprintly +default +(\d+) *$/i, (msg) ->
    bot.brain.data.sprintly ?= {}
    bot.brain.data.sprintly.product_id = msg.match[1]
    msg.send "Default Product ID set to #{msg.match[1]}"

  bot.respond /sprintly +default_env +(.*)/i, (msg) ->
    bot.brain.data.sprintly ?= {}
    bot.brain.data.sprintly.env = msg.match[1]
    msg.send "Default environment set to #{msg.match[1]}"

  bot.respond /sprintly *(?: +(\d+))?(?: +(backlog|in-progress|completed|accepted))?(?: +(\d+))? *$/i, (msg) ->
  ...
  bot.respond /sprintly +(?:(\d+) +)?mine *$/i, (msg) ->
  ...
  bot.respond /sprintly +(?:(\d+) +)?#(\d+) *$/i, (msg) ->
    sprintly(msg).product()
  ...
  bot.respond /sprintly +(?:(\d+) +)?#(\d+) +tasks *$/i, (msg) ->
    sprintly(msg).product()
      .scope("items/#{msg.match[2]}/children.json")
      .get()(formatItems(msg, true))

  bot.respond /sprintly +(?:(\d+) +)?(start|stop|finish|accept|reject|delete) +#?(\d+) *$/i, (msg) ->
    withUserId msg, (user_id) ->
      query = {}
      method = 'post'
      switch msg.match[2]
  ...
  bot.respond /sprintly *(?: +(\d+))?(?: +(.*))?deploy +([\d]+(,[\d]+)*)/i, (msg) ->
    query =
      environment: msg.match[2] ? msg.bot.brain.data.sprintly?.env
      numbers: msg.match[3]
...
sprintlyUser = (msg) ->
  sp = msg.bot.brain.data.sprintly ?= {}
  sp[msg.message.user.id] ?= {}

sprintly = (msg, auth) ->
  if auth ?= sprintlyUser(msg).auth
    client = msg.bot.http('https://sprint.ly')
      .header('accept', 'application/json')
      .header('authorization', "Basic #{new Buffer(auth).toString('base64')}")
      .path('/api')
    client.product = ->
      if product_id = msg.match[1] ? msg.bot.brain.data.sprintly?.product_id
