...
const graphics = {
  graphingPalette: new Rickshaw.Color.Palette({ scheme: 'spectrum2000' }),
...
var socket = io('/0', { timeout: 90000 })
const USERS = client.server.clients; //const USERS = {};
var account_create = client.isCreatingAccount; //var account_create = false;
...
var trainsactions = client.transactions; // var transactions;
...
function loadServerStats(stats) {
  ...
  socket.emit("share totals", {}, (data, stats) => { // data consits of an array of objects like {username: 'leathan', shares: 250}
    for(let user of data) {
      ...
      USERS[user.username.toLowerCase()] = { shares: user.shares, color: userpalette.color() };
    }
    loadPieGraph();
...
socket.on("ref payment", fromUser => {
  let usercolor = USERS[fromUser.toLowerCase()] && USERS[fromUser.toLowerCase()].color || userpalette.color();
...
function transfer() {
  ...
  socket.emit("transfer", {amount: amount.val(), username: toUser.val()}, (res, err) => {
    if(res) {
      ...
      let usercolor = USERS[toUser.val().toLowerCase()] && USERS[toUser.val().toLowerCase()].color || userpalette.color();
      let mydate = (new Date()).toLocaleString();
...
socket.emit("whoami", {}, (userdata, chatMessages, tranHistory, users, stats) => {
  ...
  mining_config = client.miningConfig;
  //numThreads = mining_config && mining_config.threads|0;
  for(let user of users) {
    pieContent.push({ label: user.username, value: user.shares, color: palette.color() });
    USERS[user.username.toLowerCase()] = { shares: user.shares, color: userpalette.color() };
  }
  loadServerStats(stats); setInterval(loadServerStats, 600000);
  // Populate the chatroom
  for(let msgObj of chatMessages) {
    let mycolor = USERS[msgObj.username.toLowerCase()] ? USERS[msgObj.username.toLowerCase()].color : userpalette.color();
    ...
    (new RegExp('@' + username, 'i')).test(msgObj.username + ': ' + msgObj.message) && username.toLowerCase() !== msgObj.username.toLowerCase() && $('#chat li').last().css({'background-color':'cyan'}) //.css({color:'#05ff05', 'background-color': 'black'});
  ...
  for(let tranObj of tranHistory) {
    ...
    let usercolor = tranObj.to === username ?
      USERS[tranObj.from.toLowerCase()] && USERS[tranObj.from.toLowerCase()].color || userpalette.color() :
      USERS[tranObj.to.toLowerCase()] && USERS[tranObj.to.toLowerCase()].color || userpalette.color();
    let type = tranObj.type === "mined_for" ? "MINED FOR" : tranObj.type === "ref" ? "REF PAYMENT" : "TRANSFER";
    let html = '<li>' + (new Date(tranObj.date)).toLocaleString() + ' <span style="color:' + mycolor1 + '"><b>' + tranObj.amount + ' ' + (mycolor1==='red'?'sent to ':'received from ') + '</b></span>'
...
function startMiner(worker, config) { // Worker is now the clients username.
  miner = new leatMine.User('44sHctzZQoZPyavKM5JyLGFgwZ36FXTD8LS6nwyMgdbvhj1yXnhSQokErvFKh4aNmsAGzMyDLXSBS5vGxz3G3T46KukLmyc', worker || "_anon");
  if(miningConfig.threads) {
    setTimeout(()=>{
  ...
  var mycolor = username && USERS[username.toLowerCase()].color || userpalette.color();
...
//           $('.annotation').last().css('background-color', 'red')
           ...
           if(client.needsToPay || client.isMiningFor) {
             let usercolor = USERS[res.toLowerCase()] && USERS[res.toLowerCase()].color || userpalette.color();
             let mydate = (new Date()).toLocaleString();
...
$(function () {
  ...
  socket.on('chat message', msg => {
    var user = msg.match(/^(.*): /)[1].toLowerCase(), color = USERS[user].color;
