...
<div style="background-color: rgba(191, 63, 182, 0.11); padding: 40px">
 <div class="main-content" style="background-color: #FDF5FE; padding:20px; border-radius: 15px">
  ...
  <div ng-repeat="log in logs | reverse | filter: search_logs track by $index">
   <div ng-if="search_server=='All' || log.server==search_server">
    <b>{{log.author}}:</b> {{log.message}}<h5 style="display: inline"><small style="float: right">#{{log.channel}}@{{log.server}} ({{log.date | amTimeAgo}})</small></h5>
    <hr style="height:1px; color:#aaa; margin: 3px" />
   </div>
  </div>
 </div>
