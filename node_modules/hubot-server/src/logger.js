...
module.exports = (robot) => {
  robot.hear(/.*/, r => {
    ...
    if(robot.adapterName === 'discord')  {
      ...
      msgObj = robot.adapter.client.channels.find('id', r.message.room)
      if(msgObj.type === 'dm') return; // Don't log private messages.
      channel = msgObj.name
      server  = msgObj.guild.name
    }
    else if(robot.adapterName === 'slack') {
      channel      = robotSlack.adapter.client.rtm.dataStore.getChannelGroupOrDMById(r.message.room).name
      if(channel === 'DM') return; // Don't log private messages.
      message      = r.message.text;
      var serverID = robot.adapter.client.rtm.dataStore.getUserByName(r.message.user.name).team_id
      if (typoeof serverID == 'undefined') { console.log("Error in logger, dumping response obj:"); console.log(r); return }
