...
module.exports = bot => {
    ...
    isLoggedIn(socket, username => {
      ...
      socket.on("private message open", pm => {
        ...
        if(pm.exists) delete pm.exists
          ...
          console.log(pm.username + "'s private message opened by " + username + ".")
        });
      ...
      socket.on("private message toggle", pm => {
        ...
        Users.findOneAndUpdate({ "username": username, "private_messages.username": pm.username  }, { $set: { "private_messages.$": pm } }, (err, user)=>{
          console.log(pm.username + "'s private message toggled by " + username + ".")
        });
      })
      socket.on("private message read", pm => {
        var data = "awaiting_pm." + pm.username;

        Users.findOneAndUpdate({ "username": username }, { $set: { data: false } }, (err, user)=>{
  ...
  function createPm(sender, pmData, cb) {
    ...
    Pms.create(pm, (err, pm) => {
      var data = "awaiting_pm." + sender;
      Users.findOneAndUpdate({ "username": pmData.username, "private_messages.maximized": { $not: true }, "private_messages.username": { $not: new RegExp('^'+sender+'$') }  }, { $inc: { data: 1 } }).then((e,u)=>{
