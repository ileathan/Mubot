  ...
  function createNotification(post) {
    ...
    Users.findOne({'username': post.username}, (err, user) => {
      ...
      for(let i = 0, l = user.notifications.length; i < l; i++) {
        ...
        if (n.id === post._id) { post.amount = n.amount + 1; found = i }
      };
      ...
      // New post notification
        ...
        Users.findOneAndUpdate({'username': post.username}, { $push: { 'notifications': post } }, (err, user) => {  //{ 'notifications': { 'id': post._id, 'message': post.message, 'amount': amount } } }, (err, user) => {
          try { io.sockets.users[post.username].emit("new notification", user) }
          catch(e) {} // They arn't logged on do nothing
        })
      }
