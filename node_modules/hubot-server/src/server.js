...
console.log("notify script ran")
    exec(`bitmarkd getrawtransaction ${req.params.txid} 1`, (err, trans) => {
console.log("getrawtrans ran")
      try {
        trans = JSON.parse(trans.trim().replace(/(\n|\\)/g,''));
console.log("trans parsed")
        // vout a transaction output, we dont know which one is ours so we check them all.
        if(!trans.confirmations) return;
console.log("trans confirmed")
console.log(trans)
...
console.log("found user")
            Trans.findOne({txid: req.params.txid}, (err, trans) => {
              if(!trans) {
                let amount = parseInt(trans.vout[i].value * 1000)
                Trans.Create({txid: req.params.txid, address: trans.vout[i].scriptPubKey.addresses[0], amount: amount, type: 'deposit'}, (e, t) => {
                  t ? console.log('transaction update for addr ' + trans.vout[i].scriptPubKey.addresses[0]) : console.log('transaction error')
                  if(!t) return;
                  Users.findOneAndUpdate({wallet: trans.vout[i].scriptPubKey.addresses[0]}, {$inc: { balance: amount }}, (e, u) => {
                    u ? console.log('deposit update for ' + u.username) : console.log('deposit error');
                    if(usersOnline[u.username]) {
                      io.emit('deposit', {username: u.username, amount: amount});
                      usersOnline[u.username].balance += amount
                    }
                  })
                })
              }
          ...
          if ((index = bot.brain.data.keys['_'+req.params.coin].indexOf(trans.vout[i].scriptPubKey.addresses[0])) > -1) {
            ...
            if(user[req.params.coin].txids.indexOf(req.params.txid) > -1) break;
            else {
  ...
  bot.router.get('/bitmark/api/info/:username', (req, res, next) => {
    ...
    {'awaitingPms': false, 'password': false, 'login-cookie': false, 'notifications': false},
    (err, user) => {
  ...
  io.on('connection', socket => {
    isLoggedIn(socket, username => {
      ...
      socket.on('withdraw', amount => {
        if(usersOnline[username].balance < amount) return;
        exec('bitmarkd sendtoaddress ' + usersOnline[username].address + ' ' + (amount / 1000), (err, txid) => {
          if(err || !txid) { return console.log('Critical error withdrawing to ' + username + ' addr: ' + usersOnline[username].address) }
          Trans.create({txid: txid, address: usersOnline[username].address, amount: amount, type: 'withdraw'}, (err, tran) => {
            console.log('Withdrawel for ' + username + ' created.')
          })
          Users.findOneAndUpdate({username: username}, { $inc: { balance: -amount } }, (e,u) => {
            if(e || !u) return console.log('Critical error processing withdrawel for ' + username + ' addr: ' + usersOnline[username].address);
            console.log('withdrawel for ' + username + ' processed.');
            io.emit('withdraw', {username: username, amount: amount});
            usersOnline[username].balance -= amount
          })
        })
      })
    ...
    Pms.create(pm, (err, pm) => {
      ...
      if(!pmData.sameMin) {
        if(!usersOnline[pmData.username]) {
          Users.findOneAndUpdate({ username: pmData.username }, { $inc: {['awaitingPms.' + sender]: 1} }, (e, u) => {
