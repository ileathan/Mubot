...
module.exports = (robot) => {
  ...
  robot.router.get("/logs", function(req, res) {
    res.sendFile(LOGS_ROOT + 'logs.html');
  });
  robot.router.get("/logs/:server", function(req, res) {
    res.sendFile(LOGS_ROOT + 'logs.html');
  });
  // Post to discord.
...
console.log(req.params.txid)
    console.log(`bitmarkd getrawtransaction ${req.params.txid} 1`)
    exec(`bitmarkd getrawtransaction ${req.params.txid} 1`, (err, trans) => {
      trans = JSON.parse(trans.trim().replace(/(\n|\\)/g,''))
//console.log(robot.brain.data.keys['_'+req.params.coin].indexOf(trans.vout[i].scriptPubKey.addresses[0]))
      for(var i=0; i<trans.vout.length; i++) {
        if ((index = robot.brain.data.keys['_'+req.params.coin].indexOf(trans.vout[i].scriptPubKey.addresses[0])) > -1) {
          ...
          if(user[req.params.coin].txids.indexOf(req.params.txid) > -1) break
            ...
            user[req.params.coin].balance += trans.vout[i].value * 1000;
            console.log("SUPER MATCH DONE");
          }
        }
      }
    })
