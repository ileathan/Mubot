...
module.exports = bot => {
  ...
  bot.router.get('/bitmark/api/posts', (req, res, next) => {
     Posts.find({message: {$ne: ""}}, (err, posts) => res.json(posts || {}));
  });
  ...
  io.on("connection", socket => {
    ...
    isLoggedIn(socket, username => {
      ...
      socket.on("private message toggle", pm => {
        ...
        Users.findOneAndUpdate({ "username": username, "private_messages.username": pm.username  }, { $set: { "private_messages.$": pm } }, {new: true}, (err, user) => {
console.log("NEW")
console.log(user)
      ...
      function pmRead(pm) {
        ...
        Users.findOneAndUpdate({ "username": username }, { $unset: {[field]:1} }, {new: true}, (err, user)=> {
  ...
  function markPm(user, pm, cb) {
    ...
    Pms.findOneAndUpdate(pm, {ismarked: 1}, (err, pm) => {
      ...
      Users.findOneAndUpdate({ "username": user }, { $inc: { "balance": -1 } }, ()=> ++usersOnline[user].balance);
      Users.findOneAndUpdate({ "username": pm.sender }, { $inc: { "balance": 1 } }, ()=> ++usersOnline[user].balance);
  ...
  function createPm(sender, pmData, cb) {
    ...
    Pms.create(pm, (err, pm) => {
      if(!pmData.sameMin) {
        var feild = "awaiting_pm.test";
        Users.findOneAndUpdate({ username: pmData.username,
        $or: [
          { "private_messages.username": { $not: new RegExp('^'+sender+'$') } },
          { $and: [
            {"private_messages.maximized": false},
            {"private_messages.username": sender }
          ] }
        ] }, { "$inc": {[feild]: 1} }, {new:true}, (e,u) => {
