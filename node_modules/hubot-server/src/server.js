    ...
    Users.findOne({'username': post.username}, (err, user) => {
      ...
      if (post.amount > 1) {
        ...
        Users.findOneAndUpdate({'username': post.username}, { $set: obj }, {"new":true}, (err, user) => {
          try { io.sockets.users[post.username].emit("new notification", post) }
          catch(e) {console.log("error #0")} // They arn't logged on do nothing
        });
       }
      // New post notification
      else {
        Users.findOneAndUpdate({'username': post.username}, { $push: { 'notifications': post } }, {"new":true}, (err, user) => {
          try { io.sockets.users[post.username].emit("new notification", post) }
          catch(e) {console.log("error #0")} // They arn't logged on do nothing
        })
      }
