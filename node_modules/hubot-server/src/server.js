...
module.exports = bot => {
  ...
  bot.router.get('/bitmark/api/info/:username', (req, res, next) => {
    Users.findOne({username: new RegExp("^" + req.params.username + "$", "i") }, (err, user) => {
      if (user) {
        user.awaiting_pm = {}; user.password = ""; user['login-cookie'] = ""; user.notifications = ""; // Don't return the sensitive information via ajax response
        res.json(user);
      } else {
  ...
  bot.router.get('/bitmark/api/info', (req, res, next) => {
    Users.findOne({"login-cookie": req.cookies["login-cookie"]}, function (err, muser) {
      if (muser) {
        muser.password = ""; muser['login-cookie'] = ""; // Don't return the sensitive information via ajax response
        res.json(muser)
      } else {
