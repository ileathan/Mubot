...
module.exports = bot => {
  ...
  function updateLoggedIn(socket, loggedIn, cb) {
    if(socket.handshake.headers.cookie) {
      let cookie = /login-cookie=(\w{32})/.exec(socket.handshake.headers.cookie)[1];
      // optimized shortcut.
      if(!loggedIn) {
        let user = logged[cookie];
      ...
      Users.findOne({ "login-cookie": cookie }, (err, user) => {
        if(err) return throw new Error('Error @ Users.findOne({ "login-cookie": cookie }, (err, user) => {');
  ...
  io.on("connection", socket => {
    if(!logged[socket.id]) {global.socket=socket; console.log("socket " + socket.id + " is not logged in");}
    else console.log("socket " + socket.id + " is " + logged[socket.id])
