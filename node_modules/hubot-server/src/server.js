...
module.exports = bot => {
  ...
  io.on("connection", socket => {
    ...
    isLoggedIn(socket, username => {
      ...
      socket.on("private message open", pm => {
        Users.findOneAndUpdate({ "username": username }, { $push: { "private_messages": pm } }, (err, user)=>{console.log(user)});
      ...
      socket.on("private message toggle", pm => {
        var data = "private_messages.username" + pm.user.username;
        Users.findOneAndUpdate({ "username": username, "private_messages.username": pm.user.username  }, { $set: { "private_messages.$": pm } }, (err, user)=>{console.log(user)});
      })
  ...
  function createPm(sender, pmData, cb) {
    ...
    Pms.create(pm, (err, pm) => {
      ...
      Users.findOneAndUpdate({ "username": pmData.username }, { $inc: { data: 1 } }).then(()=>{
        console.log("created an 'awaiting_pm'")
      })
      cb(pm || {})
    })
  }
