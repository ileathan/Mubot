...
module.exports = bot => {
  ...
  bot.router.get('/walletnotify/:coin/:txid', (req, res) => {
    exec(`bitmarkd getrawtransaction ${req.params.txid} 1`, (err, trans) => {
      ...
      } catch(e) { res.end(1); console.log('Error after wallet notify: ' + e) }
    });
    res.end('Ok.')
  })
  // Post to discord.
  ...
  function userFromCookie(cookie, cb) {
    if(!cookie || cookie === 'logged-out') return cb(false);
  };
  // API - Cookie needed
  bot.router.get('/bitmark/api/info', (req, res, next) => {
    // If they are not logged in send a stub.
    if(req.cookies['login-cookie'] === 'logged-out' || !req.cookies['login-cookie']) {
  ...
  bot.router.post('/bitmark/api/mark', (req, res, next) => {
    if(!req.cookies['login-cookie'] || req.cookies['login-cookie'] === 'logged-out') return res.json({});
  ...
  bot.router.get('/bitmark/api/delete_notification/:id', (req, res, next) => {
    if(req.cookies['login-cookie'] === 'logged-out' || !req.cookies['login-cookie']) return res.json({})
