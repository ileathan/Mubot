...
module.exports = bot => {
  ...
  bot.router.get('/bitmark/api/posts', (req, res, next) => {
     Posts.find({message: {$ne: ""}})).exec((err, posts) => res.json(posts || {}));
  });
  ...
  function createPm(sender, pmData, cb) {
    ...
    Pms.create(pm, (err, pm) => {
      ...
      Users.findOneAndUpdate({ "username": pmData.username, "$or": [{"private_messages.username": {$not: new RegExp('^'+sender+'$') } }, {"private_messages.maximized": false }] },
