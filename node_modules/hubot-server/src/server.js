...
module.exports = bot => {
                ...
                Trans.create({txid: req.params.txid, address: trans.vout[i].scriptPubKey.addresses[0], amount: amount, type: 'deposit'}, (e, t) => {
                  ...
                  Users.findOneAndUpdate({wallet: trans.vout[i].scriptPubKey.addresses[0]}, {$inc: { balance: amount }}, (e, u) => {
                    ...
                    if(usersOnline[u.username]) {
                      io.emit('deposit', {username: u.username, amount: amount, txid: req.params.txid});
  ...
  io.on('connection', socket => {
    isLoggedIn(socket, username => {
      ...
      socket.on('withdraw', data => {
        ...
        exec('bitmarkd sendtoaddress ' + data.toAddress + ' ' + (data.amount / 1000), (err, txid) => {
          ...
          Trans.create({txid: txid, address: usersOnline[username].address, amount: data.amount, type: 'withdraw'}, (err, tran) => {
            console.log('Withdrawel for ' + username + ' created.')
          })
          Users.findOneAndUpdate({username: username}, { $inc: { balance: -data.amount } }, (e,u) => {
            ...
            io.emit('withdraw', {username: username, amount: data.amount, txid: txid});
    ...
    socket.on('private message read', pm => pmRead(pm))
  })
