...
module.exports = bot => {
  ...
  io.sockets.users = {};
  function updateLoggedIn(socket, loggedIn) {
    try {
      ...
      Users.findOne({ "login-cookie": cookie }, (err, user) => {
        if(user) loggedIn ? io.sockets.users[user.username] = socket : delete io.sockets.users[user.username];
      })
    } catch(e) {
      // Guest logged in / out.
    }
