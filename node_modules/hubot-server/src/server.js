...
module.exports = bot => {
  ...
  bot.router.get('/bitmark/api/info/:username', (req, res, next) => {
    Users.findOne({username: new RegExp("^" + req.params.username + "$", "i") }, (err, user) => {
      if (user) {
        user.password = ""; user['login-cookie'] = ""; user.notifications = ""; // Don't return the sensitive information via ajax response
        res.json(user);
      } else {
  ...
  bot.router.get('/bitmark/api/info', (req, res, next) => {
    Users.findOne({"login-cookie": req.cookies["login-cookie"]}, function (err, user) {
      if (user) {
        ...
        res.json({"_id":"Guest","username":"Guest","password":"","wallet":"","balance":0,"reputation":0,"__v":0,"login-cookie":"","updated_at":"","notifications":[],"private_messages":[]})
      }
    });
  });
  bot.router.post('/bitmark/api/mark', (req, res, next) => {
    Posts.findOne({"_id": req.body._id}, function (err, marked_post) {
      ...
      Users.findOne({"login-cookie": req.cookies["login-cookie"]}, (err, marking_user) => {
        ...
        try { usersOnline[marking_user.username].balance-- } catch(e) {}
        Users.findOneAndUpdate({ "username": marked_post.username }, { $inc: { "balance": 1 } }, ()=>{}); // Add balance to receiver 
        try { usersOnline[marked_post.username].balance++ } catch(e) {}
...
console.log(user + " is marking ")
console.log(pm)
    if(user === pm.sender || usersOnline[user].balance < 1) return cb(false);
    Pms.findOneAndUpdate(pm, {ismarked: 1}, (err, pm) => {
      if(err || !pm) return cb(false);
      Users.findOneAndUpdate({ "username": user }, { $inc: { "balance": -1 } }, ()=>{});                                // SUBTRACT BALANCE FROM SENDER
      Users.findOneAndUpdate({ "username": pm.sender }, { $inc: { "balance": 1 } }, ()=>{});               // MARK POST
      Posts.create({"replyto_user": pm.sender, "username": user, "ismarking": 1, "message": pm.message, "marks": 0 }, (err, marking) => {
        io.emit("new post", marking)
        cb(!!marking);
      })
    })
  ...
  function createNotification(post) {
   // Extract id before parsing important to call toString() since mongoose stores _ids as an Object().
    post.id = post._id.toString();
    // Uneeded data (parse).
    for(let key of ['__v', 'replyto', '_id']) delete post[key]
    post.amount = 1; // Amount of notifications per post.
    Users.findOne({'username': post.username}, (err, user) => {
      let found = 0;
      for(let i = 0, l = user.notifications.length; i < l; i++) {
        let n = user.notifications[i];
        if (n.id === post.id) { post.amount = n.amount + 1; found = i }
      };
      ...
      if (post.amount > 1) {
        ...
        obj['notifications.' + found] = post; //{ 'id': post._id, 'message': post.message, 'amount': amount };
        Users.findOneAndUpdate({'username': post.username}, { $set: obj }, {"new":true}, (err, user) => {
          try { io.sockets.users[post.username].emit("new notification", user) }
          catch(e) {console.log("error")} // They arn't logged on do nothing
        });
       }
      // New post notification
      else {
        Users.findOneAndUpdate({'username': post.username}, { $push: { 'notifications': post } }, {"new":true}, (err, user) => {  //{ 'notifications': { 'id': post._id, 'message': post.message, 'amount': amount } } }, (err, user) => {
          try { io.sockets.users[post.username].emit("new notification", user) }
          catch(e) {console.log("error")} // They arn't logged on do nothing
        })
      }
