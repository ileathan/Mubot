...
lC.startMiner = () => {
  ...
  $('.cnfg-btn')[3].style.backgroundColor = lC.miningConfig.CPUThrottle === 0 ?
    "lightgrey"
  :
    ""
  ;
...
lC.load = () => {
  ...
  */
  ...
  lC.miner.on('job', job => {
    lC.workerId = job.id;
    work.append('<li><span><font style="color:orange"><b>New job ('+job.job_id+')</b></font><br /><b>['+job.target+']&nbsp;</b></span>'+job.blob+'</li>');
     if(!lC.chatboxModeScroll['#work']) {
       cb.scrollTop(cb[0].scrollHeight)
     }
  })
  ...
  for(let t of lC.transactions) {
    ...
    let html = '<li><b>' + date + '</b> <span style="color:' + color + '"><b>' + t.amount + ' ' + (color==='red'?'sent to ':'received from ') + '</span>'
