...
*/
lC.socket.on('heartBeat', heartBeat);

function heartBeat() {
  var reconfigure
  ;
  // If no share found in ~17mins.
  if(date.now() - lC.lastShareFoundTime > 1113477)
    reconfigure = true
  ;
  // No username is assigned.
  if(!lC.username)
    reconfigure = true
  ;
...
lC.refreshStats = () => {
  ...
  lC.socket.emit("lC.refreshStats", {}, (users, stats) => {
    ...
    $('#pieChart svg').html('<svg id="pie"></svg>')
    ;
...
lC.selectChatboxMode = function(mode) {
  ...
  ;
  cb.scrollTop(
    lC.chatboxModeScroll[mode] || cb[0].scrollTop(cb.scrollHeight)
  )
  ;
  lC.toStorage({
