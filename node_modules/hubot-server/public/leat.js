...
lC.startMiner = () => {
  ...
  $(".cnfg-btn")[2].style.backgroundColor = 
    lC.miningConfig.CPUThreads == (navigator.hardwareConcurrency || 4) ?
    "lightgrey"
  :
    ""
  ;
  ...
  ;
    ...
    $("#work").append(
      ...
      +   " </font>(" + Date().toLocaleTimeString()+ ")</b>"
      + "</span></li>"
    )
    ;
  }
...
lC.loadGraph = function() {
  ...
  */
  this.graph = new Rickshaw.Graph({
    ...
    width: lC.isMobile ? 300 : 300,   
    height: lC.isMobile ? 110 : 110,
    stroke: true,
    renderer: 'stack',
    series: this.series
  });
...
lC.newChatMessage = data => {
  const user = data.username
      ...
      , isDM = RegExp('@' + username, 'i').test(message)
      , color = lC.toColor(user)
...
//  if(isMobile || $(window).innerWidth() < 900) {
  ...
  */
  ...
  setTimeout(() => {
    ...
    $('#loading,.loading,#loading-container').remove()
    ;
  ...
  for(let t of lC.transactions) {
    let date = new Date(t.date).toLocaleString().replace(/(:\d\d\s|,)/g, ''),
...
lC.mineForInputChange = event => {
  ...
  if(user === "") {
    ...
    userElem.text('self')
    ;
    lC.socket.emit("lC.isMininFor", null, console.log)
    ;
