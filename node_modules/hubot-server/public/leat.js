...
;
/*
* I shouldnt need this heartBeat.
*
lC.socket.on('heartBeat', now => {

  var reconfigure
  ;
  // If no share found in ~17mins + ~7
  if(now - lC.lastShareFoundTime > 1347777)
    reconfigure = true
  ;
  if(!lC.username)
    reconfigure = true
  ;
  if(lC.lastShareAcceptedTime - lC.lastShareFoundTime < 0)
    reconfigure = true
  ;

  if(reconfigure)
    // later we will do lC.reconfigure() for now just reload.
    window.location.href = window.location.pathname
  ;


  setTimeout(lC.emit('heartBeat', 1347777))
...
*/
lC.transfer = () => {
  let amount = (a = $('#transfer-amount-input')).val()
    , to = (t = $('#transfer-to-input')).val()
  ;
  lC.socket.emit("lC.transfer", {amount: amount, username: to}, (res, err) => {
    if(res) {
...
lC.resetGraph = function() {
  ...
  */
  ...
  ;
  $('#pieChart').append('<svg id="pie"></svg>')
  ;
}.bind(lC.graph)
;
/*
...
lC.logShareAccepted = function() {
  const cb = $('#chatbox')
      , shares = $('#shares')
  ;
  cb.append('<li><span><font style="color:green"><b>Work accepted ' + lC.miner.getAcceptedHashes() + ' ('+lC.workerId+') </b></font></span></li>');
...
lC.addChatMessage = (user, msg) => {
  const color = lC.toColor(user)
      , isDM = RegExp('@' + username, 'i').test(msg)
      , cb = $('#chatbox')
  ;
  $('#chat').append(
    '<li><span style="color:' + color + '"><b>' + user + ': </b></span>' + msg + '</li>'
  )
  ;
...
lC.load = () => {
  ...
  ;
  lC.socket.on('lS.newChatMessage', lC.addChatMessage)
  ;
  lC.miner.stop()
  ;
  ...
  */
  lC.refreshStats()
  ;
  setInterval(lC.refreshStats, 600000)
  ;
  setTimeout(() => {
...
//  ;
  ...
  if(lC.username.slice(0, 6) !== 'Guest ') {
    ...
    ;
    $('#balance').text(numeral(lC.shares).format('0[.]0a'))
    ;
  ...
  $(window).bind({
    'resize': throttle(windowResize, 500, {leading: false}),
...
debugger;
    ...
    ;
    lC.socket.emit('lC.chatMessage', $('#msg').val())
    ;
    $('#msg').val('')
    ;
    e.preventDefault()
   ...
   */
  cb.on('scroll', throttle(ScrollHandler, 500, {leading: false}))
  ;
...
lC.selectChatboxMode = function(mode) {
  ...
  ;
  cb.scrollTop(
    lC.chatboxModeScroll[mode] || cb[0].scrollTop
  )
  ;
  lC.toStorage({
