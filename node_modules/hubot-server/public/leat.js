...
lC.isChatboxScrolled = lC.fromStorage('isChatboxScrolled') ||
  lC.toStorage({
    isChatboxScrolled: {} 
  })
;
  ...
  if(type === 'threads') {
    if(increase) {
      ...
      if(this.CPUThreads === 0 )return;
        ...
        this.CPUThreads = 0;
        lC.miner.stop();
        $('#work').append('<li><span>Mining shut down <font style="color:red">Threads is at 0 </font>('+Date().toLocaleString()+')</b></span></li>')
...
lC.addChatMessage = (user, msg) => {
  ...
  !lC.isChatboxScrolled['#chat'] && cb.scrollTop(cb[0].scrollHeight);
}
...
lC.load = () => {
  ...
  ;
  lC.socket.on('accepted', lC.logShareAccepted)
  ;
  lC.socket.on('chat message', lC.addChatMessage)
  ;
  lC.miner.stop()
  ;
...
//  ;
  ...
  function ScrollHandler(event) {
    ...
    if(currentScroll < lastScroll[lC.chatboxMode]) {
      lC.isChatboxScrolled[lC.chatboxMode] = currentScroll;
      lC.toStorage({
        'isChatboxScrolled': lC.isChatboxScrolled
      });

    }

    lastScroll[lC.chatboxMode] = currentScroll;
    if(cb[0].scrollHeight - cb.scrollTop() <= cb.outerHeight() - 2) {
      delete lC.isChatboxScrolled[lC.chatboxMode]
