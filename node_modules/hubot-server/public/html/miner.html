...
const HOST_NAME = document.getElementsByTagName('script')[0].src.split(/[/]/).slice(0,3).join('/')
// Loop through every iframe and check if one
var parent = [].slice.call(window.parent.document.getElementsByTagName('iframe')).filter(
  _=> RegExp('^' + HOST_NAME, 'i').test(_.src)
)[0];

if(parent) {
 parent.style.width || (parent.style.width = '400px');
 parent.style.height || (parent.style.height = '300px');
 parent.style.border = 'none';
}
options.user && (document.getElementById('user-name').textContent = options.user)
;
...
function load(){
  ...
  function fromStorage() {
    ...
    const shortcutToCmd = {
      s: 'start', pm: 'powermode', hl: 'hidelogo', bg: 'background', c: 'color', txt: 'text', g: 'graph', r: 'ref',
      a: 'address', u: 'user', h: 'hide', as: 'autostart', p: 'preserve', t: 'threads', th: 'throttle', i: 'interval'
    }
    ;
   	const optionsArray = window.location.href.match(RegExp("[?&]" + ".*?" + "=?(?:([^&#]*)|&|#|$)", 'g' ) ) || [];
