...
lC.socket.emit("whoami", null, res => {
  ...
  /complete|interactive/.test(document.readyState) ? lC.loadContent() : lC._needloadContent === true;
...
lC.refreshStats = () => {
  ...
  for(let u in users) {
    pieContent.push({ label: u.username, value: u.shares, color: lC.userpalette.color() })
  }
...
lC.resetGraph = function() {
  ...
  /* Hack the legend off the screen, the user should know its for threads by now. */
