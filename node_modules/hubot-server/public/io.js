...
=======
>>>>>>> 0383d06c89d8c72dd4a93fbaf61aba50b5ff9427
...
=======
      '<li><span><b>Mining as <font style="color:' + toColor(lC.username) + '">'
>>>>>>> 0383d06c89d8c72dd4a93fbaf61aba50b5ff9427
...
=======
    let usercolor = t.to === lC.username ? toColor(t.from) : toColor(t.to);

    let type = t.type === "mined_for" ? "MINED FOR" : t.type === "ref" ? "REF PAYMENT" : "TRANSFER";
    let html = '<li>' + new Date(t.date).toLocaleDateString() + ' ' + new Date(t.date).toLocaleTimeString() +
    + ' <span style="color:' + typeColor + '"><b>' + t.amount + ' ' + (color==='red'?'sent to ':'received from ') + '</b></span>'
    + '<font style="color:' + usercolor + '">' + (color==='red'?t.to:t.from) + '</font><span style="float:right;background-color:'+typeColor+';padding: 1px 19px 1px 19px">'+type+'</span></li>'
    $('#transactions').append(html);
  }
//debugger;
  lC.showChat('chat', true)
>>>>>>> 0383d06c89d8c72dd4a93fbaf61aba50b5ff9427
...
=======
  ...
  $('#username').on('keyup', lC.validateUsername);
  lC.socket.on('chat message', msg => {
    var user = msg.match(/^(.*): /)[1], color = toColor(user);
>>>>>>> 0383d06c89d8c72dd4a93fbaf61aba50b5ff9427
...
lC.enable2fa = () => {
  ...
  if(btn.html() === 'Okay') {
    lC.socket.emit("verify tfa", $("tfa-input").val(), correct => {
      if(!correct) return $('#tfa-info').text('Incorrect');
      lC.tfa = true
    })
  } else {
  ...
  if(lC.isCreatingAccount) {
    ...
    lC.socket.emit("create account", { "ref": pRef, "username": $('#username').val(), "password": $('#password').val() }, data => {
      if(data.error) alert(data.error);
        ...
        console.log("Setting cookie to: " + data.slice(0, 32));
        document.cookie = 'loginCookie=' + data + '; expires=Thu, 01 Jan 2222 00:00:01 GMT; path=/;';
