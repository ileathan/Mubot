...
lC.load = () => {
  ...
  if(true){ //username.slice(0, 6) !== 'Guest ') {
    ...
    $('#user-stats-container').html(
      ...
      +       '<input type="button" id="tfa-button" value="Enable" onclick="lC.enable2fa()" id="enable2fa-button">' 

      +       '<input id="tfa1" maxlength="6" pattern="[0-9]{4}" required="true" type="number"'
      +       'min="1" onkeydown="event.keyCode === 13 && lC.enable2fa()"'
      +       ' placeholder="Code" tabindex="1">'
      +       '<font class="dd-tfa-text">Enter Code</font>'
      +       '<input id="tfa2" maxlength=6 pattern="[0-9]{4}"'
      +       ' required="true" type="number" onkeydown="event.keyCode === 13 && lC.enable2fa()" placeholder="Code" tabindex="2">'
      +       '<small id="tfa-info">Waiting...</small>'
      +     '</span>'
      +   '</span></span>' : '' )
...
lC.enable2fa = () => {
  ...
  $('#tfa-button').prop('value', 'Enter Code')
