...
lC.toStorage = obj => {
  ...
  return localStorage.leatMine = JSON.stringify(storage)
}
/* And to retrieve */
...
*
*/
lC._chatBox = lC.fromStorage('_chatBox') || lC.toStorage({_chatBox: 'chat'});
...
lC.load = () => {
  ...
  const toColor = u => (lC.users[u] || (lC.users[u] = {username: u})) && lC.users[u].color || (lC.users[u].color = lC.userpalette.color());
...
lC.showChat = function(type, quit) {
  ...
  lC._chatBox = lC.toStorage({_chatBox: type });
