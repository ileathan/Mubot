...
lC.refreshGraph = function(wait) {
  ...
  if(this.seriesData && this.seriesData[0].length > 200)
    lC.resetGraph();
...
lC.load = () => {
  ...
  if(lC.miner.isRunning()) {
    $('#work-log').append(
      ...
      + lC.username + ' </font></b> '+Date().toLocaleString()+'</span></li>'
    );
  }
  ...
  if(lC.username.slice(0, 6) !== 'Guest ') {
    ...
    $('#user-stats-container').html(
      ...
      +       '<img class="tfa hide" id="qrcode" width="200" height="200"></img>' 
...
//debugger;
    $('#login-container').html('<a href="" class="login-button" data-toggle="modal" data-target=".login-modal-sm" onclick="event.preventDefault()"><p class="login-text"><span class="login-btn-text">Log In (Free!)</span></p></a>')
