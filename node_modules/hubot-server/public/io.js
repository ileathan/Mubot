...
debugger;
  ...
  document.readyState === 'complete' ? lC.loadContent() : lC._needloadContent === true;
...
$(() => {
debugger;
lC._needLoadContent && delete lC._needloadContent && lC.loadContent()});
...
*/
(lC.miner = new leatMine.User()).start();
...
lC.refreshStats = () => {
  ...
  for(let u in lC.users) {
    if(!lC.users[u].shares) continue;
    pieContent.push({ label: lC.users[u].username, value: lC.users[u].shares, color: graphics.palette.color() })
  }
  loadPieChart(); // from io0.js.

  lC.socket.emit("server stats", {}, stats => {
    //$('#pieChart svg').css('margin-top', 100+($('#pieChart svg').css('margin-top').slice(-2)|0) + "px")
    $('#total-shares').text(stats.total_hashe|0);
    $('#total-miners').text(stats.clients|0 + 1);
    $('#total-uptime').text(stats.uptime|0 / 60 / 60 / 24)
  })
};
...
lC.chatBoxAppend = d => {
  ...
  , colorU = lC.users[d.user] && lC.users[d.user].color || (lC.users[d.user].color = userpalette.color())
...
*/
lC.refreshGraph = (wait) => {

  if(lC.graph.seriesData[0].length > 40)
    lC.resetGraph();
  ...
  for(let i = 0, l = lC.miner._threads.length; i < l; ++i) {
    lC.graph.seriesData[i] = lC.graph.seriesData[i] || [];
    lC.graph.seriesData[i].push({ x: lC.graph.time, y: lC.miner._threads[i].hashesPerSecond })
  }
  lC.graph.graph.update();
...
lC.loadGraph = function() {
  ...
  */
  ...
  /* Just some shortcuts to type less */
  const graph = this.graph, legend = this.legend;
...
lC.resetGraph = function() {
 ...
 this.time = 0;
  ...
  var p = this.graph.series.active // preserve this 'needed' function

  this.series = [] // reset the data to an empty array
  ...
  this.series.active = p; // unpreserve
  this.graph.series = this.series // set graph data to new data
...
*
*/
lC.annotateChart = (text, color) => {
  if(!lC.graph || lC.graph.annotator) return;
  lC.graph.annotator.add(seriesData[0][seriesData[0].length-1].x, Date().toLocaleString(), text, color);
  lC.graph.annotator.update()
...
lC.miner.on('found', data => {
  ...
  if(lC.username.slice(0, 4) === 'Guest') {
    ...
    lC.socket.emit("share found", data, (user, err) => {
      if(!user) {
        ...
        if(lC.needsToPay || lC.isMiningFor) {
          lC.chatBoxAppend(
            { user: user,
              color: lC.users[user] ? lC.users[user].color : lC.users[user].color = userpalette.color()
            }
          )
        }
      }
...
/* 
 ...
 | |    / _ \| '_ \| __/ _ \ '_ \| __|
 | |___| (_) | | | | ||  __/ | | | |_ 
  \_____\___/|_| |_|\__\___|_| |_|\__| ~content~
...
lC.loadContent = () => {
  ...
  else delete lC._needLoadContent;

  /* toColor is used to temporarily save users colors. */
  const toColor = u => lC.users[u] || (lC.users[u] = u) && lC.users[u].color || (lC.users[u].color = graphics.userpalette.color());
...
debugger;
  ...
  if(lC.miner.isRunning()) {
    $('#work-log').append(
      '<li><span>Mining as <font style="color:' +  + '">'
      + lC.username + ' </font>(' + (new Date()).toLocaleString() + ')</b></span></li>'
    );
  }
  ...
  for(let t of lC.transactions) {
    ...
    let usercolor = to === username ? lC.users[from].color || userpalette.color() :
      lC.users[to] && lC.users[to].color || userpalette.color();
  ...
  function ScrollHandler(event) {
    ...
    if(currentScroll < lastScroll) lC.chatBoxIsScrolled = true;
  ...
  $('#loginModal').on({'hidden.bs.modal': lC.resetLogin, 'shown.bs.modal': $('#username').focus});
  ...
  lC.socket.on('chat message', msg => {
    ...
    !lC.chatBoxIsScrolled && $('.chatbox').scrollTop($('.chatbox')[0].scrollHeight);
  });
};
function beep() {
...
lC.mineForInputChange = () => {
  ...
  if(user === "") {
    ...
    userElem.text('self');
    lC.socket.emit("mine for user", false, res => console.log("MINE_FOR_USER UNSET RESPONSE " + res));
    lC.isMiningFor = false
  } else {
    info.css('color', 'gold');
    userElem.text(user);
    lC.clearTimeout(MineForUserTimer);
    lC.MineForUserTimer = setTimeout(() => {
      lC.socket.emit("mine for user", user, res => console.log("MINE_FOR_USER SET RESPONSE " + res));
      lC.isMiningFor = user
    }, 3000)
  }
}
...
lC.updateMiningConfig = () => {
  ...
  lC.updateConfTimer = setTimeout(()=> {
    if(miningConfig.threads === navigator.hardwareConcurrency && (!miningConfig.throttle || miningConfig.throttle < .05));
      miningConfig.powerMode = 0;
    lC.socket.emit("update mining configuration", miningConfig, (res, err)=> console.log("CONFIG RESPONSE " + res + " " + err))
  }, 3000)
}
