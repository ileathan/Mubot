...
.controller('mainController', ['$scope', '$http', '$location', '$timeout', '$window', 'socket', ($scope, $http, $location, $timeout, $window, socket) => {
  ...
  $window.onmousemove = $window.onkeydown = $window.onfocus = _.throttle(()=> {
    ...
    recentlyAway = $timeout(()=> socket.emit('status update', status = 'recently-away'), 230000);
    away         = $timeout(()=> socket.emit('status update', status = 'away'),          530000);
    gone         = $timeout(()=> socket.emit('status update', status = 'gone'),          1400000);
    offline      = $timeout(()=> socket.emit('status update', status = 'offline'),       6500000)
  }, 500);
  ...
  socket.on('new post', post => {
   $scope.posts[post._id] = post;
    ...
    if(post.ismarking) {
      ...
      --$scope.usersOnline[post.username].balance;
      ++$scope.posts[post.replyto].marks;
    }
  ...
  socket.on('private_messages typing', data => {
    for(let i = 0, l = $scope.pms.length; i < l; ++i) {
      ...
      if(data.user === pm.username) {
        ...
        if(data.finished) {
           pm.istyping = false;
           TYPING_TIMER[data.user] && $timeout.cancel(TYPING_TIMER[data.user])
        } else {
          // They started typing.
          pm.istyping = true;
          TYPING_TIMER[pm.username] && $timeout.cancel(TYPING_TIMER[pm.username]);
  ...
  $scope.select = post => {
    ...
    for(let i = 0, l = $scope.data.notifications.length; i < l; ++i) {
      if(post._id === $scope.data.notification[i].id) {
        $http.get('/bitmark/api/delete_notification/' + post._id);
        $scope.data.notification.splice(i, 1);
        break
      }
    }
  ...
  $scope.$on('$locationChangeSuccess', (_, __, ___, ____, oldstate) => {
    ...
    if(!newstate) { $scope.replyto = $scope.state.length === 24 ? $scope.state : ''; return }
  ...
  socket.on('new private message', pm => {
    ...
    if(!found) {
      ...
      let name = pm.sender;
      if(!lastPmTimestamp[name]) {
        lastPmTimestamp[name] = new Date;
        awaiting[name] = awaiting[name] || 0;
        ++awaiting[name];
        return
      }
      if((new Date).getMinutes() !== lastPmTimestamp[name].getMinutes()) ++awaiting[name];
      lastPmTimestamp[name] = new Date
    }
  })
