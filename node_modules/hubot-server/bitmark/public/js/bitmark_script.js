...
.controller('mainController', ['$scope', '$http', '$location', '$timeout', '$window', 'socket', ($scope, $http, $location, $timeout, $window, socket) => {
  ...
  $scope.offline = _.debounce(() => socket.emit("status update", { status: 'offline' }), 6000000);
  $scope.away = _.debounce(() => socket.emit("status update", { status: 'away' }), 230000);
  $scope.gone = _.debounce(() => socket.emit("status update", { status: 'gone' }), 1200000);
  socket.on('status update', user => $scope.usersOnline[user.username].status = user.status)
