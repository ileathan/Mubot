...
($scope, $http, $location, $rootScope, $timeout, $window, socket) => {
  ...
  $scope.loaded = {}; $scope.data.message = ""; data.marking_msg = "";
  ...
  socket.on("new notification", data => !console.log(data) && ($scope.data.notifications = data.notifications) && $scope.notificationAmount++);
  ...
  $scope.reasonBlur = post => {
    ...
    $timeout(() => {
      ...
      if (!$scope.cancel) {
        data.message_needed[post._id] = false;
        $scope.mark(post)
      }
    }, 200)
  };
  ...
  $scope.mark = post => {
    data.message_needed[post._id] = false;
    post.marking_msg = data.marking_msg; data.marking_msg = "";
  ...
  socket.on("new private message", pm => {
    for(let i = 0, l = $scope.pms.length; i < l; i++) {
      if(pm.sender === $scope.pms[i].user.username || pm.receiver === $scope.pms[i].user.username) {
        $scope.pms[i].history.unshift(pm)
      }
    }
  })
  ...
  $scope.create = $event => {
    $http.post("/bitmark/api/create", { 'message': data.message, 'replyto': $scope.replyto }).then(res => {
      $event.preventDefault();
      data.message = "";
      $event.target.blur();
      $event.target.rows = 1;
