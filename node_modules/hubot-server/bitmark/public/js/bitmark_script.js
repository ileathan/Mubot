...
.controller('mainController', ($scope, $http, $location, $rootScope, $timeout, $window, socket) => {
  ...
  $http.get("/bitmark/api/info").then((res) => {
    $scope.private_messages = res.data.private_messages;
    for(let i = 0, l = $scope.private_messages.length; i < l; i++) {
      socket.emit("need pm data", $scope.private_messages[i].user.username)
    }
    $scope.data = res.data;
    var amount = 0;
  ...
  socket.on("new post", post => {
    ...
    // Post is replying to you.
    if(post.replyto_user === $scope.data.username) {
      // Post is marking a pm of yours.
      if(!post.replyto) {
        for(let i = 0, l = $scope.private_messages.length; i < l; i++) {
          if($scope.private_messages[i].user.username === post.username) {
            socket.emit("need pm data", post.username)
          }
        }
      }
    }
  ...
  socket.on("new notification", data => ($scope.data.notifications = data.notifications) && $scope.notificationAmount++);
  ...
  $scope.closePmWindow = pm => {
    var found;
    for(let i = 0, l = $scope.private_messages.length; i < l; i++) {
      if(pm === $scope.private_messages[i]) {
        $scope.private_messages.splice(i, 1);
        found = i;
      }
    }
    for(let i = found-1; i >= 0; i--) {
      rightOffset = parseInt($scope.private_messages[i].style.right.slice(0, -2));
      rightOffset -= PM_WIDTH + 10;
      $scope.private_messages[i].style.right = rightOffset + 'px';
    }
    socket.emit("private_messages change", $scope.private_messages)
  };
  $scope.ignoreFor2 = false;
  $scope.isIgnore2 = () => {
    if($scope.ignoreFor2) {
      $scope.ignoreFor2--
      return true;
  ...
  $scope.getAll2 = (sender, user, msg, history, index) => {
   index = history.length - index - 1;
   var results = [];
    var i = index;
    while(history[i-1] && $scope.sameMin(history[i].date, history[i-1].date)) {
      results.push(history[i].message);
      i--;
    }
    if(i < index) {
      results.push(history[i].message);
      $scope.ignoreFor2 = results.length-1;
      return results.join(' ');
   } else {
  ...
  $(window).on("resize", _.throttle(function () {
      $scope.$apply(function(){
        var len = $scope.private_messages.length;
        if((len) * PM_WIDTH + (len * 10) + 170 > $window.innerWidth) {
          $scope.private_messages.shift()
        }
      });
  }, 100));

  $scope.ignoreFor = false;
  $scope.isIgnore = () => {
    if($scope.ignoreFor) {
      $scope.ignoreFor--
      return true;
  ...
  $scope.getAll = (history, index) => {
   ...
   var results = [];
    ...
    if(i < index) {
      results.push(history[i].message);
      $scope.ignoreFor = results.length-1;
     // $scope.ignoreFor = index === 1 ? 0 : results.length-1;
      return results.join(' ');
   } else {
      return history[index].message;
    }
  ...
  function updatePms(pms) {
    if(!pms[0]) return;
    for(let i = 0, l = $scope.private_messages.length; i < l; i++) {
      if(pms[0].receiver === $scope.private_messages[i].user.username || pms[0].sender === $scope.private_messages[i].user.username) {
        $scope.private_messages[i].history = $scope.private_messages[i].history.concat(pms)
      }
    }
  }
  ...
  socket.on("private_messages typing", data => {
    for(let i = 0, l = $scope.private_messages.length; i < l; i++) {
      if(data.user === $scope.private_messages[i].user.username) {
        if(data.finished) {
           $scope.private_messages[i].istyping = false;
  ...
  function startTypingTimer(index) {
    var ref = $scope.private_messages[index];
    ref.istyping = true;
  ...
  socket.on("new private message", pm => {
    for(let i = 0, l = $scope.private_messages.length; i < l; i++) {
      if(pm.sender === $scope.private_messages[i].user.username || pm.receiver === $scope.private_messages[i].user.username) {
        $scope.private_messages[i].history.unshift(pm)
      }
    }
  })
  $scope.createPm = ($event, pm) => {
    $event.preventDefault();
    socket.emit("new private message", pm)
    pm.text = "";
  };
  $scope.create = $event => {
