...
.controller('mainController', ['$scope', '$http', '$location', '$timeout', '$window', 'socket', ($scope, $http, $location, $timeout, $window, socket) => {
  ...
  $http.get('/bitmark/api/posts').then((res) => {
    for(let i = 0, l = res.data.length; i < l; ++i) {
      ...
      //$scope.state === post._id && ($scope.selectedPost = post)
    }
  });
  ...
  $(window).on('resize', _.throttle(function () {
      $scope.$apply(function(){
        ...
        if(len * (PM_WIDTH + 10) + 150 > $window.innerWidth) {
  ...
  $scope.privateMessage = user => {
    ...
    if((len+1) * (PM_WIDTH + 10) + 150 > $window.innerWidth) {
      $scope.pms.shift()
  ...
  $scope.mark = post => {
    ...
    $scope.data.username !== post.username && $http.post('/bitmark/api/mark/', post).then(()=> $scope.data.balance && --$scope.data.balance)
  };
  $scope.updateState = (path, state, scroll) => {
    ...
    $location.new = {
      ...
      state: state,
      scrollto: scroll,
      allscrollto: !$location.old.replyto ? scroll : $location.old.allscrollto // || $location.old.scrollto || scroll
    };
  ...
  $scope.$on('$locationChangeSuccess', (_, __, ___, ____, oldstate) => {
    ...
    setTimeout(() => {
      window.scrollTo(0, (!state_data.replyto ? state_data.allscrollto : oldstate.scrollto || 0));
    }, 0)
  });
  $scope.postGlow = post => {
  ...
  $scope.createPm = ($event, message, username) => {
    ...
    var last;
    for(let i = 0, l = $scope.pms.length; i<l; ++i) username === $scope.pms[i].username && (last = new Date($scope.pms[i].history[$scope.pms[i].history.length-1].date));
    var sameMin = (new Date()).getMinutes() === last.getMinutes();
