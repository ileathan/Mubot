...
.controller('mainController', ($scope, $http, $location, $rootScope, $timeout, $window, socket) => {
  ...
  $scope.pmMergeIndex = 0; $scope.allowIndex = false;
...
//alert(index + " " + i)
  ...
  console.log(pms)
    if(!pms) return;
    for(let i = 0, l = $scope.private_messages.length; i < l; i++) {
      if(pms[0].receiver === $scope.private_messages[i].user.username ||
      pms[0].sender === $scope.private_messages[i].user.username) {
