...
($scope, $http, $location, $timeout, $window, socket) => {
  ...
  $http.get("/bitmark/api/info").then((res) => {
    ...
    $scope.data = res.data;
    //for(user in $scope.data.awaiting_pm) {
    //  for(pm of $scope.pms) if(user === pm.username)
    //}
    var amount = 0;
  ...
  $scope.togglePmWindow = pm => {
    ...
    if(pm.maximized) delete $scope.data.awaiting_pm[pm.username]
