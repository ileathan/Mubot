...
.controller('mainController', ['$scope', '$http', '$location', '$timeout', '$window', 'socket', ($scope, $http, $location, $timeout, $window, socket) => {
  ...
  $scope.replyto = ''; $scope.loaded = {}; $scope.usersOnline = {}; $scope.data = {};
...
console.log("now")
  ...
  $scope.createPm = ($event, message, username) => {
    ...
    var last;
    for(let i = 0, l = $scope.pms.length; i<l; ++i) if(username === $scope.pms[i].username) last = new Date($scope.pms[i].history[$scope.pms[i].history.length-1].date);
