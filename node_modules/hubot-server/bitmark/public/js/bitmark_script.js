...
($scope, $http, $location, $timeout, $window, socket) => {
  ...
  $scope.data = {}; $scope.data.message_needed = {}; $scope.replyto = ""; $scope.selected_style = "";
  $scope.loaded = {}; $scope.data.message = ""; $scope.data.marking_msg = "";
  $scope.usersOnline = {}; $scope.amountOnline = 0; $scope.pms = []; $scope.data.awaiting_pm = {}
  $scope.pmMergeIndex = 0; $scope.allowIndex = false; $scope.data.mouserOverSomething = false;
...
console.log(JSON.stringify(res.data))
    ...
    for(let i = 0, l = $scope.pms.length; i < l; i++) {
      // I merge in exists: true here so that the backend knows this window isnt being freshly opened.
    ...
    // Took me 5 hours to find this bug... mongoose can be cancer.
    // 1.) it returns a 'document' that looks like perfect JSON.
    // 2.) nested objected cant be read withont the following line.
    $scope.data = res.data
  ...
  socket.on("new post", post => {
    ...
    if(post.ismarking) {
      if($scope.usersOnline[post.replyto_user]) $scope.usersOnline[post.replyto_user].balance++
      if($scope.usersOnline[post.username]) $scope.usersOnline[post.username].balance--
    }
  ...
  $(window).on("resize", _.throttle(function () {
      $scope.$apply(function(){
        if($scope.selected_style) {
          if($window.innerWidth > 950) $scope.selected_style = { 'width': '45vw' }
          if($window.innerWidth < 951) $scope.selected_style = { 'width': '65vw' }
          if($window.innerWidth < 751) $scope.selected_style = { 'width': '90vw' }
        }
  ...
  $scope.togglePmWindow = pm => {
    ...
    if(pm.maximized) if($scope.data.awaiting_pm[pm.username]) delete $scope.data.awaiting_pm[pm.username]
  ...
  function startTypingTimer(index) {
    ...
    ref.istyping = true;
    if(typingTimer[ref.username]) {
      $timeout.cancel(typingTimer[ref.username])
    }
    typingTimer[ref.username] = $timeout(()=> {
      ref.istyping = false;
    }, 5750)
  };
  //openPmWindow
  ...
  $scope.select = post => {
    ...
    $scope.selected_style = {"width": "45vw"};
    if($window.innerWidth < 951) $scope.selected_style = { 'width': '65vw' }
    if($window.innerWidth < 751) $scope.selected_style = { 'width': '90vw' }
