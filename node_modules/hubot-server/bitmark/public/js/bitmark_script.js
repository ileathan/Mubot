...
.controller('mainController', ['$scope', '$http', '$location', '$timeout', '$window', 'socket', ($scope, $http, $location, $timeout, $window, socket) => {
  ...
  var throttled = _.throttle(()=> {
    ...
    status = 'online';
    if(recentlyAway) {
      $timeout.cancel(recentlyAway);
      $timeout.cancel(away);
      $timeout.cancel(gone);
      $timeout.cancel(offline)
    }
    recentlyAway = $timeout(()=> (status = 'recently-away') && socket.emit('status update', 'recently-away'), 230000);
    away = $timeout(()=> (status = 'away') && socket.emit('status update', 'away'), 530000);
    gone = $timeout(()=> (status = 'gone') && socket.emit('status update', 'gone'), 1400000);
    offline = $timeout(()=> (Status = 'offline') && socket.emit('status update', 'offline'), 6500000)
  }, 500);
  $window.onmousemove = $window.onkeydown = $window.onfocus = throttled;
  $window.onblur = () => {
    return $timeout(() => {
      if(status === 'away') return;
      if(recentlyAway) { $timeout.cancel(recentlyAway); $timeout.cancel(away) }
      status = 'away';
  ...
  socket.on('status update', user => {
    console.log(user);
    user.status === 'offline' ? delete $scope.usersOnline[user.username] : $scope.usersOnline[user.username].status = user.status
  })
  ...
  $scope.$on('$locationChangeSuccess', (_, __, ___, ____, oldstate) => {
    ...
    if (!state_data) { $scope.replyto = $scope.state.length === 24 ? $scope.state : ''; return }
