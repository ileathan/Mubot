...
.controller('mainController', ['$scope', '$http', '$location', '$timeout', '$window', 'socket', ($scope, $http, $location, $timeout, $window, socket) => {
  ...
  $scope.replyto = ""; $scope.selected_post = ""; $scope.loaded = {}; $scope.usersOnline = {};
  // $scope.data = {}; $scope.data.message_needed = {}; $scope.data.awaiting_pm = {};
  // $scope.data.message = ""; $scope.data.marking_msg = ""; $scope.data.mouserOverSomething = false;
  // $scope.pmMergeIndex = 0; $scope.allowIndex = false; $scope.amountOnline = 0; $scope.pms = [];
...
console.log(res.data);
  ...
  socket.on('connect_error', data => console.log('CONNECTION ERROR ' + data))
  socket.on('connect_timeout', data => console.log('CONNECTION TIMEOUT ' + data))
  socket.on('disconnect', data => console.log('DISCONNECT ' + data))
  socket.on('reconnect', data => console.log('RECONNECTION ' + data))
  ...
  $(window).on("resize", _.throttle(function () {
      $scope.$apply(function(){
        if($scope.selected_post) {
          if($window.innerWidth > 950) $scope.selected_post = { 'width': '45vw' }
