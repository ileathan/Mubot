...
.controller('mainController', function($scope, $http, $interval, $timeout, $location) {
  ...
  $scope.updateState = function (path, state) {
    ...
    try { if ($location.new.state_cookie === $location.old.state_cookie) return; } catch(e) {} //Error is thrown first time page is loaded since $location.old is undefined.
    state == 'home' && (state = '');
  ...
  $scope.select = function (post) {
    ...
    window.scrollTo(0, 0);
    $scope.updateState('/bitmark/posts/', post._id);
  };
