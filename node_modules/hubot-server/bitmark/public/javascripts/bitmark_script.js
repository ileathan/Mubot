...
.controller('mainController', ($scope, $http, $interval, $timeout, $location, $rootScope) => {
  ...
  $scope.create = ($event) => {
    $http.post("/bitmark/api/create", { 'message': $rootScope.message, 'replyto': $scope.replyto }).then((res) => {
      $rootScope.message = ""; $event.target.blur(); $event.target.rows = 1;
    },err=>{alert(2)}).catch(err=>{alert(3)})
  };
  $scope.cancelMarking = (post) => {
    $rootScope.message_needed[post._id] = false; $rootScope.marking_msg = ''; $scope.cancel = true;
  };
  $scope.logout = () => {
