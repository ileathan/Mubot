      ...
      <div id="mn" class="collapse navbar-collapse" style="padding-right:25px;">
        ...
        <ul class="nav navbar-nav navbar-right">
          <li class="dropdown">
            ...
            <ul class="dropdown-menu top-right-menu">
              <li class="top-right-menu-option" ng-repeat="notification in data.notifications track by $index">
                <div ng-click="notificationClicked(notification)" id="reply" ng-style="postGlow({marks: posts[notification.id].marks})" class="posting"
                  ...
                  <div style="position: relative; bottom: 10px"class="post-body">
                    <div ng-click="profileClick(posts[notification.id].username); $event.preventBubble()">
                      <a href="/users/{{posts[notification.id].username}}" target="_parent">
                        <img class="avatar" ng-src="/uploads/{{posts[notification.id].username}}" alt="">
                        <strong>{{posts[notification.id].username}}</strong>
                      </a>
                    </div>
                    <span class="spacer"></span>
                    <small class="time">
                      <span class="timestamp">{{posts[notification.id].date | amTimeAgo}}</span>
                    </small>
                    <p style="position: relative; bottom: 10px" class="post-paragraph">{{posts[notification.id].message}}</p>
                    <div style="float:right">
                      <button style="position: relative; bottom: 25px" type="button" class="btnMark">
                        <span class="iconMark"></span><span class="markAmnt">{{posts[notification.id].marks}}</span>
                      </button>
        ...
        <div class="navbar-form navbar-right">
          <input type="text" ng-model="search_posts.message" class="form-control search-posts" placeholder="Search Posts"
                 ng-if="!replyto"> <!-- && (state==='home' || state.length===24)"> -->
  ...
  <div ng-if="state=='wallet'">
    <div style="margin-top:25px; text-align: center; vertical-align: middle;">
      <div style="background-color: ghostwhite; padding: 10px; border-radius: 15px; box-shadow: inset 0px 0px 50px #8b8c8e;">
        <i>Address</i> <strong>{{ data.wallet }}</strong><br />
        <i>marks</i> <strong>{{ data.balance }}</strong>
        <br />
        <button ng-click="withdrawMarks(withdraw.amount, withdraw.address)">Withdraw</button>&nbsp;<input size="6"
                 ng-model="withdraw.amount" placeholder="amount"> to
  ...
  <footer class="navbar-fixed-bottom" style="width: 10%; margin-left: 90vw">
    <div ng-repeat="pm in pms track by $index" class="container">
      <div ng-style="{'right': (($index) * 310 + 160 + 'px'), 'height': pm.maximized ? '348px' : '34px'}" class="popup-box-on popup-box" id="qnimate"
        ...
        <div ng-if="pm.maximized" class="popup-messages-footer">
          <textarea msd-elastic class="pm_text" ng-keydown="$event.keyCode !== 13 && startedTyping(pm); $event.keyCode === 13 && !$event.shiftKey && createPm($event, pm.text, pm.username);"
    ...
    <div ng-if="popupUserlist" ng-mouseover="data.mouseOverSomething=true" ng-mouseleave="data.mouseOverSomething=false" id="userlist" class="popup-userlist">
      ...
      <div class="popup-userlist-tab-open">
        <a ng-click="data.mouseOverSomething = false; $parent.popupUserlist = !popupUserlist" href="#" id="chat"><span class="glyphicon glyphicon-comment"></span> {{amountOnline}} Online</a>
      </div>
    </div>
  </footer>
</body>
