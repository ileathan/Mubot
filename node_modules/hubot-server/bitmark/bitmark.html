...
<body loaded="navbar" ng-cloak ng-controller="mainController">
    ...
    <div class="topbar container-fluid">
      ...
      <div id="mn" class="collapse navbar-collapse" style="padding-right:25px;">
        ...
        <div class="navbar-form navbar-right">
          <input type="text" ng-model="search_posts.message" class="form-control search-posts" placeholder="Search Posts"
                 ng-show="!replyto"> <!-- && (state==='home' || state.length===24)"> -->
          <input type="text" ng-model="search_replies.message" class="form-control ng-pristine search-posts" placeholder="Search Replies"
                 ng-show="replyto"> <!-- && (state==='home' || state.length===24)"> -->
        </div>
      </div><!-- /.navbar-collapse -->
    </div><!-- /.container-fluid -->
  ...
  <div ng-if="state === 'home' || state === replyto"> <!-- Start home page -->
    ...
    <div ng-disable-scroll="data.mouseOverSomething" class="selected-gridcell fixed-container" id="post2">
      <div ng-click="!isDisabled && select(post, $event)" ng-if="posts[replyto]" class="gridcell post-selected" id="post">
        <div ng-style="postGlow(posts[replyto])" class="post-selected posting">
          <!-- For future use (allow images in posts), for now it just adds spacing -->
          <img ng-src="" alt=""> <!-- style="max-height: {{replyto ? '620px' : '340px' }}; max-width: {{replyto ? '620px' : '340px' }};"> example to allow larger imgs ons elected post-->
          <div class="post-body">
            <div ng-click="$event.preventBubble()">
              <a href="/users/{{ posts[replyto].username }}" target="_parent">
                <img class="avatar" ng-src="/uploads/{{posts[replyto].username}}" alt="">
                <strong>{{ posts[replyto].username }}</strong>
              </a>
            </div>
            <span class="spacer"></span>
            <small class="time"><span class="timestamp">{{ posts[replyto].date | amTimeAgo}}</span></small>
            <p class="post-paragraph">{{ posts[replyto].message }}</p>
            <div class="fright">
              <div style="position:relative">
                <div ng-click="back({'replyto': ''}); $event.stopPropagation()" ng-if="replyto" class="x x-big"><b></b><b></b><b></b><b></b></div>
              </div>
              <div ng-click="back(posts[replyto]); $event.stopPropagation()" ng-if="replyto" style="float: right;" class="x"><b></b><b></b><b></b><b></b></div>
              <button ng-click="!isDisabled && (data.message_needed[posts[replyto]._id] = true) && $event.stopPropagation()"
                ...
                <span class="iconMark"></span><span class="markAmnt">{{posts[replyto].marks}}</span>
              </button>
              <div class="bcmcontainer">
                <button class="btn-cancel-marking"
                        ng-if="data.message_needed[posts[replyto]._id]" ng-click="cancelMarking(posts[replyto]); $event.stopPropagation()" class="btn-cancel-marking-shifted">
                 Cancel
                </button>
              </div>
            </div>
            <textarea class="reason-textarea" ng-model="data.marking_msg" ng-if="data.message_needed[posts[replyto]._id]" auto-focus ng-blur="reasonBlur(posts[replyto])"
                      ng-keydown="$event.keyCode == 13 && !$event.shiftKey && mark(posts[replyto])" placeholder="Reason? (This may be left blank)">
            </textarea>
    ...
    <!-- POSTS  END-->
    <div ng-show="replyto" class="comment-container" loaded="post">
      <img class="comment-img" ng-src="/uploads/{{data.username}}">
    ...
    <div class="selected-gridcell">
      <div ng-click="!isDisabled && select(post, $event)" ng-repeat="post in replies[posts[replyto]._id] | filter: search_replies track by $index" Class="gridcell">
        <div style="display: inline-block" id="reply" ng-style="postGlow(post)" class="posting">
          ...
          <div class="post-body">
            ...
            <div style="float:right">
              ...
              <div class="bcmcontainer">
                <button class="btn-cancel-marking"
                        ng-if="data.message_needed[post._id]" ng-click="cancelMarking(post); $event.stopPropagation()" class="btn-cancel-marking-shifted">
                 Cancel
                </button>
              </div>
            </div>
            <textarea class="reason-textarea" ng-model="data.marking_msg" ng-if="data.message_needed[post._id]" auto-focus
  ...
  <footer class="navbar-fixed-bottom" style="width: 10%; margin-left: 90vw">
    <div ng-repeat="pm in pms track by $index" class="container">
      <div ng-style="{'right': (($index) * 310 + 160 + 'px'), 'height': pm.maximized?'348px':'34px'}" style="width: 300px; position: fixed; display: none; bottom: 0; border: 1px solid #b0b0b0; background-color: #ffffff"  class="popup-box-on popup-box" id="qnimate">
        <div class="popup-head" ng-click="togglePmWindow(pm)">
          <div class="popup-head-left pull-left">
            <span id="pm_status" ng-class="usersOnline[pm.username] ? 'user-online' : 'user-offline'"></span>
            <span id="pm_username">{{pm.username}}</span>
            <!-- User is typing dots -->
            ...
            <div ng-repeat="msg in pm.history track by $index">
              ...
              <div ng-if="msg.sender !== data.username && !skipSameMin()">
                <div class="direct-chat-msg doted-border">
                  ...
                  <div ng-if="msg.ismarked || msg.sender !== pm.history[$index+data.skipSameMins+1].sender" class="direct-chat-info clearfix">
                    <span onmouseleave="this.childNodes[1].style.transform = 'rotate(0deg)'" onmouseover="this.childNodes[1].style.transform='rotate(45deg)'"
                     ...
                     <span ng-if="msg.sender !== pm.history[$index+data.skipSameMins+1].sender" class="direct-chat-reply-name">{{pm.username}}</span>
                  </div>
                </div>
              </div>
              <div ng-if="msg.sender === data.username && !skipSameMin()">
                <div class="direct-chat-msg-self doted-border-self">
                  ...
                  <div ng-if="msg.ismarked || msg.sender !== pm.history[$index+data.skipSameMins+1].sender"
                    ...
                    <span ng-class="{'pm-marked':msg.ismarked}" class="direct-chat-img-reply-small pull-right">
                     ...
                     <span ng-if="msg.sender !== pm.history[$index+data.skipSameMins+1].sender" class="direct-chat-reply-name pull-right" style="margin-right: 10px;">{{data.username}}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div ng-if="pm.maximized" class="popup-messages-footer">
          <textarea msd-elastic class="pm_text" ng-keydown="startedTyping(pm); $event.keyCode == 13 && !$event.shiftKey && createPm($event, pm.text, pm.username);"
    ...
    <!-- User list -->
    <div ng-show="popupUserlist" ng-mouseover="data.mouseOverSomething=true" ng-mouseleave="data.mouseOverSomething=false" id="userlist" class="popup-userlist">
      <div class="content container-fluid bootstrap snippets">
        <div class="row row-broken" style="padding: 2px">
          <div class="chat" style="overflow: scroll;" tabindex="5000">
            <!-- offline, busy, online, -->
            <div class="chat-users">
              <div id="user-container">
                <div ng-if="user.username !== data.username" ng-repeat="user in usersOnline" ng-click="openPmWindow(user)" style="padding: 0px 10px 0px 10px;"
                     onmouseover="this.style.backgroundColor='#cbffff'" onmouseleave="this.style.backgroundColor='#fffff100'" ng-class="{'awaiting-pm': data.awaiting_pm[user.username]}">
