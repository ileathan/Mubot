...
=======
        <input type="text" ng-model="$parent.search_posts" class="form-control search-posts" placeholder="Search Posts"
               ng-show="!selected_style && (state==='home' || state.length===24)">
        <input type="text" ng-model="$parent.search_replies" class="form-control ng-pristine search-posts" placeholder="Search Replies"
>>>>>>> 372eca364027b28c0a3c2a7547a136464741314c
               ng-show="selected_style && (state==='home' || state.length===24)">
      </div>
    </div><!-- /.navbar-collapse -->
  </div><!-- /.container-fluid -->
...
<div ng-if="state != 'profile' && state != 'wallet' && state !='other'"> <!-- Start home page -->
  <div ng-if="!selected_style" class="comment-container">
    ...
    <textarea ng-focus="$event.target.style.height='120px'" ng-blur="$event.target.style.height='50px'" ng-keyup="$event.keyCode == 13 && !$event.shiftKey && create($event);"
              ng-model="$root.$$childHead.message" class="post-textarea" placeholder="What would you like to post {{data.username}}?">
    </textarea>
  </div>
  <!-- POSTS START -->
        ...
        <div ng-style="mergeStyles([selected_style,postGlow(post)])" class="posting">
          ...
          <div class="post-body">
            ...
            <div class="fright">
              ...
              <button ng-click="!isDisabled && ($root.$$childHead.message_needed[post._id] = true) && $event.stopPropagation()"
                      class="btnMark" ng-mouseover="$event.target.chi$ldNodes[1].style.transform = 'rotate(45deg)'" ng-mouseleave="$event.target.childNodes[1].style.transform = 'rotate(0deg)'">
                <span class="iconMark"></span><span class="markAmnt">{{post.marks}}</span>
              </button>
            </div>
            <textarea class="reason-textarea" ng-model="$root.$$childHead.marking_msg" ng-if="message_needed[post._id]" auto-focus ng-blur="reasonBlur(post)"
                      ng-keyup="$event.keyCode == 13 && !$event.shiftKey && !($root.$$shildHead.message_needed[post._id]=false) && mark(post)" placeholder="Reason? (This may be left blank)">
            </textarea>
    ...
    <div ng-show="selected_style" class="comment-container" loaded="post">
      ...
      <textarea ng-click="$event.target.style.height='120px'" ng-blur="$event.target.style.height='50px'" ng-keyup="$event.keyCode == 13 && !$event.shiftKey && create($event);"
                ng-model="$root.$$childHead.message" class="post-textarea" placeholder="What would you like to reply {{data.username}}?" >
    </textarea>
    </div>
    <!-- REPLIES  START-->
        ...
        <div style="display: inline-block" id="reply" class="posting">
          ...
          <div class="post-body">
            ...
            <div style="float:right">
              <button ng-click="!isDisabled && ($root.$$childHead.message_needed[post._id] = true) && $event.stopPropagation()" type="button" class="btnMark"
                      onmouseout="this.childNodes[1].style.transform = 'rotate(0deg)'" onmouseover="this.childNodes[1].style.transform = 'rotate(45deg)'">
                <span class="iconMark"></span><span class="markAmnt">{{post.marks}}</span>
              </button>
            </div>
            <textarea class="reason-textarea" ng-model="$root.$$childHead.marking_msg" ng-if="message_needed[post._id]" auto-focus
                      ng-blur="$root.$$childHead.message_needed[post._id]=false; disableClicks(); mark(post)"
                      ng-keyup="$event.keyCode == 13 && !$event.shiftKey && !($root.$$childHead.message_needed[post._id]=false) && mark(post)" placeholder="Reason? (This may be left blank)">
          </textarea>
