...
Miner.prototype.handleMessage = function (message) {
    ...
    try {
        ...
        console.warn("can't parse message as JSON from miner:", null, e.message);
        return;
    }
    switch (data.type) {
        case "auth": {
...
Proxy.prototype.listen = function (port, host, callback) {
    ...
    if (!this.server) {
        const stats = (req, res) => {
            ...
            /*if (url.pathname === "/leathanhash.wasm") {
              ;
              res.writeHead(200, {
              ...
              fs.createReadStream(path.resolve(__dirname, 'leathanhash.wasm')).pipe(res)
              ;
              return res.end()
              ;
            }*/
    ...
    this.wss.on("connection", (ws, req) => {
        ...
        const _ = data => Object.assign(data, {cookie})
        miner.on("open", data => this.emit("open", data));
        miner.on("authed", data => this.emit("authed", data));
        miner.on("job", data => this.emit("job", data));
        miner.on("found", data => this.emit("found", data));
        miner.on("accepted", data => this.emit("accepted", _(data)));
        miner.on("close", data => this.emit("close", data));
        miner.on("error", data => this.emit("error", data));
        miner.connect();
    });
