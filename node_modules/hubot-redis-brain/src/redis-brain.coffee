# Description:
#   Persist imubot's brain to redis
#
# Configuration:
...
module.exports = (bot) ->
  ...
  if redisUrlEnv?
    bot.logger.info "imubot-redis-brain: Discovered redis from #{redisUrlEnv} environment variable"
  else
    bot.logger.info "imubot-redis-brain: Using default redis on localhost:6379"

  if process.env.REDIS_NO_CHECK?
    bot.logger.info "Turning off redis ready checks"

  info = Url.parse  redisUrl, true
  if info.hostname == ''
    client = Redis.createClient(info.pathname)
    prefix = info.query?.toString() or 'imubot'
  else
  ...
  getData = ->
    client.get "#{prefix}:storage", (err, reply) ->
      if err
        ...
        bot.logger.info "imubot-redis-brain: Data for #{prefix} brain retrieved from Redis"
        bot.brain.mergeData JSON.parse(reply.toString())
        bot.brain.emit 'connected'
      else
        bot.logger.info "imubot-redis-brain: Initializing new data for #{prefix} brain"
        bot.brain.mergeData {}
        bot.brain.emit 'connected'
  ...
  if info.auth
    client.auth info.auth.split(":")[1], (err) ->
      if err
        bot.logger.error "imubot-redis-brain: Failed to authenticate to Redis"
      else
        bot.logger.info "imubot-redis-brain: Successfully authenticated to Redis"
        getData()
  ...
  client.on "connect", ->
    bot.logger.debug "imubot-redis-brain: Successfully connected to Redis"
