...
module.exports = (bot) ->
  ...
  if redisUrlEnv?
    bot.logger.info "hubot-redis-brain: Discovered redis from #{redisUrlEnv} environment variable"
  else
    bot.logger.info "hubot-redis-brain: Using default redis on localhost:6379"
  ...
  getData = ->
    client.get "#{prefix}:storage", (err, reply) ->
      if err
        ...
        bot.logger.info "hubot-redis-brain: Data for #{prefix} brain retrieved from Redis"
        bot.brain.mergeData JSON.parse(reply.toString())
        bot.brain.emit 'connected'
      else
        bot.logger.info "hubot-redis-brain: Initializing new data for #{prefix} brain"
        bot.brain.mergeData {}
        bot.brain.emit 'connected'
  ...
  if info.auth
    client.auth info.auth.split(":")[1], (err) ->
      if err
        bot.logger.error "hubot-redis-brain: Failed to authenticate to Redis"
      else
        bot.logger.info "hubot-redis-brain: Successfully authenticated to Redis"
        getData()
  ...
  client.on "connect", ->
    bot.logger.debug "hubot-redis-brain: Successfully connected to Redis"
