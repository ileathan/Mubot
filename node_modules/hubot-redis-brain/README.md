...
A mubot script to persist mubot's brain using redis
...
`npm install mubot-redis-brain --save`

Then add **mubot-redis-brain** to your `external-scripts.json`:
...
mubot-redis-brain requires a redis server to work. It uses the `REDIS_URL` environment variable for determining
...
mubot-redis-brain will automatically use the boxen-managed redis (ie by using `BOXEN_REDIS_URL`).
...
is added to mubot-redis-brain (or mubot-redis-brain needs to be updated to look
