...
(function() {
  ...
  DiscordBot = (function(superClass) {
    ...
    DiscordBot.prototype.message = function(message) {
      ...
      return this.receive(new TextMessage(user, message.content, message.id));
    };
    ...
    DiscordBot.prototype.sendMessage = function(channelId, message) {
      ...
      sendChannelMessage = function(channel, message) {
        ...
        if (hasPerm) {
          return channel.send(message).then(function(msg) {
          ...
          if (process.env.HUBOT_OWNER) {
            owner = robot.client.users.get(process.env.HUBOT_OWNER);
            return owner.send("Couldn't send message to " + channel.name + " (" + channel + ") in " + channel.guild.name + ", contact " + channel.guild.owner + " to check permissions").then(function(msg) {
              return robot.logger.debug("SUCCESS! Message sent to: " + owner.id);
            }).catch(function(err) {
              return robot.logger.debug("Error sending: " + message + "\r\n" + err);
            });
      ...
      sendUserMessage = function(user, message) {
        return user.send(message, {split: true }).then(
    ...
    DiscordBot.prototype.guildDelete = function(guild, client) {
      var rooms = (function() {
        var results = [],  chans = guild.channels;
        for (let i = 0, len = chans.length; i < len; i++) {
          results.push(chans[i].id);
        }
        return results;
      })();
