...
module.exports = (bot) ->
  ...
  bot.respond /((driving|walking|bike|biking|bicycling) )?directions from (.+) to (.+)/i, (msg) ->
    ...
    bot.http(url).query(query).get()((err, res, body) ->
  ...
  bot.respond /(?:(satellite|terrain|hybrid)[- ])?map( me)? (.+)/i, (msg) ->
