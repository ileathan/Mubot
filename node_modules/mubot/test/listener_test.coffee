...
describe 'Listener', ->
  beforeEach ->
    ...
    @bot =
      ...
      logger:
        debug: ->
      # Why is this part of the Bot object??
      Response: Response
  ...
  describe 'Unit Tests', ->
    describe '#call', ->
      it 'calls the matcher', (done) ->
        ...
        testListener = new Listener(@bot, testMatcher, callback)
      ...
      it 'passes the matcher result on to the listener callback', (done) ->
        ...
        testListener = new Listener(@bot, testMatcher, listenerCallback)
      ...
      describe 'if the matcher returns true', ->
        beforeEach ->
          @createListener = (cb) ->
            new Listener @bot, sinon.stub().returns(true), cb
      ...
      describe 'if the matcher returns false', ->
        beforeEach ->
          @createListener = (cb) ->
            new Listener @bot, sinon.stub().returns(false), cb
    ...
    describe '#constructor', ->
      it 'requires a matcher', ->
        expect(-> new Listener(@bot, undefined, {}, sinon.spy())).to.throw(Error)

      it 'requires a callback', ->
        # No options
        expect(-> new Listener(@bot, sinon.spy())).to.throw(Error)
        # With options
        expect(-> new Listener(@bot, sinon.spy(), {})).to.throw(Error)
    ...
    describe 'TextListener', ->
      describe '#matcher', ->
        it 'matches TextMessages', ->
          ...
          testListener = new TextListener(@bot, testRegex, callback)
