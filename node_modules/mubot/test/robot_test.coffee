...
# Preload the Mubot mock adapter but substitute in the latest version of Adapter
mockery.enable()
mockery.registerAllowable 'mubot-mock-adapter'
mockery.registerAllowable 'lodash' # mubot-mock-adapter uses lodash
# Force mubot-mock-adapter to use the latest version of Adapter
mockery.registerMock 'mubot/src/adapter', Adapter
# Load the mock adapter into the cache
require 'mubot-mock-adapter'
# We're done with mockery
mockery.deregisterMock 'mubot/src/adapter'
mockery.disable()


describe 'Bot', ->
  beforeEach ->
    @bot = new Bot null, 'mock-adapter', yes, 'TestMubot'
    @bot.alias = 'Mubot'
    @bot.run
  ...
  describe 'Unit Tests', ->
    describe '#http', ->
      ...
      it 'sets a sane user agent', ->
        expect(@httpClient.options.headers['User-Agent']).to.contain('Mubot')
    ...
    describe '#loadFile', ->
      ...
      describe 'proper script', ->
        ...
        it 'should call the script with the Bot', ->
          @bot.loadFile('./scripts', 'test-script.coffee')
  ...
  describe 'Listener Registration', ->
    ...
    describe '#respond', ->
      it 'matches TextMessages addressed to the bot', ->
        callback = sinon.spy()
        testMessage = new TextMessage(@user, 'TestMubot message123')
        testRegex = /message123$/
