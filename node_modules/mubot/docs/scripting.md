...
Mubot out of the box doesn't do too much but it is an extensible, scriptable bot friend. There are [hundreds of scripts written and maintained by the community](/docs/#scripts.md) and it's easy to write your own.  You can create a custom script in mubot's `scripts` directory or [create a script package](#creating-a-script-package) for sharing with the community!

## Anatomy of a script

When you created your mubot, the generator also created a `scripts` directory. If you peek around there, you will see some examples of scripts. For a script to be a script, it needs to:

* live in a directory on the mubot script load path (`src/scripts` and `scripts` by default)
...
The `bot` parameter is an instance of your bot friend. At this point, we can start scripting up some awesomeness.

## Hearing and responding

Since this is a chat bot, the most common interactions are based on messages. Mubot can `hear` messages said in a room or `respond` to messages directly addressed at it. Both methods take a regular expression and a callback function as parameters. For example:

```coffeescript
module.exports = (bot) ->
  bot.hear /badger/i, (res) ->
    # your code here

  bot.respond /open the pod bay doors/i, (res) ->
    # your code here
```

The `bot.hear /badger/` callback is called anytime a message's text matches. For example:

* Stop badgering the witness
* badger me
* what exactly is a badger anyways

The `bot.respond /open the pod bay doors/i` callback is only called for messages that are immediately preceded by the bot's name or alias. If the bot's name is HAL and alias is /, then this callback would be triggered for:
...
* HAL: please open the pod bay doors
   *  because its `respond` is bound to the text immediately following the bot name
*  has anyone ever mentioned how lovely you are when you open the pod bay doors?
   * because it lacks the bot's name
...
module.exports = (bot) ->
  ...
  bot.hear /I like pie/i, (res) ->
    res.emote "makes a freshly baked pie"
```

The `bot.hear /badgers/` callback sends a message exactly as specified regardless of who said it, "Badgers? BADGERS? WE DON'T NEED NO STINKIN BADGERS".

If a user Dave says "HAL: open the pod bay doors", `bot.respond /open the pod bay doors/i` callback sends a message "Dave: I'm afraid I can't let you do that."
...
```coffeescript
  bot.respond /open the (.*) doors/i, (res) ->
    # your code here
```
...
Mubot can make HTTP calls on your behalf to integrate & consume third party APIs. This can be through an instance of [node-scoped-http-client](https://github.com/technoweenie/node-scoped-http-client) available at `bot.http`. The simplest case looks like:


```coffeescript
  bot.http("https://midnight-train")
    .get() (err, res, body) ->
      # your code here
```
...
If you are talking to APIs, the easiest way is going to be JSON because it doesn't require any extra dependencies. When making the `bot.http` call, you should usually set the  `Accept` header to give the API a clue that's what you are expecting back. Once you get the `body` back, you can parse it with `JSON.parse`:
...
As mentioned, mubot uses [node-scoped-http-client](https://github.com/technoweenie/node-scoped-http-client) to provide a simple interface for making HTTP and HTTPS requests. Under its hood, it's using node's builtin [http](http://nodejs.org/api/http.html) and [https](http://nodejs.org/api/https.html) libraries, but providing an easy DSL for the most common kinds of interaction.

If you need to control options on http and https more directly, you pass a second argument to `bot.http` that will be passed on to node-scoped-http-client which will be passed on to http and https:

```
  options =
    # don't verify server certificate against a CA, SCARY!
    rejectUnauthorized: false
  bot.http("https://midnight-train", options)
```
...
A common pattern is to hear or respond to commands, and send with a random funny image or line of text from an array of possibilities. It's annoying to do this in JavaScript and CoffeeScript out of the box, so Mubot includes a convenience method:
...
Mubot can react to a room's topic changing, assuming that the adapter supports it.
...
Mubot can see users entering and leaving, assuming that the adapter supports it.
...
Mubot can access the environment he's running in, just like any other node program, using [`process.env`](http://nodejs.org/api/process.html#process_process_env). This can be used to configure how scripts are run, with the convention being to use the `MUBOT_` prefix.

```coffeescript
answer = process.env.MUBOT_ANSWER_TO_THE_ULTIMATE_QUESTION_OF_LIFE_THE_UNIVERSE_AND_EVERYTHING

module.exports = (bot) ->
  bot.respond /what is the answer to the ultimate question of life/, (res) ->
    res.send "#{answer}, but what is the question?"
```

Take care to make sure the script can load if it's not defined, give the Mubot developer notes on how to define it, or default to something. It's up to the script writer to decide if that should be a fatal error (e.g. mubot exits), or not (make any script that relies on it to say it needs to be configured. When possible and when it makes sense to, having a script work without any other configuration is preferred.

Here we can default to something:

```coffeescript
answer = process.env.MUBOT_ANSWER_TO_THE_ULTIMATE_QUESTION_OF_LIFE_THE_UNIVERSE_AND_EVERYTHING or 42
...
answer = process.env.MUBOT_ANSWER_TO_THE_ULTIMATE_QUESTION_OF_LIFE_THE_UNIVERSE_AND_EVERYTHING
unless answer?
  console.log "Missing MUBOT_ANSWER_TO_THE_ULTIMATE_QUESTION_OF_LIFE_THE_UNIVERSE_AND_EVERYTHING in environment: please set and try again"
  process.exit(1)
...
And lastly, we update the `bot.respond` to check it:
...
module.exports = (bot) ->
  bot.respond /what is the answer to the ultimate question of life/, (res) ->
    unless answer?
      res.send "Missing MUBOT_ANSWER_TO_THE_ULTIMATE_QUESTION_OF_LIFE_THE_UNIVERSE_AND_EVERYTHING in environment: please set and try again"
      return
    res.send "#{answer}, but what is the question?"
```
...
Mubot uses [npm](https://github.com/isaacs/npm) to manage its dependencies. To add additional packages, add them to `dependencies` in `package.json`. For example, to add lolimadeupthispackage 1.2.3, it'd look like:
...
If you are using scripts from mubot-scripts, take note of the `Dependencies` documentation in the script to add. They are listed in a format that can be copy & pasted into `package.json`, just make sure to add commas as necessary to make it valid JSON.

# Timeouts and Intervals

Mubot can run code later using JavaScript's built-in [setTimeout](http://nodejs.org/api/timers.html#timers_settimeout_callback_delay_arg). It takes a callback method, and the amount of time to wait before calling it:

```coffeescript
module.exports = (bot) ->
  bot.respond /you are a little slow/, (res) ->
    setTimeout () ->
      res.send "Who you calling 'slow'?"
    , 60 * 1000
```

Additionally, Mubot can run code on an interval using [setInterval](http://nodejs.org/api/timers.html#timers_setinterval_callback_delay_arg). It takes a callback method, and the amount of time to wait between calls:

```coffeescript
module.exports = (bot) ->
  bot.respond /annoy me/, (res) ->
...
Mubot includes support for the [express](http://expressjs.com) web framework to serve up HTTP requests. It listens on the port specified by the `EXPRESS_PORT` or `PORT` environment variables (preferred in that order) and defaults to 8080. An instance of an express application is available at `bot.router`. It can be protected with username and password by specifying `EXPRESS_USER` and `EXPRESS_PASSWORD`. It can automatically serve static files by setting `EXPRESS_STATIC`.
...
module.exports = (bot) ->
  ...
  bot.router.post '/mubot/chatsecrets/:room', (req, res) ->
...
curl -X POST -H "Content-Type: application/json" -d '{"secret":"C-TECH Astronomy"}' http://127.0.0.1:8080/mubot/chatsecrets/general

// defaults Content-Type: application/x-www-form-urlencoded, must st payload=...
curl -d 'payload=%7B%22secret%22%3A%22C-TECH+Astronomy%22%7D' http://127.0.0.1:8080/mubot/chatsecrets/general
```

All endpoint URLs should start with the literal string `/mubot` (regardless of what your bot's name is). This consistency makes it easier to set up webhooks (copy-pasteable URL) and guarantees that URLs are valid (not all bot names are URL-safe).

## Events

Mubot can also respond to events which can be used to pass data between scripts. This is done by encapsulating node.js's [EventEmitter](http://nodejs.org/api/events.html#events_class_events_eventemitter) with `bot.emit` and `bot.on`.
...
module.exports = (bot) ->
  bot.router.post "/mubot/gh-commits", (req, res) ->
    bot.emit "commit", {
        user    : {}, #mubot user object
        repo    : 'https://github.com/github/mubot',
...
module.exports = (bot) ->
  bot.on "commit", (commit) ->
    bot.send commit.user, "Will now deploy #{commit.hash} from #{commit.repo}!"
    #deploy code goes here
```

If you provide an event, it's highly recommended to include a mubot user or room object in its data. This would allow for mubot to notify a user or room in chat.

## Error Handling

No code is perfect, and errors and exceptions are to be expected. Previously, an uncaught exceptions would crash your mubot instance. Mubot now includes an `uncaughtException` handler, which provides hooks for scripts to do something about exceptions.

```coffeescript
# src/scripts/does-not-compute.coffee
module.exports = (bot) ->
  bot.error (err, res) ->
    bot.logger.error "DOES NOT COMPUTE"
...
Mubot scripts can be documented with comments at the top of their file, for example:
...
The most important and user facing of these is `Commands`. At load time, Mubot looks at the `Commands` section of each scripts, and build a list of all commands. The included `help.coffee` lets a user ask for help across all commands, or with a search. Therefore, documenting the commands make them a lot more discoverable by users.
...
* Refer to the Mubot as mubot, even if your mubot is named something else. It will automatically be replaced with the correct name. This makes it easier to share scripts without having to update docs.
* For `bot.respond` documentation, always prefix with `mubot`. Mubot will automatically replace this with your bot's name, or the bot's alias if it has one
...
The other sections are more relevant to developers of the bot, particularly dependencies, configuration variables, and notes. All contributions to [mubot-scripts](https://github.com/github/mubot-scripts) should include all these sections that are related to getting up and running with the script.



## Persistence

Mubot has an in-memory key-value store exposed as `bot.brain` that can be
used to store and retrieve data by scripts.

```coffeescript
bot.respond /have a soda/i, (res) ->
  # Get number of sodas had (coerced to a number).
  sodasHad = bot.brain.get('totalSodas') * 1 or 0
...
bot.respond /sleep it off/i, (res) ->
  bot.brain.set 'totalSodas', 0
  msg.reply 'zzzzz'
```

If the script needs to lookup user data, there are methods on `bot.brain` for looking up one or many users by id, name, or 'fuzzy' matching of name: `userForName`, `userForId`, `userForFuzzyName`, and `usersForFuzzyName`.

```coffeescript
module.exports = (bot) ->

  bot.respond /who is @?([\w .\-]+)\?*$/i, (res) ->
    name = res.match[1].trim()

    users = bot.brain.usersForFuzzyName(name)
...
* all scripts __bundled__ with your mubot installation under `scripts/` directory
* __community scripts__ specified in `mubot-scripts.json` and shipped in the `mubot-scripts` npm package
...
Once you've built some new scripts to extend the abilities of your bot friend, you should consider sharing them with the world! At the minimum, you need to package up your script and submit it to the [Node.js Package Registry](http://npmjs.org). You should also review the best practices for sharing scripts below.

## See if a script already exists

Start by [checking if an NPM package](/docs/index.md#scripts) for a script like yours already exists.  If you don't see an existing package that you can contribute to, then you can easily get started using the `mubot` script [yeoman](http://yeoman.io/) generator.

## Creating A Script Package

Creating a script package for mubot is very simple.  Start by installing the `mubot` [yeoman](http://yeoman.io/) generator:
...
Once you've got the mubot generator installed, creating a mubot script is similar to creating a new mubot.  You create a directory for your mubot script and generate a new `mubot:script` in it.  For example, if we wanted to create a mubot script called "my-awesome-script":

```
% mkdir mubot-my-awesome-script
% cd mubot-my-awesome-script
% yo mubot:script
```

At this point, the you'll be asked a few questions about the author for the script, name of the script (which is guessed by the directory name), a short description, and keywords to find it (we suggest having at least `mubot, mubot-scripts` in this list).
...
You now have a mubot script repository that's ready to roll! Feel free to crack open the pre-created `src/awesome-script.coffee` file and start building up your script! When you've got it ready, you can publish it to [npmjs](http://npmjs.org) by [following their documentation](https://docs.npmjs.com/getting-started/publishing-npm-packages)!
...
see the [Testing Mubot Scripts](#testing-mubot-scripts) section.
...
```coffeescript
module.exports = (bot) ->
  bot.respond /annoy me/, id:'annoyance.start', (msg)
    # code to annoy someone

  bot.respond /unannoy me/, id:'annoyance.stop', (msg)
...
For synchronous middleware (never yields to the event loop), mubot will automatically catch errors and emit an an `error` event, just like in standard listeners. Mubot will also automatically call the most recent `done` callback to unwind the middleware stack. Asynchronous middleware should catch its own exceptions, emit an `error` event, and call `done`. Any uncaught exceptions will interrupt all execution of middleware completion callbacks.

# Listener Middleware

Listener middleware inserts logic between the listener matching a message and the listener executing. This allows you to create extensions that run for every matching script. Examples include centralized authorization policies, rate limiting, logging, and metrics. Middleware is implemented like other mubot scripts: instead of using the `hear` and `respond` methods, middleware is registered using `listenerMiddleware`.

## Listener Middleware Examples

A fully functioning example can be found in [mubot-rate-limit](https://github.com/michaelansel/mubot-rate-limit/blob/master/src/rate-limit.coffee).
...
module.exports = (bot) ->
  bot.listenerMiddleware (context, next, done) ->
    # Log commands
    bot.logger.info "#{context.response.message.user.name} asked me to #{context.response.message.text}"
...
module.exports = (bot) ->
  ...
  bot.listenerMiddleware (context, next, done) ->
    try
      ...
      bot.emit('error', err, context.response)
```
...
```coffeescript
module.exports = (bot) ->
  bot.hear /hello/, id: 'my-hello', rateLimits: {minPeriodMs: 10000}, (msg) ->
...
This simple middlware bans mubot use by a particular user, including `hear`
...
bot.receiveMiddleware (context, next, done) ->
  if context.response.message.user.id in BLACKLISTED_USERS
    ...
    # If the message starts with 'mubot' or the alias pattern, this user was
    # explicitly trying to run a command, so respond with an error message.
    if context.response.message.text?.match(bot.respondPattern(''))
...
Response middleware runs against every message mubot sends to a chat room. It's
...
# Testing Mubot Scripts

[mubot-test-helper](https://github.com/mtsmfm/mubot-test-helper) is a good
framework for unit testing Mubot scripts.  (Note that, in order to use
mubot-test-helper, you'll need a recent Node version with support for Promises.)

Install the package in your Mubot instance:

``` % npm install mubot-test-helper --save-dev ```
...
[Mubot sample script](https://github.com/github/generator-mubot/blob/master/generators/app/templates/scripts/example.coffee).  This script uses *Mocha*, *chai*, *coffee-script*, and of course *mubot-test-helper*:

**test/example-test.coffee**
```coffeescript
Helper = require('mubot-test-helper')
chai = require 'chai'
  ...
  it 'doesn\'t need badgers', ->
    @room.user.say('alice', 'did someone call for a badger?').then =>
      expect(@room.messages).to.eql [
        ...
        ['mubot', 'Badgers? BADGERS? WE DON\'T NEED NO STINKIN BADGERS']
      ]

  it 'won\'t open the pod bay doors', ->
    @room.user.say('bob', '@mubot open the pod bay doors').then =>
      expect(@room.messages).to.eql [
        ['bob', '@mubot open the pod bay doors']
        ['mubot', '@bob I\'m afraid I can\'t let you do that.']
      ]

  it 'will open the dutch doors', ->
    @room.user.say('bob', '@mubot open the dutch doors').then =>
      expect(@room.messages).to.eql [
        ['bob', '@mubot open the dutch doors']
        ['mubot', '@bob Opening dutch doors']
      ]
```
